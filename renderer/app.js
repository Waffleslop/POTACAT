...
const setKiwiLabel1 = document.getElementById('set-kiwi-label-1');
const setKiwiLabel2 = document.getElementById('set-kiwi-label-2');
const setKiwiLabel3 = document.getElementById('set-kiwi-label-3');
...
// (NSAllowsLocalNetworking neuters NSAllowsArbitraryLoads); the
...
const LOGBOOK_DEFAULTS = {
  ...
  logger32: { port: 2237, help: 'In Logger32: Setup > WSJT-X Settings (or File > Setup > WSJT-X Interface, depending on version). Enable "Log QSOs received from WSJT-X" and set the UDP Server port to 2237. POTACAT speaks the WSJT-X binary protocol so Logger32 sees QSOs as if they came from WSJT-X. Logger32 must be running to receive.' },
...
function showTuneArc(lat, lon, freq, source) {
  ...
  for (const seg of segments) {
    ...
    for (const offset of [-360, 0, 360]) {
      ...
      // the arc stays legible against busy OSM dark tiles + clusters of
      // bright markers. K3SBP 2026-05-29.
      tuneArcLayers.push(
...
const PRIVILEGE_RANGES = {
  us_extra: [
    ...
    [5330.5, 5333.5, 'all'], [5346.5, 5349.5, 'all'], [5351.5, 5366.5, 'all'], [5371.5, 5374.5, 'all'], [5403.5, 5406.5, 'all'],
    [7000, 7125, 'cw_digi'], [7125, 7300, 'phone'], [10100, 10150, 'cw_digi'],
    [14000, 14150, 'cw_digi'], [14150, 14350, 'phone'], [18068, 18110, 'cw_digi'], [18110, 18168, 'phone'],
    [21000, 21200, 'cw_digi'], [21200, 21450, 'phone'], [24890, 24930, 'cw_digi'], [24930, 24990, 'all'],
    [28000, 28300, 'cw_digi'], [28300, 29700, 'phone'], [50000, 50100, 'cw'], [50100, 54000, 'all'],
    [144000, 144100, 'cw'], [144100, 148000, 'all'], [420000, 450000, 'all'],
    [902000, 928000, 'all'], [1240000, 1300000, 'all'],
  ],
  us_advanced: [
    [1800, 2000, 'all'], [3525, 3600, 'cw_digi'], [3700, 4000, 'phone'],
    [5330.5, 5333.5, 'all'], [5346.5, 5349.5, 'all'], [5351.5, 5366.5, 'all'], [5371.5, 5374.5, 'all'], [5403.5, 5406.5, 'all'],
    [7025, 7125, 'cw_digi'], [7125, 7300, 'phone'], [10100, 10150, 'cw_digi'],
    [14025, 14150, 'cw_digi'], [14175, 14350, 'phone'], [18068, 18110, 'cw_digi'], [18110, 18168, 'phone'],
    [21000, 21200, 'cw_digi'], [21225, 21450, 'phone'], [24890, 24930, 'cw_digi'], [24930, 24990, 'all'],
    [28000, 28300, 'cw_digi'], [28300, 29700, 'phone'], [50000, 50100, 'cw'], [50100, 54000, 'all'],
    [144000, 144100, 'cw'], [144100, 148000, 'all'], [420000, 450000, 'all'],
    [902000, 928000, 'all'], [1240000, 1300000, 'all'],
  ],
  us_general: [
    ...
    [5330.5, 5333.5, 'all'], [5346.5, 5349.5, 'all'], [5351.5, 5366.5, 'all'], [5371.5, 5374.5, 'all'], [5403.5, 5406.5, 'all'],
    [7025, 7125, 'cw_digi'], [7175, 7300, 'phone'], [10100, 10150, 'cw_digi'],
    [14025, 14150, 'cw_digi'], [14225, 14350, 'phone'], [18068, 18110, 'cw_digi'], [18110, 18168, 'phone'],
    [21025, 21200, 'cw_digi'], [21275, 21450, 'phone'], [24890, 24930, 'cw_digi'], [24930, 24990, 'all'],
    [28000, 28300, 'cw_digi'], [28300, 29700, 'phone'], [50000, 50100, 'cw'], [50100, 54000, 'all'],
    [144000, 144100, 'cw'], [144100, 148000, 'all'], [420000, 450000, 'all'],
    [902000, 928000, 'all'], [1240000, 1300000, 'all'],
  ],
  us_technician: [
    ...
    [28000, 28300, 'cw_digi'], [28300, 28500, 'phone'], [50000, 50100, 'cw'], [50100, 54000, 'all'],
    [144000, 144100, 'cw'], [144100, 148000, 'all'], [420000, 450000, 'all'],
    [902000, 928000, 'all'], [1240000, 1300000, 'all'],
  ],
  ca_basic: [
...
async function openSettingsDialog(tab) {
  ...
  if (setKiwiLabel1) setKiwiLabel1.value = s.kiwiSdrLabel1 || '';
  if (setKiwiLabel2) setKiwiLabel2.value = s.kiwiSdrLabel2 || '';
  if (setKiwiLabel3) setKiwiLabel3.value = s.kiwiSdrLabel3 || '';
...
settingsSave.addEventListener('click', async () => {
  ...
  await window.api.saveSettings({
    ...
    kiwiSdrLabel1: setKiwiLabel1 ? setKiwiLabel1.value.trim() : '',
    kiwiSdrLabel2: setKiwiLabel2 ? setKiwiLabel2.value.trim() : '',
    kiwiSdrLabel3: setKiwiLabel3 ? setKiwiLabel3.value.trim() : '',
...
// --- FreeDV Digital Voice (mode-driven, no dedicated view) ---
// FreeDV activates/deactivates automatically based on the current mode.
// When tuned to a FreeDV spot, the engine starts and PTT encodes FreeDV.
// When tuned to SSB/CW/etc, FreeDV stops and PTT is normal audio.

let freedvActive = false;     // true when FreeDV engine is running
let freedvAudioCtx = null;
let freedvAudioStream = null;
let freedvProcessor = null;
let freedvRxPlaybackCtx = null;
let freedvTxOutputCtx = null;
let freedvMicStream = null;
let freedvMicCtx = null;
let freedvMicProcessor = null;

const FREEDV_MODES = new Set(['FREEDV', 'DV', 'FREEDV-700E', 'FREEDV-700D', 'FREEDV-1600']);

function isFreedvMode(mode) {
  if (!mode) return false;
  return FREEDV_MODES.has(mode.toUpperCase()) || mode.toUpperCase().startsWith('FREEDV');
}
...
async function startFreedvForMode(mode) {
  ...
  // Determine codec mode from spot mode string
  let codecMode = '700E';
  const upper = (mode || '').toUpperCase();
  if (upper.includes('700D')) codecMode = '700D';
  ...
  try {
    ...
    console.error('[FreeDV] RX audio capture failed:', err.message);
  }
}
...
function stopFreedv() {
  ...
  freedvActive = false;
  console.log('[FreeDV] Auto-stopping');
  ...
  stopFreedvTx();
  // Hide sync indicator
  const syncEl = document.getElementById('freedv-sync-indicator');
  if (syncEl) syncEl.classList.add('hidden');
}

async function startFreedvTx() {
  if (!freedvActive) return;
  window.api.freedvSetTx(true);
  // Capture mic for TX speech
  try {
    ...
    freedvMicProcessor.port.onmessage = function(e) { window.api.freedvTxAudio(e.data); };
...
// Sync indicator in status bar (appears only when FreeDV is active)
window.api.onFreedvSync((data) => {
  const syncEl = document.getElementById('freedv-sync-indicator');
  if (!syncEl) return;
  if (freedvActive) {
    syncEl.classList.remove('hidden');
    syncEl.style.background = data.sync ? '#4ecca3' : '#f0a500';
    syncEl.title = `FreeDV ${data.sync ? 'synced' : 'searching'} SNR: ${data.snr != null ? data.snr.toFixed(1) : '--'} dB`;
  }
});
...
// Auto-stop: tuned away from FreeDV
window.api.onFreedvAutoStop(() => {
  stopFreedv();
});

// RX speech playback — decoded voice through speakers
window.api.onFreedvRxSpeech(async (data) => {
  try {
    if (!freedvRxPlaybackCtx || freedvRxPlaybackCtx.state === 'closed') {
      freedvRxPlaybackCtx = new AudioContext({ sampleRate: 8000 });
    }
    const samples = new Int16Array(data.samples);
    const floats = new Float32Array(samples.length);
    for (let i = 0; i < samples.length; i++) floats[i] = samples[i] / 32768;
    const buffer = freedvRxPlaybackCtx.createBuffer(1, floats.length, 8000);
    buffer.getChannelData(0).set(floats);
    const source = freedvRxPlaybackCtx.createBufferSource();
    source.buffer = buffer;
    source.connect(freedvRxPlaybackCtx.destination);
    source.start();
  } catch {}
});

// TX modem playback — modulated signal to radio USB audio output
window.api.onFreedvTxModem(async (data) => {
  try {
    ...
    if (freedvTxOutputCtx.state === 'suspended') await freedvTxOutputCtx.resume();
    ...
    const floats = new Float32Array(samples.length);
    for (let i = 0; i < samples.length; i++) floats[i] = samples[i] / 32768;
    const buffer = freedvTxOutputCtx.createBuffer(1, floats.length, 8000);
    buffer.getChannelData(0).set(floats);
    const source = freedvTxOutputCtx.createBufferSource();
    source.buffer = buffer;
