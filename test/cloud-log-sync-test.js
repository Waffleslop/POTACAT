...
const QSO_DATA = {
  ...
  uuid: 'uuid-new-qso', respot: true, skipLogbookForward: false,
};
...
check('buildAdifFields: real ADIF names, FREQ in MHz, 6-char TIME_ON, no internal keys', () => {
  ...
  assert.strictEqual(f.APP_POTACAT_UUID, 'uuid-new-qso');
...
// ── pseudo-name repair ────────────────────────────────────────────────────
const PSEUDO = {
  BAND: '40m', MODE: 'FT8', UUID: 'u-pseudo', TIMEON: '0159', QSODATE: '20260402',
  RSTRCVD: '+00', RSTSENT: '+01', CALLSIGN: 'W3BOO', OPERATOR: 'K3SBP', FREQUENCY: '7074',
  MYSIGINFO: 'US-1234', APP_POTACAT_UUID: 'u-pseudo', APP_POTACAT_VERSION: '1',
};
...
check('normalizeAdifFields: maps pseudo keys, converts kHz, never clobbers real values', () => {
  ...
  assert.strictEqual(n.APP_POTACAT_UUID, 'u-pseudo');
...
check('mutateLog: a rewrite keeps pseudo-named records (it used to delete them)', () => {
  const file = path.join(freshDir(), 'log.adi');
  writeLog(file, [{ CALL: 'K1ABC', QSO_DATE: '20260101', TIME_ON: '1200', APP_POTACAT_UUID: 'a' }, PSEUDO]);
...
check('appendRecords: assigns identity, keeps a unique incoming uuid, replaces a colliding one', () => {
  const file = seededLog([A]);
  const sink = recordingSink();
  const recs = [{ CALL: 'X1' }, { CALL: 'X2', APP_POTACAT_UUID: 'foreign' }, { CALL: 'X3', APP_POTACAT_UUID: 'ua' }];
  ...
  assert.strictEqual(qsos[2].APP_POTACAT_UUID, 'foreign');
...
function cloudEnv(settingsOverride) {
  ...
  const settings = { cloudSyncEnabled: false, cloudAccessToken: 'tok', ...settingsOverride };
  ...
  return { dir, logPath, settings, cloud, sink };
}
...
check('recordLogChanges: journaled while signed in even with auto-sync OFF; nothing when signed out', () => {
  ...
  assert.deepStrictEqual([e.uuid, e.action, e.version, e.adifFields.CALL, e.adifFields.FREQ], ['uuid-new-qso', 'create', 1, 'W1AW', '14.074000']);
...
check('pull merge: pseudo-named rows from the server land with real names', () => {
  ...
  env.cloud.getSyncCallbacks().onPulled([{ uuid: 'u-pseudo', version: 1, adifFields: PSEUDO }]);
  const got = parseAllRawQsos(env.logPath).find((q) => q.APP_POTACAT_UUID === 'u-pseudo');
...
check('cloud-sync-now: pushes, merges pulls, persists the cursor, reports counts', async () => {
  ...
  fakeServer(client, (m, url, body) => {
    ...
    return { qsos: [{ uuid: 'remote-1', version: 1, adifFields: B, updatedAt: '2026-09-17T20:00:00Z' }], hasMore: false };
  });
