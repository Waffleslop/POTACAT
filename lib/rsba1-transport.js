...
function buildControl(type, seq, sentid, rcvdid) {
  ...
  buf.writeUInt16LE(seq & 0xffff,      6); // mask: counter must wrap at 65535
...
class IcomUdpStream extends EventEmitter {
  ...
  _sendPing() {
    ...
    this._send(buildPing(this.pingSeq & 0xffff, this.myId, this.remoteId, t, false));
  ...
  _sendIdle() {
    if (this.trackedIdle) {
      ...
      this._sendControl(TYPE_IDLE, this.seq & 0xffff);
      this.seq = (this.seq + 1) & 0xffff;
    }
  }
...
class ControlStream extends IcomUdpStream {
  ...
  _onCapabilities(msg) {
    ...
    // The radio names its own CI-V address here. Until 2026-09-14 this was
    // logged and dropped while the CI-V codec kept addressing whatever the
    // rig model said (0x94 for a blank model) — and an Icom ignores every
    // frame not addressed to it, so an IC-7760 (0xB2) on a blank model had a
    // perfect RS-BA1 session and no CAT at all. The transport forwards this
    // so the connect path can re-address the codec.
    this.emit('radio-info', this.radioInfo);
...
class RsBa1Transport extends EventEmitter {
  ...
  /** The radio as it described itself in the Capabilities packet (name, civAddress, ...), or null. */
  get radioInfo()      { return this.control && this.control.radioInfo ? this.control.radioInfo : null; }
  ...
  connect({ host, controlPort = 50001, civPort, username = '', password = '', compName = 'POTACAT', enableRxAudio = false, enableTxAudio = false, rxAudioCodec = AUDIO_CODEC_LPCM16_MONO, rxAudioSampleRate = 48000, txAudioCodec = AUDIO_CODEC_LPCM16_MONO, txAudioSampleRate = ICOM_AUDIO_TX_SAMPLE_RATE, txAudioBufferMs = DEFAULT_TX_AUDIO_BUFFER_MS, controlAuthMode = 'direct', timeoutMs = HANDSHAKE_TIMEOUT } = {}) {
    ...
    dns.lookup(rawHost, { family: 4 })
      .then(async ({ address, family }) => {
        ...
        this.control.on('radio-info', (info) => {
          if (connectToken !== this._connectToken) return;
          this.emit('radio-info', info);
        });
