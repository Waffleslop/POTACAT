...
class QrzClient {
  ...
  static async fetchLogbook(apiKey, callsign) {
    ...
    const resp = await httpPost('https://logbook.qrz.com/api', body, ua);
    // QRZ returns RESULT=OK&COUNT=nnn&ADIF=<adif data...> or RESULT=FAIL&REASON=...
    // The ADIF data runs to end-of-response and may contain '&', so extract it
    // directly from the raw text instead of relying on &-split parsing.
    const parsed = parseQrzResponse(resp);
    ...
    if (parsed.RESULT === 'OK' && adifRaw) {
      return adifRaw;
    }
    if (parsed.RESULT === 'OK' && (parsed.COUNT === '0' || !adifRaw)) {
