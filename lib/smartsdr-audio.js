...
class SmartSdrAudio extends EventEmitter {
  ...
  pushTxAudioChunk(mono24k) {
    // Diagnostics for "keyed, 0 W": how many chunks were refused and why,
    // and how many packets actually went to the radio (see streamTxStats).
    if (!this._streamStats) this._streamStats = { chunks: 0, refused: 0, reason: '', packets: 0 };
    this._streamStats.chunks++;
    if (!this._connected || !this._udpSock || !this._txStreamId || !this._host) {
      this._streamStats.refused++;
      this._streamStats.reason = !this._connected ? 'audio not connected' : !this._udpSock ? 'no UDP socket' : !this._txStreamId ? 'no dax_tx stream' : 'radio host unknown';
      return;
    }
    ...
    if (this._txInFlight) { this._streamStats.refused++; this._streamStats.reason = 'a one-shot TX was in flight'; return; }
  ...
  /** Streamed-TX counters since the last call, then cleared. */
  takeStreamTxStats() {
    const s = this._streamStats || { chunks: 0, refused: 0, reason: '', packets: 0 };
    this._streamStats = null;
    return { ...s, streamId: this._txStreamId ? '0x' + this._txStreamId.toString(16) : null, host: this._host || null };
  }
