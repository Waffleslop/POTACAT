...
class Ft8Engine extends EventEmitter {
  ...
  _onWorkerMessage(msg) {
    ...
    if (msg.type === 'decode-result') {
      ...
      } else if (this._hasEverDecoded && this._lastCycleMax > 0.1) {
  ...
  _onCycleBoundary() {
    ...
    // Silence watchdog: if multiple consecutive cycles have zero audio, emit event
    // so the renderer can restart audio capture
    if (maxSample === 0 && off > 10000) {
      this._silentCycles = (this._silentCycles || 0) + 1;
      if (this._silentCycles >= 3) {
        console.warn(`[JTCAT] ${this._silentCycles} consecutive silent cycles — requesting audio restart`);
        this.emit('silent');
