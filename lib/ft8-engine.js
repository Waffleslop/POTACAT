...
class Ft8Engine extends EventEmitter {
  ...
  tryImmediateTx() {
    ...
    // Late-start tolerance — beyond this, defer to the next compatible
    // slot. The renderer's leading-silence pad ALWAYS lands the envelope
    // at slot+500ms or later, so the on-air DT seen by receivers is
    // (msIntoCycle − 500) ms. FT8 decoders tolerate ±2.5s of DT before
    // sync acquisition fails, giving us a safe ceiling around 3000ms
    // into the cycle. Use 2500ms to leave a 500ms margin and to keep
    // the trailing PTT-off well inside the slot.
    //
    // Originally 1500ms (K3SBP 2026-05-05 leading-silence pad commit) —
    // that was too conservative: a click at slot+2.2s after decode +
    // IPC + encode roundtrip would defer, and slot-parity rules push
    // the next opportunity 30s away. KZ7Y QSO @ slot+2.224s waited
    // through three cycles before TX'ing.
  ...
  _scheduleTx() {
    ...
    // Wall clock, not adjustedNow — see tryImmediateTx() for why.
    const now = Date.now();
    const cycleSec = this._cycleSec();
    const cycleMs = cycleSec * 1000;
    const msIntoCycle = now % cycleMs;
  ...
  _onTxBoundary() {
    ...
    // Wall clock for slot parity too — see tryImmediateTx() for why.
    const now = Date.now();
    const cycleSec = this._cycleSec();
    const slot = Math.floor(now / 1000 / cycleSec) % 2 === 0 ? 'even' : 'odd';
