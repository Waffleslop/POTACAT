...
// A cycle that captured less than this share of its slot is not decoded, and
// counts as starved for the audio-capture restart watchdog.
const PARTIAL_CYCLE_FRACTION = 0.5;
...
class Ft8Engine extends EventEmitter {
  constructor() {
    ...
    this._cycleNumber = 0;
    this._msgId = 0;
    this._decodeJobSlots = new Map(); // decode job id -> slot its audio came from
  ...
  setMode(mode) {
    ...
    if (this._mode !== prev) {
      ...
      this._lastCycleFireSlot = -1;
      this._lastTxFireSlot = -1;
      // The worker watchdog tolerates 2.5 cycle lengths of silence, and the
      // cycle length just changed. Leaving WSPR (120 s) for FT8 (15 s) with
      // the stamp of the last WSPR decode — legitimately up to two minutes
      // old — fired the watchdog at the first FT8 boundary (41.3 s > 37.5 s,
      // K3SBP 2026-09-13) and respawned a perfectly healthy worker. Give the
      // new cadence its full grace window. Zero means no worker is alive
      // (stopped / never started) and the spawn will seed it.
      if (this._lastWorkerResponseMs) this._lastWorkerResponseMs = Date.now();
    }
  }
  ...
  _respawnWorker() {
    ...
    this._worker = null;
    this._workerReady = false;
    if (old) { try { old.terminate(); } catch {} }
    this._spawnWorker();
  }
  ...
  _spawnWorker() {
    ...
    this._worker = w;
    // Every handler checks that `w` is still THE worker: a worker we replaced
    // keeps emitting on later ticks — terminate() delivers its 'exit' (code 1)
    // after the replacement is already up. Until 2026-09-13 the respawn set
    // a `_respawning` flag and cleared it synchronously, so that late exit
    // saw the flag down and spawned a THIRD worker: two "FT8 worker ready"
    // lines in the log, an orphan holding a native decoder, and its decode
    // results attributed to the live one. (K3SBP, the WSPR->FT8 watchdog fire.)
    w.on('message', (msg) => { if (this._worker === w) this._onWorkerMessage(msg); });
    w.on('error', (err) => {
    ...
    w.on('exit', (code) => {
      ...
      if (this._running && code !== 0) {
        ...
        setTimeout(() => { if (this._running && !this._worker) this._spawnWorker(); }, 1000);
      }
    });
  ...
  _onWorkerMessage(msg) {
    ...
    if (msg.type === 'decode-result') {
      ...
      // Every decode carries the slot its audio was received in. Until
      // 2026-09-18 only the batch did, and nothing in main copied it onto
      // the results — so Hunt's "reply on the opposite slot" read
      // `caller.slot` as undefined, defaulted to even, and always answered
      // on odd: a coin flip against whichever slot the activator chose.
      // K4HXM: "often TX on the same time slot as the CQ POTA".
      const jobSlot = this._decodeJobSlots.has(msg.id) ? this._decodeJobSlots.get(msg.id) : this._lastRxSlot;
      this._decodeJobSlots.delete(msg.id);
      for (const r of results) if (r && typeof r === 'object' && r.slot == null) r.slot = jobSlot;
      this.emit('decode', {
  ...
  _onCycleBoundary() {
    ...
    if (this._mode === 'FT2') {
      ...
      // The slot whose audio this snapshot holds. The tick fires this
      // DECODE_LEAD_MS before the WALL-CLOCK boundary, so wall time is
      // always still inside that slot. The latency-adjusted `now` is not:
      // a negative calibration (allowed down to -3 s) pushes it past the
      // boundary and stamps the NEXT slot's parity on this slot's decodes.
      slot = Math.floor(wallNow / 1000 / cycleSec) % 2 === 0 ? 'even' : 'odd';
    ...
    // Same threshold as the decode skip below. This used to be 1/4 while the
    // skip was 1/2, so a capture delivering 25-50% of a slot had every decode
    // skipped and was never called starved: no decodes, no restart, nothing in
    // the log, and the popout stuck on "Switching to 17m..." (AF0DB
    // 2026-09-07, FT-991 on Linux, after a band change).
    const audioStarved = writtenThisCycle < samples.length * PARTIAL_CYCLE_FRACTION;
    if (skipWatchdog) {
    ...
    if (writtenThisCycle >= samples.length * PARTIAL_CYCLE_FRACTION) {
      this._partialSkips = 0;
      // Remember which slot this job's audio came from. The result is
      // stamped from THIS, not from _lastRxSlot at arrival: a decode that
      // outlives the next snapshot would otherwise carry the wrong parity.
      const jobId = ++this._msgId;
      this._decodeJobSlots.set(jobId, slot);
      if (this._decodeJobSlots.size > 8) this._decodeJobSlots.delete(this._decodeJobSlots.keys().next().value);
      ...
      // One partial cycle is the engine starting mid-slot. Two in a row is
      // the capture itself running short, and a skipped cycle sends the UI no
      // decode event at all — so say so where a bug report will carry it.
      this._partialSkips = (this._partialSkips || 0) + 1;
      const pct = Math.round((100 * writtenThisCycle) / samples.length);
      console.log(`[JTCAT] Skip decode — partial cycle (${writtenThisCycle}/${samples.length} samples captured, ${pct}%)`);
      if (this._partialSkips === 2) {
        this.emit('log', `decode skipped for 2 cycles in a row — only ${pct}% of the slot's audio arrived `
          + `(${writtenThisCycle}/${samples.length} samples); the audio capture is running short`);
      }
    }
