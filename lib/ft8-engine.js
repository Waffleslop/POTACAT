...
class Ft8Engine extends EventEmitter {
  ...
  tryImmediateTx() {
    ...
    // Allow late-start TX up to 1.5s into the cycle. The renderer pads the
    // start of the audio with leading silence so the FT8 envelope always
    // begins at slot+500ms (WSJT-X convention) — this preserves the
    // leading Costas sync tones that receivers need to acquire timing.
    // Beyond ~1.5s the padded audio would overlap the next slot
    // (slot+1500 + 12.64 envelope = slot+14.14, plus PTT-off grace).
    // Old behavior allowed 12s late by skipping into the buffer, which
    // dropped the start-of-message Costas array and made the TX
    // undecodable for receivers. K3SBP 2026-05-05.
