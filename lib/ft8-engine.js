...
class Ft8Engine extends EventEmitter {
  ...
  _onCycleBoundary() {
    ...
    // Silence watchdog: detect dead audio capture
    // Case 1: audio arriving but all zeros (maxSample === 0, buffer partially filled)
    // Case 2: audio capture died completely (offset === 0 or barely advanced)
    const audioStarved = off < 1000; // less than ~80ms of audio received this cycle
    if (maxSample === 0 || audioStarved) {
      this._silentCycles = (this._silentCycles || 0) + 1;
      if (this._silentCycles >= 3) {
        const reason = `${this._silentCycles} consecutive silent cycles (max=${maxSample.toFixed(4)} off=${off}) — audio capture is delivering zero samples`;
        console.warn('[JTCAT] ' + reason);
        // Surface in Verbose CAT log so the user can see the problem without
        // opening DevTools. Common causes for a Flex setup: DAX RX channel
        // not active on the slice, AudioContext suspended (no user gesture),
        // or Windows audio device exists but isn't producing samples.
        // (K3SBP 2026-05-04: phone-triggered FT8 start with cold-start
        // desktop captured silence indefinitely; only a desktop restart
        // recovered.)
        this.emit('log', `${reason} — restarting audio capture`);
        this.emit('silent');
        // Track total restart attempts so we don't loop forever silently.
