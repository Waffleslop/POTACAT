...
// CW for real regardless of the rig's own protocol, so it always wins. Icom rigs
// key CW via the DTR/RTS control line ('dtr' route) or CW *text* (CI-V 0x17) —
// there is NO per-element CW key command in the Icom CI-V set (0x1C 0x01 is the
// antenna tuner). The Icom 'txrx' route here is only a PTT dead-key, but it's not
// treated as unavailable because the real Icom path is 'dtr', not 'txrx'. Only
// Kenwood-protocol rigs (every Yaesu + Kenwood serial model, protocol: 'kenwood')
// are flagged dead on 'txrx'. DTR/RTS routes stay optimistic here — a Linux
// 'pin-unsupported' failure is caught lazily on first key and drops availability then.
