...
function registerCloudIpc(ctx) {
  ...
  function recordLogChanges(changes) {
    if (!ctx.getSettings().cloudAccessToken) return;
    const entries = [];
    for (const { action, fields } of changes) {
      const uuid = fields.APP_POTACAT_UUID;
      if (!uuid) continue;
      const version = (parseInt(fields.APP_POTACAT_VERSION || '1', 10) || 1) + (action === 'delete' ? 1 : 0);
      entries.push({ uuid, action, adifFields: normalizeAdifFields({ ...fields }), version });
    }
    journal.appendMany(entries);
  }

  /** Push everything pending, pull everything new. */
  async function syncNow() {
    const result = await getCloudSync().sync(journal, getSyncCallbacks());
    return { success: true, ...result, pending: journal.length };
  }
  ...
  async function uploadFullLog(onProgress) {
    ...
    journal.removeWhere((e) => e.action === 'create' && uploaded.has(e.uuid));
