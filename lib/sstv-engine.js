 ...
 *   'rx-vis'           { mode, modeName, visCode?, auto?, midLock? }
 *   'rx-lock-lost'     { mode, modeName, reason, detail }
...
class SstvEngine extends EventEmitter {
  ...
  /** Manually re-decode the worker's retained raw-audio buffer (MMSSTV
   *  "replay from RX buffer"). opts.slantPpm resamples before the replay. */
  redecode(opts) {
    if (!this._running || !this._workerReady || !this._worker) return;
    this._worker.postMessage({ type: 'redecode', slantPpm: (opts && opts.slantPpm) || 0 });
  }
  ...
  _onWorkerMessage(msg) {
    switch (msg.type) {
      case 'ready':
        ...
        // A fresh worker (first start, restart after stop(), or respawn after
        // a crash) starts at 48 kHz with an empty decoder: re-apply the audio
        // rate the host set, or every tone reads off-frequency.
        if (this._sampleRate && this._sampleRate !== SAMPLE_RATE && this._worker) {
          this._worker.postMessage({ type: 'set-sample-rate', sampleRate: this._sampleRate });
        }
      ...
      case 'rx-vis':
        this._decoding = true;
        console.log(`[SSTV] ${msg.auto ? 'Joined from line sync' : 'VIS detected'}: ${msg.modeName} (${msg.mode})`);
        this.emit('rx-vis', {
          mode: msg.mode,
      ...
      case 'rx-lock-lost':
        // The decoder let go of a lock (wrong mode, signal gone, or a new
        // VIS took over) — no image follows for it.
        this._decoding = false;
        console.log(`[SSTV] ${msg.detail}`);
        this.emit('rx-lock-lost', { mode: msg.mode, modeName: msg.modeName, reason: msg.reason, detail: msg.detail });
        this.emit('status', { state: 'running' });
        break;
...
// The CAT-log line for an rx-vis event. `mode` is the mode KEY ('pd160');
// until 2026-09-26 main.js ran it through toString(16) and logged
// "mode 0xpd160" (WB8IMY's log). The VIS code is its own numeric field.
function sstvRxVisLogLine(data) {
  const d = data || {};
  const name = d.modeName || d.mode || '?';
  if (d.auto) return `[SSTV] No VIS heard — joined ${name} from its line sync`;
  const code = Number.isInteger(d.visCode) ? ` (VIS ${d.visCode})` : '';
  if (d.midLock) return `[SSTV] New VIS during a decode: ${name}${code} — restarting on it`;
  return `[SSTV] VIS detected: ${name}${code} — locking onto signal`;
}

module.exports = { SstvEngine, sstvRxVisLogLine };
