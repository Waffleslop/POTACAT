...
/** Same 24 h either side the desktop board and map overlay use to decide an
 *  event is "near" (renderer EVENT_BADGE_GRACE_MS / lib/event-geo.js). */
const HUNT_GRACE_MS = 24 * 3600 * 1000;

/**
 * Is there anything for "Hunt: Event stations" to hunt — which opted-in
 * checklist events are inside (or within `graceMs` of) their schedule?
 *
 * Drives whether the hunt option is OFFERED (JTCAT popout, ECHOCAT clients):
 * the option appears the day before an event opens and lingers a day after,
 * so an operator can arm it ahead of the window and the select never loses
 * a value it is showing. Matching itself (eventDecodeMatch) stays strictly
 * in-window — the grace is for the control, not the QSO. Computed by main
 * and shipped in `jtcat-auto-cq-state`, so every client sees one answer.
 *
 * @param events active events catalog
 * @param eventStates settings.events
 * @param now Date (injectable)
 * @param graceMs how far outside the window still counts (default 24 h)
 * @returns { available: boolean, events: [{ id, name, badge }] }
 */
function eventHuntAvailability(events, eventStates, now = new Date(), graceMs = HUNT_GRACE_MS) {
  const out = [];
  if (Array.isArray(events) && eventStates) {
    const t = now.getTime();
    for (const ev of events) {
      if (!ev || !ev.id) continue;
      const state = eventStates[ev.id];
      ...
      const near = (ev.schedule || []).some((s) => {
        ...
        return Number.isFinite(start) && Number.isFinite(end)
          && t >= start - graceMs && t < end + graceMs;
      });
      if (!near) continue;
      out.push({ id: ev.id, name: ev.name || ev.id, badge: ev.badge || '' });
    }
  }
...
module.exports = {
  ...
  eventHuntAvailability, HUNT_GRACE_MS,
};
