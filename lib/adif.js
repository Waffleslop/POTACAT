'use strict';

const fs = require('fs');
const path = require('path');
const { normalizeAdifFields } = require('./adif-normalize');

/**
 * Normalize a mode to the name a SPOT uses: USB/LSB -> SSB, and an ADIF
 * family MODE with a SUBMODE (MFSK/FT4, PSK/PSK31, DIGITALVOICE/FREEDV) -> the
 * submode, which is the mode anyone means. Comparing the raw MODE field is
 * why a spot's FT4 never matched a logged FT4 (stored MFSK) and a spot's SSB
 * never matched a logged USB (LZ3AW #14).
 */
const FAMILY_MODES = new Set(['MFSK', 'PSK', 'DIGITALVOICE']);
function normalizeMode(mode, submode) {
  if (!mode) return '';
  const m = String(mode).toUpperCase().trim();
  const sub = submode ? String(submode).toUpperCase().trim() : '';
  if (sub && FAMILY_MODES.has(m)) return sub;
  if (m === 'USB' || m === 'LSB') return 'SSB';
  return m;
}

/**
 * Normalize band string: "20M" -> "20m"
 */
function normalizeBand(band) {
  if (!band) return '';
  return band.toLowerCase().trim();
}

/**
 * Parse an ADIF field tag like <CALL:5>W1AW
 * Returns array of { field, value } objects for one record.
 */
function parseRecord(record) {
  const fields = {};
  const re = /<(\w+):(\d+)(?::[^>]*)?>/gi;
  let match;
  while ((match = re.exec(record)) !== null) {
    const field = match[1].toUpperCase();
    const len = parseInt(match[2], 10);
    const start = match.index + match[0].length;
    const value = record.substring(start, start + len);
    fields[field] = value;
  }
  return fields;
}

/**
 * Parse an ADIF file. Returns array of confirmed QSO objects:
 * { call, band, mode, dxcc, qsoDate }
 *
 * When confirmedOnly is true (default), only includes QSOs where QSL_RCVD='Y' or LOTW_QSL_RCVD='Y'.
 */
function parseAdifFile(filePath, { confirmedOnly = true } = {}) {
  const content = fs.readFileSync(filePath, 'utf-8');

  // Skip header (everything before first <EOH> if present)
  let body = content;
  const eohIdx = content.toUpperCase().indexOf('<EOH>');
  if (eohIdx !== -1) {
    body = content.substring(eohIdx + 5);
  }

  // Split into records by <EOR>
  const records = body.split(/<eor>/i).filter((r) => r.trim().length > 0);

  const qsos = [];
  for (const rec of records) {
    const f = parseRecord(rec);
    if (!f.CALL) continue;

    // Only confirmed QSOs (when confirmedOnly is set)
    if (confirmedOnly) {
      const qslRcvd = (f.QSL_RCVD || '').toUpperCase();
      const lotwRcvd = (f.LOTW_QSL_RCVD || '').toUpperCase();
      if (qslRcvd !== 'Y' && lotwRcvd !== 'Y') continue;
    }

    qsos.push({
      call: f.CALL.toUpperCase(),
      band: normalizeBand(f.BAND || ''),
      mode: normalizeMode(f.MODE || ''),
      dxcc: f.DXCC ? parseInt(f.DXCC, 10) : null,
      qsoDate: f.QSO_DATE || '',
    });
  }

  return qsos;
}

/**
 * Parse an ADIF file and return a Map of worked callsigns -> QSO entries.
 * Each entry has { date: 'YYYYMMDD', ref: 'K-1234' } for smart hide-worked filtering.
 * Unlike parseAdifFile(), this includes ALL QSOs regardless of QSL status.
 * @returns {Map<string, Array<{date: string, ref: string}>>}
 */
function parseWorkedQsos(filePath) {
  const content = fs.readFileSync(filePath, 'utf-8');
  let body = content;
  const eohIdx = content.toUpperCase().indexOf('<EOH>');
  if (eohIdx !== -1) body = content.substring(eohIdx + 5);

  const records = body.split(/<eor>/i).filter((r) => r.trim().length > 0);
  const qsoMap = new Map();
  for (const rec of records) {
    const f = parseRecord(rec);
    if (!f.CALL) continue;
    const call = f.CALL.toUpperCase();
    // ref = the OTHER station's park (SIG_INFO, e.g. P2P). myRef = MY park for
    // this QSO (MY_SIG_INFO / MY_POTA_REF) — needed so the activator log's
    // "worked before" (PREV) badge can tell THIS activation's contacts from
    // genuine prior work. SIG_INFO is empty for ordinary activation QSOs, so
    // comparing against it flagged every contact as PREV (Casey 2026-06-16).
    const entry = { date: f.QSO_DATE || '', ref: (f.SIG_INFO || '').toUpperCase(), myRef: (f.MY_SIG_INFO || f.MY_POTA_REF || '').toUpperCase(), band: (f.BAND || '').toUpperCase(), mode: normalizeMode(f.MODE, f.SUBMODE) };
    if (!qsoMap.has(call)) qsoMap.set(call, []);
    qsoMap.get(call).push(entry);
  }
  return qsoMap;
}

/**
 * Decide whether a callsign counts as "worked before" (the PREV badge in the
 * activator log) relative to the CURRENT activation.
 *
 * A worked-QSO entry belongs to THIS activation when it is from today (UTC)
 * AND at one of the activation's own park refs (MY_SIG_INFO → entry.myRef).
 * Any entry that is NOT part of this activation is genuine prior work, so the
 * call gets a PREV badge.
 *
 * IMPORTANT: match on entry.myRef (MY_SIG_INFO), NOT entry.ref (SIG_INFO, the
 * other station's park). SIG_INFO is empty for ordinary activation QSOs, so
 * comparing against it made every contact look like prior work (Casey
 * 2026-06-16). This is the canonical implementation; renderer/app.js mirrors
 * it inline because the renderer cannot require() Node modules.
 *
 * @param {Array<{date?:string, myRef?:string}>} entries  worked-QSO entries for one call
 * @param {string[]} currentRefs  the activation's park refs (any case)
 * @param {string} todayUtc       YYYYMMDD (UTC) for "today"
 * @returns {boolean}             true if the call should show the PREV badge
 */
function isPriorActivationWork(entries, currentRefs, todayUtc) {
  if (!entries || !entries.length) return false;
  const refs = (currentRefs || []).map((r) => (r || '').toUpperCase());
  return entries.some((e) => {
    const sameDay = e.date === todayUtc;
    const sameRef = refs.includes((e.myRef || '').toUpperCase());
    return !(sameDay && sameRef);
  });
}

/**
 * Parse an ADIF file and return ALL QSOs with preserved fields for import.
 * Returns array of objects with key fields extracted from each record.
 * Skips records without a CALL field.
 */
function parseAllQsos(filePath) {
  const content = fs.readFileSync(filePath, 'utf-8');
  let body = content;
  const eohIdx = content.toUpperCase().indexOf('<EOH>');
  if (eohIdx !== -1) body = content.substring(eohIdx + 5);

  const records = body.split(/<eor>/i).filter((r) => r.trim().length > 0);
  const qsos = [];
  for (const rec of records) {
    const f = parseRecord(rec);
    if (!f.CALL) continue;
    qsos.push({
      call: f.CALL,
      qsoDate: f.QSO_DATE || '',
      timeOn: f.TIME_ON || '',
      band: f.BAND || '',
      mode: f.MODE || '',
      freq: f.FREQ || '',
      dxcc: f.DXCC || '',
      country: f.COUNTRY || '',
      cont: f.CONT || '',
      qslRcvd: f.QSL_RCVD || '',
      lotwQslRcvd: f.LOTW_QSL_RCVD || '',
      gridsquare: f.GRIDSQUARE || '',
      rstSent: f.RST_SENT || '',
      rstRcvd: f.RST_RCVD || '',
      comment: f.COMMENT || '',
    });
  }
  return qsos;
}

/**
 * Check if a file path is a SQLite database (by extension).
 */
function isSqliteFile(filePath) {
  const ext = path.extname(filePath).toLowerCase();
  return ext === '.sqlite' || ext === '.db';
}

/**
 * Load sql.js and open a SQLite database file.
 */
async function openSqliteDb(filePath) {
  const initSqlJs = require('sql.js');
  const SQL = await initSqlJs();
  const buffer = fs.readFileSync(filePath);
  return new SQL.Database(buffer);
}

/**
 * Parse qsoconfirmations JSON from Log4OM.
 * Returns { qslRcvd, lotwQslRcvd } with 'Y' or 'N' values.
 */
function parseConfirmations(json) {
  let qslRcvd = 'N';
  let lotwQslRcvd = 'N';
  if (!json) return { qslRcvd, lotwQslRcvd };
  try {
    const arr = JSON.parse(json);
    for (const entry of arr) {
      if (entry.CT === 'LOTW' && entry.R === 'Yes') lotwQslRcvd = 'Y';
      if (entry.CT === 'QSL' && entry.R === 'Yes') qslRcvd = 'Y';
    }
  } catch { /* ignore malformed JSON */ }
  return { qslRcvd, lotwQslRcvd };
}

/**
 * Parse a Log4OM SQLite file. Returns same format as parseAllQsos():
 * array of { call, qsoDate, timeOn, band, mode, freq, dxcc, country, cont,
 *            qslRcvd, lotwQslRcvd, gridsquare, rstSent, rstRcvd, comment }
 */
async function parseSqliteFile(filePath) {
  const db = await openSqliteDb(filePath);
  try {
    const stmt = db.prepare(
      `SELECT callsign, band, mode, qsodate, freq, dxcc, country, cont,
              gridsquare, rstsent, rstrcvd, comment, qsoconfirmations
       FROM Log`
    );
    const qsos = [];
    while (stmt.step()) {
      const row = stmt.getAsObject();
      if (!row.callsign) continue;

      // Parse qsodate: "2025-11-30 21:56:19Z" -> qsoDate "20251130", timeOn "215619"
      let qsoDate = '';
      let timeOn = '';
      if (row.qsodate) {
        const d = String(row.qsodate).replace(/[-:T]/g, '').replace('Z', '');
        // d is now "20251130 215619" or "20251130215619"
        const digits = d.replace(/\s/g, '');
        if (digits.length >= 8) qsoDate = digits.substring(0, 8);
        if (digits.length >= 14) timeOn = digits.substring(8, 14);
      }

      // Parse freq: kHz (e.g. 7031.48) -> MHz string ("7.03148")
      let freqMhz = '';
      if (row.freq != null) {
        freqMhz = (row.freq / 1000).toFixed(6).replace(/0+$/, '').replace(/\.$/, '');
      }

      const conf = parseConfirmations(row.qsoconfirmations);

      qsos.push({
        call: String(row.callsign),
        qsoDate,
        timeOn,
        band: String(row.band || ''),
        mode: String(row.mode || ''),
        freq: freqMhz,
        dxcc: row.dxcc != null ? String(row.dxcc) : '',
        country: String(row.country || ''),
        cont: String(row.cont || ''),
        qslRcvd: conf.qslRcvd,
        lotwQslRcvd: conf.lotwQslRcvd,
        gridsquare: String(row.gridsquare || ''),
        rstSent: String(row.rstsent || ''),
        rstRcvd: String(row.rstrcvd || ''),
        comment: String(row.comment || ''),
      });
    }
    stmt.free();
    return qsos;
  } finally {
    db.close();
  }
}

/**
 * Parse a Log4OM SQLite file for DXCC tracker (confirmed QSOs only).
 * Returns same format as parseAdifFile():
 * array of { call, band, mode, dxcc, qsoDate }
 */
async function parseSqliteConfirmed(filePath) {
  const db = await openSqliteDb(filePath);
  try {
    const stmt = db.prepare(
      `SELECT callsign, band, mode, qsodate, dxcc, qsoconfirmations FROM Log`
    );
    const qsos = [];
    while (stmt.step()) {
      const row = stmt.getAsObject();
      if (!row.callsign) continue;

      const conf = parseConfirmations(row.qsoconfirmations);
      if (conf.qslRcvd !== 'Y' && conf.lotwQslRcvd !== 'Y') continue;

      let qsoDate = '';
      if (row.qsodate) {
        const digits = String(row.qsodate).replace(/[-:T\sZ]/g, '');
        if (digits.length >= 8) qsoDate = digits.substring(0, 8);
      }

      qsos.push({
        call: String(row.callsign).toUpperCase(),
        band: normalizeBand(String(row.band || '')),
        mode: normalizeMode(String(row.mode || '')),
        dxcc: row.dxcc != null ? parseInt(row.dxcc, 10) : null,
        qsoDate,
      });
    }
    stmt.free();
    return qsos;
  } finally {
    db.close();
  }
}

/**
 * Parse an ADIF file and return ALL QSOs with ALL fields preserved as-is.
 * Each QSO is a flat object of ADIF field names -> values (all uppercase keys).
 * Used for rewrite-after-edit workflows where no fields should be dropped.
 */
function parseAllRawQsos(filePath) {
  const content = fs.readFileSync(filePath, 'utf-8');
  let body = content;
  const eohIdx = content.toUpperCase().indexOf('<EOH>');
  if (eohIdx !== -1) body = content.substring(eohIdx + 5);
  const records = body.split(/<eor>/i).filter(r => r.trim().length > 0);
  const qsos = [];
  for (const rec of records) {
    // Normalized BEFORE the CALL test: a pseudo-named record (CALLSIGN, see
    // adif-normalize.js) used to be skipped here, and every rewrite built
    // from this parse then deleted it from the file.
    const fields = normalizeAdifFields(parseRecord(rec));
    if (!fields.CALL) continue;
    qsos.push(fields);
  }
  return qsos;
}

/**
 * Length-respecting ADIF stream parser.
 *
 * Walks the input by tag, consuming exactly the bytes declared in each
 * `<tag:length>` header. Handles both well-formed ADIF (with `<EOR>` between
 * records) and degenerate streams that omit `<EOR>` — in the latter case a
 * record boundary is inferred when a duplicate `<CALL>` is encountered.
 *
 * Used for QRZ logbook downloads, which can return ADIF without `<EOR>`
 * delimiters between records.
 *
 * @param {string} text - raw ADIF text (header optional)
 * @returns {Array<object>} array of records, each a flat field-name -> value map
 *                          (uppercase keys), only records with CALL are returned
 */
function parseAdifStream(text) {
  if (!text) return [];
  const records = [];
  let current = {};
  let pos = 0;
  // Match <tagname>, <tagname:length>, or <tagname:length:type>
  const tagRe = /<(\w+)(?::(\d+)(?::[^>]*)?)?>/g;

  while (true) {
    tagRe.lastIndex = pos;
    const match = tagRe.exec(text);
    if (!match) break;

    const tagName = match[1].toUpperCase();
    const length = match[2] ? parseInt(match[2], 10) : 0;
    const valueStart = match.index + match[0].length;
    pos = valueStart + length;

    if (tagName === 'EOH') {
      current = {};
      continue;
    }
    if (tagName === 'EOR') {
      if (current.CALL) records.push(current);
      current = {};
      continue;
    }

    // Duplicate CALL means previous record ended (no <EOR> between)
    if (tagName === 'CALL' && current.CALL) {
      records.push(current);
      current = {};
    }

    const value = length > 0 ? text.substring(valueStart, valueStart + length) : '';
    current[tagName] = value;
  }

  // Trailing record with no <EOR>
  if (current.CALL) records.push(current);
  return records;
}

module.exports = { parseAdifFile, parseWorkedQsos, isPriorActivationWork, parseAllQsos, parseAllRawQsos, parseAdifStream, parseSqliteFile, parseSqliteConfirmed, isSqliteFile, normalizeMode, normalizeBand, parseRecord };
