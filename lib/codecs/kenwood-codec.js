...
function yaesuBwToIndex(hz, mode, ssbTableOverride, cwTableOverride) {
  ...
  const table = useCwTable ? (cwTableOverride || YAESU_CW_BW) : (ssbTableOverride || YAESU_SSB_BW);
...
function ftx1BandFamily(freqHz) {
...
function ftx1SdCodeToMs(code) {
  const n = Math.max(0, Math.min(33, Math.round(Number(code) || 0)));
  if (n === 0) return 30;
  if (n === 1) return 50;
  if (n === 2) return 100;
  if (n === 3) return 150;
...
function ftx1SdMsToCode(ms) {
  const val = Math.max(30, Math.min(3000, Math.round(Number(ms) || 0)));
  const presets = [30, 50, 100, 150, 200, 250];
  let bestCode = 0;
  let bestDist = Infinity;
  for (let i = 0; i < presets.length; i++) {
    const dist = Math.abs(presets[i] - val);
    if (dist < bestDist) {
      bestDist = dist;
      bestCode = i;
    }
  }
  for (let code = 6; code <= 33; code++) {
    const candidate = ftx1SdCodeToMs(code);
    const dist = Math.abs(candidate - val);
    if (dist < bestDist) {
      bestDist = dist;
      bestCode = code;
    }
...
class KenwoodCodec extends EventEmitter {
  ...
  constructor(model, writeFn) {
    ...
    this._rmSwr = model.rmSwr || (isYaesu ? 4 : 1);
    this._rmAlc = model.rmAlc || (isYaesu ? 2 : 3);
  }
  ...
  getNbLevel() {
    if (this._cmds.getNbLevel) this._write(this._cmds.getNbLevel);
  }
  ...
  getNoiseReductionLevel() {
    if (this._cmds.getNoiseReductionLevel) this._write(this._cmds.getNoiseReductionLevel);
  }
  ...
  getCompressor() {
    if (this._cmds.getCompressor) this._write(this._cmds.getCompressor);
  }
  ...
  getClarState() {
    if (this._cmds.getClarState) this._write(this._cmds.getClarState);
  }

  getClarFreq() {
    if (this._cmds.getClarFreq) this._write(this._cmds.getClarFreq);
  }
  ...
  getBreakInDelay() {
    if (this._cmds.getBreakInDelay) this._write(this._cmds.getBreakInDelay);
  }
  ...
  setFilterWidth(hz) {
    ...
    if (filterType === 'indexed') {
      ...
      const idx = yaesuBwToIndex(hz, mode, this._model.ssbBw, this._model.cwBw);
  ...
  /** Set TX CLAR (XIT). FTX-1 uses CF state/frequency modes instead of XT/RU/RD. */
  setXit(hz) {
    if (this._model.caps?.ftx1Clar) {
      this.setClarTx(!!hz);
      if (!hz) return;
      const abs = String(Math.min(9999, Math.abs(Math.round(hz)))).padStart(4, '0');
      this._clarFreq = hz;
      this._write(`CF001${hz >= 0 ? '+' : '-'}${abs};`);
      return;
    }
    if (!hz) {
  ...
  sendCwText(text) {
    ...
    if (cw.kyMode === 'km') {
      ...
      for (let i = 0; i < clean.length; i += chunk) {
        ...
        const playCmd = cw.kyPlayCmd || 'KYA;';
        this.emit('log', `CW write KM frame #${++frames}: KM5${part}; then ${playCmd}`);
  ...
  setPreamp(on) {
    if (this._model.ftx1Preamp) {
      const family = ['hf', 'vhf', 'uhf'].includes(this._ftx1PreampTarget)
        ? this._ftx1PreampTarget
        : ftx1BandFamily(this._lastFreqHz);
      if (family === 'uhf') {
        this._write(on ? 'PA21;' : 'PA20;');
        return;
      }
      if (family === 'vhf') {
        this._write(on ? 'PA11;' : 'PA10;');
        return;
      }
      // Temporary FTX-1 HF/50 mapping: off = IPO, on = AMP1.
      this._write(on ? 'PA01;' : 'PA00;');
      return;
    }
  ...
  setPreampTarget(target) {
    const norm = String(target || '').toLowerCase();
    this._ftx1PreampTarget = ['hf', 'vhf', 'uhf'].includes(norm) ? norm : 'auto';
  }
  ...
  setVox(on) {
    const cmd = on ? this._cmds.setVoxOn : this._cmds.setVoxOff;
    if (cmd) this._write(cmd);
  }

  setVoxLevel(pct) {
    const scaled = Math.max(0, Math.min(100, Math.round(Number(pct) || 0)));
    const cmd = expand(this._cmds.setVoxLevel, { val: scaled });
    if (cmd) this._write(cmd);
  }
  ...
  setMonitor(on) {
    const cmd = on ? this._cmds.setMonitorOn : this._cmds.setMonitorOff;
    if (cmd) this._write(cmd);
  }

  setMonLevel(pct) {
    const scaled = Math.max(0, Math.min(100, Math.round(Number(pct) || 0)));
    const cmd = expand(this._cmds.setMonLevel, { val: scaled });
    if (cmd) this._write(cmd);
  }

  setNrLevel(val) {
    const levelCap = this._model.caps?.maxNrLevel || 100;
    const level = Math.max(0, Math.min(levelCap, Math.round(Number(val) || 0)));
    const cmd = expand(this._cmds.setNoiseReductionLevel, { val: level });
    if (cmd) {
      this._write(cmd);
      return;
    }
    this.setNoiseReduction(level > 0);
  }
  ...
  setAgc(mode) {
    ...
    const prefix = this._cmds.setAgcPrefix;
    if (prefix && Object.prototype.hasOwnProperty.call(yaesuMap, key)) {
      this._write(`${prefix}${yaesuMap[key]};`);
    }
  }
  ...
  setCompLevel(pct) {
    const scaled = Math.max(0, Math.min(100, Math.round(Number(pct) || 0)));
    const cmd = expand(this._cmds.setCompLevel, { val: scaled });
    if (cmd) this._write(cmd);
  }

  setAutoNotch(on) {
    const cmd = on ? this._cmds.setAutoNotchOn : this._cmds.setAutoNotchOff;
    if (cmd) this._write(cmd);
  }

  setMicGain(pct) {
    const scaled = Math.max(0, Math.min(100, Math.round(Number(pct) || 0)));
    const cmd = expand(this._cmds.setMicGain, { val: scaled });
    if (cmd) this._write(cmd);
  }

  setClarFreq(hz) {
    if (!this._model.caps?.ftx1Clar) return;
    const val = Math.max(-9999, Math.min(9999, Math.round(Number(hz) || 0)));
    this._clarFreq = val;
    const abs = String(Math.abs(val)).padStart(4, '0');
    this._write(`CF001${val >= 0 ? '+' : '-'}${abs};`);
  }
  ...
  setBreakIn(on) {
    const cmd = on ? this._cmds.setBreakInOn : this._cmds.setBreakInOff;
    if (cmd) this._write(cmd);
  }

  setBreakInDelay(ms) {
    const template = this._cmds.setBreakInDelay;
    if (!template) return;
    const code = ftx1SdMsToCode(ms);
    const cmd = expand(template, { val: code });
    if (cmd) this._write(cmd);
  }

  setAutoInfo(on) {
    const cmd = on ? this._cmds.setAutoInfoOn : this._cmds.setAutoInfoOff;
    if (cmd) this._write(cmd);
  }
  ...
  _parseMessage(msg) {
    ...
    if (msg.startsWith('PC') && msg.length > 2) {
      ...
      if (this._model.pcPrefix && payload.length >= 4 && payload.charAt(0) === String(this._model.pcPrefix)) {
    ...
    if (msg.startsWith('NL') && msg.length > 3) {
      const level = parseInt(msg.slice(3), 10) || 0;
    ...
    if (msg.startsWith('RG') && msg.length > 3) {
      const digits = msg.slice(3);
      const raw = parseInt(digits, 10);
      if (!isNaN(raw)) {
        const pct = Math.max(0, Math.min(100, Math.round(raw / 2.55)));
        this.emit('rfgain', pct);
      }
      return;
    }

    if (msg.startsWith('VX') && msg.length > 2) {
      this.emit('vox', msg.slice(-1) === '1');
      return;
    }

    if (msg.startsWith('VG') && msg.length > 2) {
      const level = parseInt(msg.slice(2), 10);
      if (!isNaN(level)) this.emit('voxLevel', level);
      return;
    }

    if (msg.startsWith('RL') && msg.length > 3) {
      const level = parseInt(msg.slice(3), 10) || 0;
    ...
    if (msg.startsWith('GT') && msg.length > 3) {
      const agcMode = parseInt(msg.slice(3), 10);
      const reverse = this._model.caps?.agcAutoCollapsed
        ? { 0: 'off', 1: 'fast', 2: 'med', 3: 'slow', 4: 'auto', 5: 'auto', 6: 'auto' }
        : { 0: 'off', 1: 'fast', 2: 'med', 3: 'slow', 4: 'auto-fast', 5: 'auto-mid', 6: 'auto-slow' };
      if (Object.prototype.hasOwnProperty.call(reverse, agcMode)) {
        this.emit('agc', reverse[agcMode]);
      }
      return;
    }

    if (msg.startsWith('PR0') && msg.length > 3) {
      this.emit('comp', msg.slice(3) !== '1');
      return;
    }

    if (msg.startsWith('PL') && msg.length > 2) {
      const level = parseInt(msg.slice(2), 10);
      if (!isNaN(level)) this.emit('compLevel', level);
      return;
    }

    if (msg.startsWith('BC0') && msg.length > 3) {
      this.emit('anf', msg.slice(3) === '1');
      return;
    }

    if (msg.startsWith('ML') && msg.length > 3) {
      const selector = msg.charAt(2);
      const payload = msg.slice(3);
      if (selector === '0') {
        this.emit('mon', parseInt(payload, 10) > 0);
      } else if (selector === '1') {
        const level = parseInt(payload, 10);
        if (!isNaN(level)) this.emit('monLevel', level);
      }
      return;
    }

    if (msg.startsWith('MG') && msg.length > 2) {
      const level = parseInt(msg.slice(2), 10);
      if (!isNaN(level)) this.emit('micGain', level);
      return;
    }

    if (msg.startsWith('BI') && msg.length > 2) {
      const state = parseInt(msg.slice(2), 10);
      if (!isNaN(state)) this.emit('breakIn', state > 0);
      return;
    }

    if (msg.startsWith('SD') && msg.length > 2) {
      const code = parseInt(msg.slice(2), 10);
      if (!isNaN(code)) this.emit('breakInDelay', ftx1SdCodeToMs(code));
      return;
    }

    if (msg.startsWith('CF') && msg.length > 4) {
      const mode = msg.charAt(4);
      if (mode === '0' && msg.length >= 10) {
      ...
      if (mode === '1' && msg.length >= 10) {
        const sign = msg.charAt(5);
        const raw = parseInt(msg.slice(6), 10);
        if (!isNaN(raw)) {
          this._clarFreq = sign === '-' ? -raw : raw;
          this.emit('clarFreq', this._clarFreq);
        }
        return;
      }
    }

    // SWR/ALC meter — per-brand channel numbers (set in constructor)
  ...
  getCommandTable() {
    ...
    const LABELS = {
      ...
      getNbLevel: 'Get NB Level', setNbLevel: 'NB Level', setAfGain: 'AF Gain',
      setPreampOn: 'Preamp On', setPreampOff: 'Preamp Off',
      setAttenuatorOn: 'Atten On', setAttenuatorOff: 'Atten Off',
      getVox: 'Get VOX', setVoxOn: 'VOX On', setVoxOff: 'VOX Off',
      getVoxLevel: 'Get VOX Level', setVoxLevel: 'VOX Level',
      getNoiseReductionLevel: 'Get DNR Level', setNoiseReductionLevel: 'DNR Level',
      getAgc: 'Get AGC', getCompressor: 'Get Compressor', setCompOn: 'Compressor On', setCompOff: 'Compressor Off',
      getCompLevel: 'Get Compressor Level', setCompLevel: 'Compressor Level',
      getAutoNotch: 'Get Auto Notch', setAutoNotchOn: 'Auto Notch On', setAutoNotchOff: 'Auto Notch Off',
      getMonitor: 'Get Monitor', setMonitorOn: 'Monitor On', setMonitorOff: 'Monitor Off',
      setMonLevel: 'Monitor Level', getMicGain: 'Get Mic Gain', setMicGain: 'Mic Gain',
      setAutoInfoOn: 'Auto Info On', setAutoInfoOff: 'Auto Info Off',
