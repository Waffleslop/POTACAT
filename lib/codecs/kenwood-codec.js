...
class KenwoodCodec extends EventEmitter {
  ...
  setFilterWidth(hz) {
    ...
    if (filterType === 'indexed') {
      ...
      const snapped = this._snapFilterSlot(hz);
      const cmd = expand(this._cmds.setFilter, { val: snapped });
      if (cmd) this._write(cmd);
    }
  }

  /**
   * Nearest IF filter width this radio actually has, for the current mode.
   * Returns `hz` unchanged when the model declares no slot table (every rig
   * but the TS-480 today) so this can never alter an unverified rig.
   */
  _snapFilterSlot(hz) {
    ...
    if (!list || !list.length) return hz;
    let best = list[0];
    for (const v of list) {
      if (Math.abs(v - hz) < Math.abs(best - hz)) best = v;
    }
    if (best !== hz) {
      this.emit('log', `filter: ${hz}Hz is not a ${m} filter this rig has — using its nearest slot, ${best}Hz`);
    }
    return best;
  }
