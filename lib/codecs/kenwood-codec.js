...
class KenwoodCodec extends EventEmitter {
  ...
  setCompressor(on) {
    ...
    if (this._cmds.setCompOn === 'PR02;' || this._cmds.setCompOff === 'PR01;') {
      const level = on ? Math.max(1, Math.min(100, this._lastCompLevel || 50)) : 0;
      const levelCmd = expand(this._cmds.setCompLevel || 'PL{val:pad3};', { val: level });
      this._write(levelCmd);
    }
  }
  ...
  _parseMessage(msg) {
    ...
    if (msg.startsWith('PR')) {
      ...
      if (!isNaN(raw)) {
        const ftxProc = this._cmds.setCompOn === 'PR02;' || this._cmds.setCompOff === 'PR01;';
        this.emit('comp', ftxProc ? raw >= 2 : raw > 0);
      }
      return;
    }
    ...
    if (msg.startsWith('PL')) {
      ...
      if (!isNaN(level)) {
        ...
        if (clamped > 0) this._lastCompLevel = clamped;
        this.emit('compLevel', clamped);
        this.emit('comp', clamped > 0);
      }
      return;
    }
