...
class CivCodec extends EventEmitter {
  ...
  setAttenuator(on) {
    // CI-V: attenuator cmd 0x11. Most modern Icoms use 0x00=off, 0x20=20dB
    // ON. The IC-706 MkII (and a handful of its contemporaries) use 0x14
    // for ON instead — a different attenuator stage in the receiver. Allow
    // the rig-models entry to override with `attOnByte: 0x14`.
    const onByte = (this._model && this._model.attOnByte) || 0x20;
    this._sendCmd(0x11, null, [on ? onByte : 0x00]);
  }
