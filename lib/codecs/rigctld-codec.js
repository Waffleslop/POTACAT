...
const { rigctldRawArg } = require('../raw-cat-command');
...
// READ side. POTACAT's canonical AGC vocabulary is off/fast/med/slow (the
// allow-list in main's set-agc, and what every client renders); hamlib's own
// name for the middle step is MEDIUM. Emitting 'mid' meant the mobile client
// cleared its AGC highlight instead of lighting MED — while WRITING 'med'
// worked, because _FROM_MODE below accepts both. That asymmetry is the bug:
// we accepted their word and answered in ours. kenwood-codec.js already
// normalizes the same token on emit; this is the rigctld half.
// (GoNoGoTest, IC-7300 via wfview, #82 v1.10.14 retest.)
// Preamp/ATT "on" when dump_caps names no ladder: the values to try, in order.
// hamlib backends want dB (icom.c: preamp 10/20, ATT 6/12/18/20); wfview's
// IC-7300 server wants its own step numbers for preamp (1 = P.AMP1, 2 =
// P.AMP2) and 20 for ATT, and RPRT-rejects the rest (GoNoGoTest, #82). A
// rejected value moves on to the next; the value the rig accepts — or
// reports back on its own — is kept.
const GAIN_ON_FALLBACKS = { PREAMP: [10, 1, 20, 2], ATT: [12, 20, 6, 10, 18] };

const RIGCTLD_AGC_TO_MODE = { 0: 'off', 1: 'fast', 2: 'med', 3: 'slow', 4: 'fast', 5: 'med', 6: 'auto' };
...
const _FLOAT01 = (v, line) => /^\d+(\.\d+)?$/.test(line) && v >= 0 && v <= 1;
const RIGCTLD_ACCEPTORS = {
  ...
  nrlevel: _FLOAT01, voxlevel: _FLOAT01, monlevel: _FLOAT01,
  micgain: _FLOAT01, complevel: _FLOAT01,
  // hamlib prints float levels with %g — "0.498039" mid-scale but BARE "0"
  // at zero and BARE "1" at full scale (verified against rigctld 4.7.0;
  // wfview always prints decimals, which is what round 3 was validated
  // against and how requiring a '.' here looked safe). A '.'-requiring
  // acceptor rejects its own boundary replies, and the orphaned "0"/"1"
  // then cascades into the next 0/1-shaped kind — K6RBJ's IC-7100 showed
  // NB/preamp pinned ON by exactly this. Queue ORDER is the disambiguator
  // (rigctld answers strictly in command order); acceptors only prune
  // impossible pairings, so 0/1 overlap with the toggle kinds is fine.
  rfgain: _FLOAT01, rfpower: _FLOAT01,
...
const RIGCTLD_EXT_KINDS = new Set(['rfgain', 'rfpower', 'pwrmeter', 'preamp', 'att', 'nr', 'comp', 'vox', 'anf', 'tuner',
  'agc', 'nrlevel', 'voxlevel', 'monlevel', 'micgain', 'complevel', 'mon', 'rit']);
// The cap is a LEAK GUARD, not a working bound — staleness is the TTL's
// job. It must exceed the worst-case in-flight window: an ext poll cycle
// writes ~24 commands back-to-back before the first reply returns, plus
// user commands can interleave. The old value (12, from an "at most ~8
// commands" era) evicted the first half of every ext cycle's entries at
// WRITE time, so replies arrived to a queue missing their owners and the
// whole cycle misattributed — K6RBJ's IC-7100 read NB's "1" as PREAMP,
// dropped the real preamp reply on the floor, and pinned both controls ON
// against every off-command (2026-08-28).
const RIGCTLD_PENDING_MAX = 48;
...
class RigctldCodec extends EventEmitter {
  ...
  constructor(model, writeFn) {
    ...
    this._preampDb = 0;      // 0 = unknown → setPreamp tries GAIN_ON_FALLBACKS.PREAMP
    this._attDb = 0;         // 0 = unknown → setAttenuator tries GAIN_ON_FALLBACKS.ATT
    this._gainTry = { PREAMP: 0, ATT: 0 }; // index into GAIN_ON_FALLBACKS while unknown
  ...
  /**
   * Send a raw command through rigctld's `w` and resolve with the radio's
   * reply line (rigctld prints it, then `RPRT 0`). Matched by prefix, as in
   * the Kenwood codec — see kenwood-codec.js queryRaw for why a feature needs
   * the text back rather than a log line. Bytes are escaped `\0xNN` on the
   * way out (lib/raw-cat-command.js).
   * @returns {Promise<string>}
   */
  queryRaw(cmd, { prefix, timeoutMs = 1500 } = {}) {
    const c = String(cmd || '').replace(/[\r\n]/g, '').trim();
    if (!c) return Promise.reject(new Error('empty command'));
    const pfx = prefix || c.replace(/;$/, '');
    if (!this._rawQueries) this._rawQueries = [];
    return new Promise((resolve, reject) => {
      const q = { prefix: pfx, resolve, reject, timer: null };
      q.timer = setTimeout(() => {
        const i = this._rawQueries.indexOf(q);
        if (i !== -1) this._rawQueries.splice(i, 1);
        reject(new Error(`no reply to ${c} within ${timeoutMs} ms`));
      }, timeoutMs);
      this._rawQueries.push(q);
      const arg = rigctldRawArg(c);
      this._noteUserCmd(`w ${arg}`);
      this._write(`w ${arg}\n`);
    });
  }

  _answerRawQuery(line) {
    if (!this._rawQueries || !this._rawQueries.length) return false;
    const i = this._rawQueries.findIndex((q) => line.startsWith(q.prefix));
    if (i === -1) return false;
    const [q] = this._rawQueries.splice(i, 1);
    clearTimeout(q.timer);
    q.resolve(line.endsWith(';') ? line : line + ';');
    return true;
  }
  ...
  setMode(modeName, freqHz) {
    ...
    if (token) {
      ...
      // Same-mode re-sends must NOT stomp the operator's width back to the
      // mode default: hamlib M always carries a passband, and several paths
      // legitimately re-send the current mode (band-recall resend, PTT
      // re-anchor). K6RBJ (IC-7100 via wfview, 2026-08-27): every explicit
      // ECHOCAT width (3000/1500/6000) was confirmed by the rig, then
      // reverted to 2400 within a second by exactly such a re-send. The
      // live width is tracked from setFilterWidth AND the passband
      // readback, so a width changed on the rig/wfview side survives too.
      // A genuine mode CHANGE still gets the mode-appropriate default.
      const samePb = this._lastKnownWidth > 0 && token === this._lastMode;
      const pb = supportsFilter ? (samePb ? this._lastKnownWidth : defaultPassbandFor(token)) : 0;
      this._write(`M ${token} ${pb}\n`);
      if (!samePb) this._lastKnownWidth = 0; // mode changed - default until told otherwise
  ...
  /**
   * FORWARD POWER, as measured by the radio right now (hamlib RFPOWER_METER,
   * 0..1 of the rig's rated output) — NOT `l RFPOWER`, which is the power
   * SETTING and is exactly what POTACAT has been showing on its "TX power
   * meter" all along: a flat number that never moves while transmitting
   * (LZ3AW 2026-08-28, TS-480). Polled during TX only, like SWR/ALC; latches
   * off per-connection if the backend rejects it, so rigs without a
   * wattmeter cost one query rather than a poll stream.
   */
  getPowerMeter() { this._getExt('pwrmeter', 'l RFPOWER_METER\n'); }
  ...
  setFilterWidth(hz) {
    ...
    this._lastKnownWidth = hz; // same-mode re-sends preserve this (see setMode)
  ...
  /** Inline-tuner enable/disable — see civ-codec setAtuEnabled for why this
   *  is distinct from the one-shot tune cycle. */
  setAtuEnabled(on) {
    this._write(on ? 'U TUNER 1' + String.fromCharCode(10) : 'U TUNER 0' + String.fromCharCode(10));
  }
  ...
  setPreamp(level) {
    if (this._yaesuRaw) {
      ...
      const db = steps.length
        ...
        : (level ? (this._preampDb || GAIN_ON_FALLBACKS.PREAMP[this._gainTry.PREAMP]) : 0);
  ...
  setAttenuator(level) {
    if (this._yaesuRaw) {
      ...
      const db = steps.length
        ? resolveStep(level, steps)
        : (level ? (this._attDb || GAIN_ON_FALLBACKS.ATT[this._gainTry.ATT]) : 0);
      const line = `L ATT ${db}\n`;
      this._noteUserCmd(line);
      this._write(line);
    }
   ...
   */
  /** Keep a preamp/ATT "on" value the rig accepted or reported (no probed ladder only). */
  _learnGainOn(kind, value) {
    if (kind === 'PREAMP' && !this._preampDbs.length && this._preampDb !== value) {
      this._preampDb = value;
      this.emit('log', `rigctld: preamp "on" is ${value} on this rig`);
    } else if (kind === 'ATT' && !this._attDbs.length && this._attDb !== value) {
      this._attDb = value;
      this.emit('log', `rigctld: attenuator "on" is ${value} on this rig`);
    }
  }

  /** The rig rejected a preamp/ATT "on" value we guessed: try the next one. */
  _retryGainOn(cmd) {
    const g = String(cmd || '').match(/^L (PREAMP|ATT) (\d+)$/);
    if (!g || +g[2] === 0) return;
    const kind = g[1];
    const probed = kind === 'PREAMP' ? this._preampDbs.length : this._attDbs.length;
    const known = kind === 'PREAMP' ? this._preampDb : this._attDb;
    if (probed) return; // the rig told us its ladder; do not guess past it
    if (known === +g[2]) { if (kind === 'PREAMP') this._preampDb = 0; else this._attDb = 0; } // stale
    const list = GAIN_ON_FALLBACKS[kind];
    const idx = list.indexOf(+g[2]);
    if (idx < 0) return;
    const next = list[idx + 1];
    if (next == null) {
      this.emit('log', `rigctld: this rig rejected every ${kind === 'PREAMP' ? 'preamp' : 'attenuator'} value POTACAT knows (${list.join(', ')})`);
      this._gainTry[kind] = 0;
      return;
    }
    this._gainTry[kind] = idx + 1;
    const line = `L ${kind} ${next}\n`;
    this.emit('log', `rigctld: "L ${kind} ${g[2]}" rejected — trying ${next}`);
    this._noteUserCmd(line);
    this._write(line);
  }
  ...
  sendRaw(text) {
    ...
    if (!cmd) return;
    // Hex bytes become `\0xNN` escapes for rigctld's `w`. The `0` is load
    // bearing: `\xNN` is not a form hamlib's parser knows, so it answered
    // `RPRT -1 (Invalid argument)` and never sent the frame (K8IKO's IC-7410
    // antenna buttons, 2026-09-20). See lib/raw-cat-command.js.
    const arg = rigctldRawArg(cmd);
    this._noteUserCmd(`w ${arg}`);
    this._write(`w ${arg}\n`);
  }
  ...
  _dispatchNumeric(kind, line, v) {
    switch (kind) {
      ...
      case 'pwrmeter': {
        // Measured forward power. Deliberately NOT change-gated: a meter that
        // only reports when it changes is not a meter.
        const watts = Math.max(0, Math.round(v * this._maxPower));
        this.emit('log', `rx: ${line} -> fwd power=${watts}W`);
        this.emit('powerMeter', watts);
        return;
      }
      case 'preamp': {
        const db = Math.round(v);
        if (db > 0) this._learnGainOn('PREAMP', db); // the rig's own reading is a value it takes
      ...
      case 'att': {
        const db = Math.round(v);
        if (db > 0) this._learnGainOn('ATT', db);
  ...
  _parseLine(line) {
    // A reply somebody is waiting for by prefix (queryRaw) is theirs first —
    // it is the radio's own text echoed by `w`, and would otherwise fall
    // through the numeric/mode chains as noise.
    if (this._answerRawQuery(line)) return;
    ...
    if (this._expectPassband) {
      ...
      if (/^\d+$/.test(line) && parseInt(line, 10) <= 100000) {
        ...
        if (hz > 0) {
          ...
          this._lastKnownWidth = hz; // keep same-mode re-sends from stomping it
    ...
    if (/^RPRT\s+-?\d+/.test(line)) {
      ...
      if (code !== 0) {
        ...
        if (u && Date.now() - u.ts < 1500) {
          ...
          this._retryGainOn(u.cmd);
        } else if (code !== this._lastRprtCode) {
          this.emit('log', `rx: ${line} (${meaning})`);
        }
      }
      // An accepted preamp/ATT "on" value is one this rig takes: keep it.
      if (code === 0) {
        const u = this._lastUserCmd;
        const g = u && Date.now() - u.ts < 1500 && u.cmd.match(/^L (PREAMP|ATT) (\d+)$/);
        if (g && +g[2] > 0) this._learnGainOn(g[1], +g[2]);
      }
