...
class WebSdrClient extends EventEmitter {
  ...
  _onData(data) {
    ...
    // Strip HTTP response headers — find the blank line separating headers from audio
    if (!this._headerDone) {
      const headerEnd = this._buf.indexOf('\r\n\r\n');
      if (headerEnd < 0) return; // headers not yet complete
      const headers = this._buf.slice(0, headerEnd).toString();
      console.log('[WebSDR] Response headers:', headers.replace(/\r\n/g, ' | '));
      this._buf = this._buf.slice(headerEnd + 4); // skip past \r\n\r\n
