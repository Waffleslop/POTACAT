...
class WebSdrClient extends EventEmitter {
  ...
  _sendTune(opts = {}) {
    ...
    // Passband edges are in kHz relative to the tuned (BFO) frequency — the
    // unit WebSDR.org's own client sends (websdr-base.js: "edges of passband,
    // in kHz w.r.t. the carrier"; USB is 0.3..2.7). POTACAT used to send Hz
    // (300..2700), which the server could only clamp.
    //
    // CW follows the official client too: a narrow window on the LSB side,
    // with the BFO ABOVE the station. WebSDR shows a narrow (< 1 kHz)
    // window's frequency as BFO + (lo+hi)/2, so with -0.8..-0.4 and the BFO
    // 0.6 kHz above the carrier, the site shows the station on its own
    // frequency and the carrier sounds at 600 Hz (the usual rig CW pitch).
    // POTACAT used to tune 0.6 kHz BELOW the carrier, so the tone was right
    // but WebSDR showed the station 600 Hz low (GitHub #93, DF1VB).
    const CW_PITCH_KHZ = 0.6;
    const toKhz = (v) => (Math.abs(Number(v)) > 20 ? Number(v) / 1000 : Number(v)); // accept a caller's Hz
    let lo, hi;
    if (opts.lo != null && opts.hi != null) {
      lo = toKhz(opts.lo); hi = toKhz(opts.hi);
    } else {
      switch (m) {
        case 'USB': lo = 0.3; hi = 2.7; break;
        case 'LSB': lo = -2.7; hi = -0.3; break;
        case 'CW': lo = -(CW_PITCH_KHZ + 0.2); hi = -(CW_PITCH_KHZ - 0.2); break;
        case 'AM': lo = -4; hi = 4; break;
        case 'FM': lo = -6; hi = 6; break;
        default: lo = 0.3; hi = 2.7; break;
      }
    }
    // CW: BFO above the station by the pitch (see above).
    let fKhz = this._desiredFreqKhz;
    if (m === 'CW') fKhz = +(fKhz + CW_PITCH_KHZ).toFixed(3);
