...
class JtcatManager extends EventEmitter {
  ...
  startSlice(config) {
    ...
    // PSK31 is a different engine class (continuous keyboard mode, no
    // slots/worker) — Ft8Engine.setMode() would silently coerce the mode
    // string to 'FT8', so the branch must happen here, not in setMode.
    let engine;
