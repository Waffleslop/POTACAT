...
class RigctldClient extends EventEmitter {
   ...
   *  Hex bytes are escaped as `\0xNN` — hamlib rejects a bare `\xNN`, and this
   *  path used to hand the hex straight through as ASCII text, so an Icom
   *  frame reached the radio as the characters "FE FE 80 …" (K8IKO). */
  sendRaw(text) {
    const cmd = text.replace(/[\r\n]/g, '').trim();
    if (!cmd) return;
    const arg = rigctldRawArg(cmd);
    this._sendImmediate(`w ${arg}\n`);
    this._log(`sendRaw: w ${arg}`);
  }
