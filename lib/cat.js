...
class CatClient extends EventEmitter {
  ...
  setSquelch(val) {
    if (!this.connected) return;
    const level = Math.max(0, Math.min(255, Math.round(val * 255)));
    this._write(`SQ0${String(level).padStart(3, '0')};`);
    this._log(`setSquelch ${level}`);
  }
...
class RigctldClient extends EventEmitter {
  ...
  setSquelch(val) {
    if (!this.connected) return;
    this._write(`L SQL ${val.toFixed(3)}\n`);
  }
