...
function loadCtyDat(filePath) {
  ...
  // Split on any newline variant — AD1C distributes cty.dat with LF, but
  // unzipped through Windows tools it can arrive with CRLF. A bare \n
  // split leaves a trailing \r on every line, which then sneaks into
  // prefix-block keys (e.g. ";\r" at end of block → "TX9W;" ending up
  // as the key, breaking TX9W lookups for Marquesas). (S50U 2026-04)
  ...
  while (i < lines.length) {
    ...
    // Parse prefix list. Strip the trailing ";" and any trailing whitespace
    // (belt-and-suspenders in case file has CR remnants or trailing blanks).
    prefixBlock = prefixBlock.replace(/[;\s]+$/, '');
