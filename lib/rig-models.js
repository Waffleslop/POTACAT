...
const RIG_MODELS = {
  ...
  'FT-710': {
    ...
    // is a SPACE character, not a digit. The codec's default of 0 produced
    // "KY0 <text>;" which the FT-710 silently rejects (radio's parser reads
    // P1='0', invalid, drops the frame — no carrier, no CW). Hamlib's newcat
    // sends "KY <text>;" for every FT-* rig; matching that here.
    cw: { text: 'ky1', textChunk: 50, speed: 'ks', paddleKey: 'txrx', dtrPins: null, taKey: false, breakIn: true, kyParam: '' },
    ...
    notes: [
      ...
      '[FT-710] Set the radio menu OPERATION SETTING -> GENERAL -> CAT-1 TIME OUT TIMER to 1000 ms. The factory default of 10 ms is too short for round-trip CAT and causes spurious "?;" rejections on commands like TX1; and MX1;.',
      '[FT-710] CW text-send needs BK-IN enabled on the radio (FUNC -> RADIO SETTING -> CW -> CW BK-IN = ON or SEMI). Without it, the KY queue fills but the radio never keys.',
    ],
  },
  'FT-817/818': {
    brand: 'Yaesu', protocol: 'kenwood',
