...
const RIG_MODELS = {
  ...
  'FT-710': {
    ...
    // TUNE button only.
    caps: { nb: true, atu: false, vfo: true, filter: true, filterType: 'indexed', rfgain: true, txpower: true, power: true, preamp: true, att: true, comp: true, nr: true, anf: true, vox: true, agc: true, rit: true },
  ...
  'FTX-1 field': {
    ...
    caps: {
      ...
      atu: false, vfo: true, filter: true, filterType: 'indexed',
      rfgain: true, afGain: true, txpower: true, power: true,
      preamp: true, attenuator: true, att: true, comp: true, compLevel: true, anf: true,
      vox: true, voxLevel: true, maxVoxLevel: 100, agc: true,
      agcAutoCollapsed: true, mon: true, monLevel: true, micGain: true,
      breakIn: true, breakInDelay: true,
      rit: true, maxClarHz: 9999, ftx1Preamp: true, ftx1Clar: true,
    },
    cw: {
      text: 'ky1', textChunk: 50, speed: 'ks', paddleKey: 'txrx',
      dtrPins: null, taKey: false, breakIn: true, kyMode: 'km', kyPlayCmd: 'KY05;',
    },
    atuCmd: false, minPower: 5, maxPower: 10,
    rmSwr: 6, rmAlc: 4, pcPrefix: 1, ftx1Preamp: true,
    ...
    notes: [
      '[FTX-1 field] HF/50 preamp is temporarily mapped as IPO for off and AMP1 for on. The radio supports a real 3-state HF/50 preamp model (IPO/AMP1/AMP2), so POTACAT still needs a dedicated UI/model update to expose AMP2.',
      '[FTX-1 field] The current preamp UI follows the MAIN VFO frequency by default, but POTACAT now lets you manually target HF/50, VHF, or UHF when you need SUB-side help. True separate SUB-side preamp state still needs a full dual-receive model.',
      '[FTX-1 field] RX and TX clarifier enable states are separate, but the CAT manual exposes one shared CLAR offset value rather than separate RX/TX offset frequencies.',
    ],
  },
  'FTX-1 optima': {
    brand: 'Yaesu', protocol: 'kenwood', connectDelay: 300,
    caps: {
      nb: true, nbLevel: true, maxNbLevel: 10,
      nr: true, nrLevel: true, maxNrLevel: 10,
      atu: true, vfo: true, filter: true, filterType: 'indexed',
      rfgain: true, afGain: true, txpower: true, power: true,
      preamp: true, attenuator: true, att: true, comp: true, compLevel: true, anf: true,
      vox: true, voxLevel: true, maxVoxLevel: 100, agc: true,
      agcAutoCollapsed: true, mon: true, monLevel: true, micGain: true,
    ...
    notes: [
      '[FTX-1 optima] HF/50 preamp is temporarily mapped as IPO for off and AMP1 for on. The radio supports a real 3-state HF/50 preamp model (IPO/AMP1/AMP2), so POTACAT still needs a dedicated UI/model update to expose AMP2.',
      '[FTX-1 optima] The current preamp UI follows the MAIN VFO frequency by default, but POTACAT now lets you manually target HF/50, VHF, or UHF when you need SUB-side help. True separate SUB-side preamp state still needs a full dual-receive model.',
      '[FTX-1 optima] RX and TX clarifier enable states are separate, but the CAT manual exposes one shared CLAR offset value rather than separate RX/TX offset frequencies.',
    ],
  },
  'FT-991/991A': {
    brand: 'Yaesu', protocol: 'kenwood',
  ...
  'FLEX-6300': {
    ...
    caps: { nb: true, atu: true, vfo: false, filter: true, filterType: 'arbitrary', rfgain: true, txpower: true, power: false, preamp: false, att: false, comp: true, nr: true, anf: true, vox: true, agc: true, nrLevel: true, nbLevel: true, voxLevel: true, mon: true, monLevel: true, rit: true },
