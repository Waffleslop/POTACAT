    ...
    return new Promise((resolve) => {
      ...
      const maybeAutoClose = () => {
        ...
        try {
          ...
          if (!/pota\.app/i.test(url)) return; // off on SSO provider — leave alone
          if (/\/user\/login/i.test(url)) return; // still on login route
          loginDetected = true;
          this._log(`login complete — URL is ${url}; auto-closing in 1.5s`);
          setTimeout(() => {
