...
class PotaSync extends EventEmitter {
  ...
  _decodeJwtLabel(token) {
    ...
    return claims.callsign || claims.username || claims['cognito:username'] || claims.email || null;
  }
  ...
  async _fetchText(url) {
    ...
    const doRequest = (token) => new Promise((resolve, reject) => {
      ...
      // Mirror the headers a fetch() from the SPA would send. KD2TJU's
      // 1.5.7 log showed freshly-refreshed tokens still 403'ing — meaning
      // the API authorizer wasn't rejecting on token freshness but on the
      // request shape. Browser fetches from pota.app always carry Origin
      // and Referer; raw `net.request` calls don't unless we set them.
      req.setHeader('Origin', 'https://pota.app');
      req.setHeader('Referer', 'https://pota.app/');
      // Sec-Fetch-* are emitted by Chromium-based browsers on every cross-
      // origin fetch and many gateways gate on these specifically. Match
      // what the SPA sends.
