        ...
        reqSession.webRequest.onBeforeSendHeaders(
          ...
          (details, callback) => {
            try {
              ...
              if (!this._liveHarvest && auth && /^Bearer eyJ[\w-]+\.[\w-]+\.[\w-]+/i.test(auth)) {
                this._liveHarvest = {
                  token: auth.replace(/^Bearer /i, ''),
                  source: 'api.pota.app Authorization header',
                };
                this._log('captured bearer token from api.pota.app request header');
              }
            } catch {}
        ...
        try {
          ...
          setTimeout(async () => {
            try {
              ...
              `);
              const stores = JSON.parse(raw || '{"ls":{},"ss":{}}');
              const lsKeys = Object.keys(stores.ls || {});
              const ssKeys = Object.keys(stores.ss || {});
              this._log(`live-window storage scan: localStorage=${lsKeys.length} keys [${lsKeys.slice(0, 8).join(', ')}${lsKeys.length > 8 ? ', ...' : ''}], sessionStorage=${ssKeys.length} keys [${ssKeys.slice(0, 8).join(', ')}${ssKeys.length > 8 ? ', ...' : ''}]`);
              let found = this._findJwt(stores.ls || {});
              let source = 'localStorage';
              if (!found.token) { found = this._findJwt(stores.ss || {}); source = 'sessionStorage'; }
              if (found.token) {
                this._liveHarvest = { token: found.token, source: `${source}["${found.key}"]` };
                this._log(`live-window harvest captured token from ${source}["${found.key}"]`);
              } else {
                this._log('live-window harvest: no JWT in either storage');
              }
            } catch (err) {
              this._log(`live-window harvest failed: ${err.message}`);
            }
            try { if (win && !win.isDestroyed()) win.close(); } catch {}
          }, 2500); // longer than the prior 1.5s — Cognito token exchange takes a beat
        } catch {}
      };
