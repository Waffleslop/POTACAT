...
class PotaSync extends EventEmitter {
  ...
  async _probeSession() {
    try {
      ...
      if (liveHarvest && liveHarvest.token) {
        ...
        // First fallback: read Cognito tokens directly from the partition
        // cookies. pota.app is configured with cookie-storage Cognito, so
        // the JWT lives at the cookie named
        //   CognitoIdentityServiceProvider.<clientId>.<userId>.idToken
        // and we can grab it without needing to spin up another window.
        const idTokenCookie = (cookies || []).find(c => /^CognitoIdentityServiceProvider\..+\..+\.idToken$/.test(c.name));
        const accessTokenCookie = (cookies || []).find(c => /^CognitoIdentityServiceProvider\..+\..+\.accessToken$/.test(c.name));
        const cookieToken = (idTokenCookie && idTokenCookie.value) || (accessTokenCookie && accessTokenCookie.value);
        if (cookieToken && /^eyJ[\w-]+\.[\w-]+\.[\w-]+/.test(cookieToken)) {
          const which = idTokenCookie ? 'idToken' : 'accessToken';
          harvest = { token: cookieToken, key: `cookie ${which}` };
          this._log(`probe captured token from Cognito ${which} cookie`);
        } else {
          // Last-resort fallback: spin up a hidden window and scan its
          // partition storage. Useful only on SPAs that genuinely use
          // localStorage; cookie-storage Cognito is already covered above.
      ...
      // pota.app's SPA sends the JWT raw (no "Bearer " prefix). Mirror that
      // when we make our own requests — sending "Bearer <jwt>" to api.pota.app
      // gets a 403 because the API Gateway authorizer expects the token by
      // itself. We default to "no Bearer prefix" since the harvested token
      // is the same JWT either way; webRequest captures may also flag this
      // explicitly via sendBearerPrefix.
      s.bearerSendPrefix = (liveHarvest && liveHarvest.sendBearerPrefix) || false;
  ...
  _fetchText(url) {
    return new Promise((resolve, reject) => {
      ...
      // prefix — the SPA itself sends just the raw token (WD4DAN report:
      // adding "Bearer " gave HTTP 403). Mirror what was observed on the
      // SPA's outbound headers, falling back to raw JWT when we don't
      // explicitly know.
      const s = this._state();
      const token = s.bearerToken;
      if (token) {
        req.setHeader('Authorization', s.bearerSendPrefix ? ('Bearer ' + token) : token);
      }
      req.on('response', (res) => {
