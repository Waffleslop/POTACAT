...
function buildContestHistory(contests, qsos, opts = {}) {
  ...
  for (const c of contests || []) {
    ...
    if ((c.durationHours || 24) > MAX_WINDOW_HOURS) continue;
    ...
    for (const year of startYears) {
      ...
      windowsByYear.get(year).push({ id: c.id, year, ...w, modeOk, bandOk });
    }
  }
  ...
  for (const rec of records) {
    ...
    // covers the QSO day, else the QSO's own year. Stamps naming ids
    // OUTSIDE the catalog (today's special events — 13col-2026 etc.) fall
    // through to the heuristics unchanged; this path activates fully once
    // the unified events/contests registry (roadmap #1) aligns the ids.
    const stampedId = String(rec.APP_POTACAT_EVENT || '').trim();
