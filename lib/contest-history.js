...
function buildContestHistory(contests, qsos, opts = {}) {
  ...
  const addWindow = (entry, withinCap) => {
    if (!provWindowsByYear.has(entry.year)) provWindowsByYear.set(entry.year, []);
    provWindowsByYear.get(entry.year).push(entry);
    if (withinCap) {
      if (!windowsByYear.has(entry.year)) windowsByYear.set(entry.year, []);
      windowsByYear.get(entry.year).push(entry);
    }
  };
  for (const c of contests || []) {
    ...
    // One-shot entries synthesized from server events (unified registry
    // Phase B) carry explicit ISO windows instead of a whenComputed rule —
    // day-inclusive like yearWindow, keyed by the window's own start year.
    if (Array.isArray(c.explicitWindows) && c.explicitWindows.length) {
      for (const w of c.explicitWindows) {
        ...
        addWindow({
          id: c.id, year: start.getUTCFullYear(),
          startDay: _dayStr(start), endDay: _dayStr(new Date(end.getTime() - 1)),
          modeOk, bandOk,
        }, hours <= MAX_WINDOW_HOURS);
      }
      continue;
    }
    if (!YEAR_BOUND_RULE_RE.test(c.whenComputed || '')) continue;
    const withinCap = (c.durationHours || 24) <= MAX_WINDOW_HOURS;
    for (const year of startYears) {
      const w = yearWindow(c, year);
      if (!w) continue;
      addWindow({ id: c.id, year, ...w, modeOk, bandOk }, withinCap);
    }
  }
