...
void freedv_comptx_2020(struct freedv *f, COMP mod_out[]) {
  ...
  if (f->test_frames) {
    #ifdef _MSC_VER
    uint8_t *payload_data_bits = (uint8_t *)_alloca((data_bits_per_frame) * sizeof(uint8_t));
    #else
    #ifdef _MSC_VER
    uint8_t *payload_data_bits = (uint8_t *)_alloca((data_bits_per_frame) * sizeof(uint8_t));
    #else
  ...
  ofdm_ldpc_interleave_tx(f->ofdm, f->ldpc, (cf_complex *)mod_out, tx_bits,
                          txt_bits);
}
...
int freedv_comprx_2020(struct freedv *f, COMP demod_in[]) {
  ...
  #ifdef _MSC_VER
  int *rx_bits = (int *)_alloca((f->ofdm_bitsperframe) * sizeof(int));
  #else
  #ifdef _MSC_VER
  int *rx_bits = (int *)_alloca((f->ofdm_bitsperframe) * sizeof(int));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  COMP *payload_syms = (COMP *)_alloca((coded_syms_per_frame) * sizeof(COMP));
  #else
  #ifdef _MSC_VER
  COMP *payload_syms = (COMP *)_alloca((coded_syms_per_frame) * sizeof(COMP));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  float *payload_amps = (float *)_alloca((coded_syms_per_frame) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *payload_amps = (float *)_alloca((coded_syms_per_frame) * sizeof(float));
  #else
  ...
  if ((ofdm->sync_state == synced) || (ofdm->sync_state == trial)) {
    ...
    #ifdef _MSC_VER
    COMP *codeword_symbols_de = (COMP *)_alloca((coded_syms_per_frame) * sizeof(COMP));
    #else
    #ifdef _MSC_VER
    COMP *codeword_symbols_de = (COMP *)_alloca((coded_syms_per_frame) * sizeof(COMP));
    #else
    ...
    #endif
    #ifdef _MSC_VER
    float *codeword_amps_de = (float *)_alloca((coded_syms_per_frame) * sizeof(float));
    #else
    #ifdef _MSC_VER
    float *codeword_amps_de = (float *)_alloca((coded_syms_per_frame) * sizeof(float));
    #else
    ...
    #ifdef _MSC_VER
    float *llr = (float *)_alloca((coded_bits_per_frame) * sizeof(float));
    #else
    #ifdef _MSC_VER
    float *llr = (float *)_alloca((coded_bits_per_frame) * sizeof(float));
    #else
    ...
    #endif
    #ifdef _MSC_VER
    uint8_t *out_char = (uint8_t *)_alloca((coded_bits_per_frame) * sizeof(uint8_t));
    #else
    #ifdef _MSC_VER
    uint8_t *out_char = (uint8_t *)_alloca((coded_bits_per_frame) * sizeof(uint8_t));
    #else
