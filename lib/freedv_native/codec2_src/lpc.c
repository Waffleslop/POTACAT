...
void levinson_durbin(float R[],    /* order+1 autocorrelation coeff */
...
#ifdef _MSC_VER
  int _a_dim = order + 1;
  float *_a_flat = (float *)_alloca(_a_dim * _a_dim * sizeof(float));
  #define a_at(r,c) _a_flat[(r) * _a_dim + (c)]
#else
  float a[order + 1][order + 1];
  #define a_at(r,c) a[r][c]
#endif
  float sum, e, k;
  int i, j; /* loop variables */
  ...
  for (i = 1; i <= order; i++) {
    sum = 0.0;
    for (j = 1; j <= i - 1; j++) sum += a_at(i - 1, j) * R[i - j];
    ...
    for (j = 1; j <= i - 1; j++)
      a_at(i, j) = a_at(i - 1, j) + k * a_at(i - 1, i - j); /* Equation 38c, Makhoul */

    e *= (1 - k * k); /* Equation 38d, Makhoul */
  }

  for (i = 1; i <= order; i++) lpcs[i] = a_at(order, i);
  lpcs[0] = 1.0;
}
