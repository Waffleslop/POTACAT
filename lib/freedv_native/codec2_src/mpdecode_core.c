    ...
    if (shift > 0) {
      ...
      for (i = 0; i < (NumberParityBits / shift); i++) {
        for (k = 0; k < shift; k++) {
          // Allocate sub nodes
          c_nodes[cnt].subs =
              (struct c_sub_node *)CALLOC(c_nodes[cnt].degree, sizeof(struct c_sub_node));
...
void Somap(float bit_likelihood[],    /* number_bits, bps*number_symbols */
  ...
  int n, i, j, k, mask;
#ifdef _MSC_VER
  float *num = (float *)_alloca(bps * sizeof(float));
  float *den = (float *)_alloca(bps * sizeof(float));
#else
...
void symbols_to_llrs(float llr[], COMP rx_qpsk_symbols[], float rx_amps[],
  ...
  #ifdef _MSC_VER
  float *symbol_likelihood = (float *)_alloca((nsyms * QPSK_CONSTELLATION_SIZE) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *symbol_likelihood = (float *)_alloca((nsyms * QPSK_CONSTELLATION_SIZE) * sizeof(float));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  float *bit_likelihood = (float *)_alloca((nsyms * QPSK_BITS_PER_SYMBOL) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *bit_likelihood = (float *)_alloca((nsyms * QPSK_BITS_PER_SYMBOL) * sizeof(float));
  #else
...
void fsk_rx_filt_to_llrs(float llr[], float rx_filt[], float v_est,
  ...
  int bps = log2(M);
  #ifdef _MSC_VER
  float *symbol_likelihood = (float *)_alloca((M * nsyms) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *symbol_likelihood = (float *)_alloca((M * nsyms) * sizeof(float));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  float *bit_likelihood = (float *)_alloca((bps * nsyms) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *bit_likelihood = (float *)_alloca((bps * nsyms) * sizeof(float));
  #else
