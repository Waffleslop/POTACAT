...
void fsk_mod(struct FSK *fsk, float fsk_out[], uint8_t tx_bits[], int nbits) {
  ...
  int M = fsk->mode;
  #ifdef _MSC_VER
  COMP *dosc_f = (COMP *)_alloca(M * sizeof(COMP));
  #else
  COMP dosc_f[M];
  #endif /* phase shift per sample */
...
void fsk_demod_freq_est(struct FSK *fsk, COMP fsk_in[], float *freqs, int M) {
  ...
  #ifdef _MSC_VER
  int *freqi = (int *)_alloca(M * sizeof(int));
  #else
  int freqi[M];
  #endif
  int st, en, f_zero;
  ...
  /* construct mask */
  #ifdef _MSC_VER
  float *mask = (float *)_alloca((Ndft) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *mask = (float *)_alloca((Ndft) * sizeof(float));
  #else
...
void fsk_demod_core(struct FSK *fsk, uint8_t rx_bits[], float rx_filt[],
  ...
  #ifdef _MSC_VER
  COMP *t = (COMP *)_alloca(M * sizeof(COMP));
  #else
  COMP t[M];
  #endif /* complex number temps */
...
#define NMP_NAME 26
  ...
  int _f_int_cols = (nsym + 1) * P;
#ifdef _MSC_VER
  COMP *_f_int_flat = (COMP *)_alloca(M * _f_int_cols * sizeof(COMP));
  memset(_f_int_flat, 0, M * _f_int_cols * sizeof(COMP));
  #define f_int_at(r,c) _f_int_flat[(r) * _f_int_cols + (c)]
#else
  COMP f_int_at(M,(nsym + 1) * P);
  #define f_int_at(r,c) f_int_at(r,c)
#endif
  for (i = 0; i < (nsym + 1) * P; i++) {
    ...
    for (m = 0; m < M; m++) {
      f_int_at(m,i) = comp0();
      for (j = st; j <= en; j++)
        f_int_at(m,i) = cadd(f_int_at(m,i), f_dc[m * Nmem + j]);
    }
  }
  ...
  for (m = 0; m < M; m++) {
    ...
    modem_probe_samp_c(mp_name_tmp, &f_int_at(m,0), (nsym + 1) * P);
  }
#endif
  ...
  for (i = 0; i < (nsym + 1) * P; i++) {
    ...
    ft1 = 0;
    for (m = 0; m < M; m++) {
      ft1 += (f_int_at(m,i).real * f_int_at(m,i).real) +
             (f_int_at(m,i).imag * f_int_at(m,i).imag);
    }
  ...
  #ifdef _MSC_VER
  float *tmax = (float *)_alloca(M * sizeof(float));
  #else
  float tmax[M];
  #endif
  ...
  for (i = 0; i < nsym; i++) {
    ...
    int st = (i + 1) * P;
    for (m = 0; m < M; m++) {
      t[m] = fcmult(1 - fract, f_int_at(m,st + low_sample));
      t[m] = cadd(t[m], fcmult(fract, f_int_at(m,st + high_sample)));
    ...
    for (m = 0; m < M; m++) {
      for (j = 0; j < neyesamp; j++) {
        ...
        */
        ...
        assert(ind >= 0);
        assert(ind < (nsym + 1) * P);
        fsk->stats->rx_eye[i * M + m][j] = cabsolute(f_int_at(m,ind));
      }
    }
  }
