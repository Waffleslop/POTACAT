...
void codec2_encode_700c(struct CODEC2 *c2, unsigned char *bits,
  ...
  int K = 20;
#ifdef _MSC_VER
  float *rate_K_vec = (float *)_alloca(K * sizeof(float));
  float mean;
  float *rate_K_vec_no_mean = (float *)_alloca(K * sizeof(float));
...
void codec2_decode_700c(struct CODEC2 *c2, short speech[],
  ...
  int M = 4;
#ifdef _MSC_VER
  COMP (*HH)[MAX_AMP + 1] = (COMP (*)[MAX_AMP + 1])_alloca(M * (MAX_AMP + 1) * sizeof(COMP));
  float (*interpolated_surface_)[NEWAMP1_K] = (float (*)[NEWAMP1_K])_alloca(M * NEWAMP1_K * sizeof(float));
#else
  COMP HH[M][MAX_AMP + 1];
...
#ifdef _MSC_VER
  int _tmp_sz = newamp1vq_cb[num].k * newamp1vq_cb[num].m;
  float *tmp = (float *)malloc(_tmp_sz * sizeof(float));
#else
