...
typedef struct {
  ...
  cf_complex inbound_pending_syms[(RELIABLE_TEXT_UW_LENGTH_BITS +
...
static int reliable_text_ldpc_decode(reliable_text_impl_t* obj, char* dest) {
  ...
  cf_complex deinterleavedSyms[LDPC_TOTAL_SIZE_BITS / 2];
...
static void reliable_text_freedv_callback_rx_sym(void* state,
  ...
  obj->inbound_pending_syms[obj->sym_index] = (cf_complex)sym;
    ...
    if (uw_bits_valid) {
      ...
      if (reliable_text_ldpc_decode(obj, rawStr) != 0) {
        ...
        memset(&obj->inbound_pending_syms, 0,
               sizeof(cf_complex) * LDPC_TOTAL_SIZE_BITS / 2);
...
reliable_text_t reliable_text_create() {
  reliable_text_impl_t* ret = (reliable_text_impl_t *)calloc(1, sizeof(reliable_text_impl_t));
  assert(ret != NULL);
