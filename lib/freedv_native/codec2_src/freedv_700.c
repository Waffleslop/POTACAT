...
void freedv_comptx_700c(struct freedv *f, COMP mod_out[]) {
  int i;
  #ifdef _MSC_VER
  COMP *tx_fdm = (COMP *)_alloca((f->n_nat_modem_samples) * sizeof(COMP));
  #else
  #ifdef _MSC_VER
  COMP *tx_fdm = (COMP *)_alloca((f->n_nat_modem_samples) * sizeof(COMP));
  #else
  ...
  i = quisk_cfInterpDecim((cf_complex *)mod_out, f->n_nat_modem_samples,
...
void freedv_comptx_ofdm(struct freedv *f, COMP mod_out[]) {
  ...
  #ifdef _MSC_VER
  uint8_t *txt_bits = (uint8_t *)_alloca((nspare) * sizeof(uint8_t));
  #else
  #ifdef _MSC_VER
  uint8_t *txt_bits = (uint8_t *)_alloca((nspare) * sizeof(uint8_t));
  #else
  ...
  if (f->test_frames) {
    #ifdef _MSC_VER
    uint8_t *payload_data_bits = (uint8_t *)_alloca((f->bits_per_modem_frame) * sizeof(uint8_t));
    #else
    #ifdef _MSC_VER
    uint8_t *payload_data_bits = (uint8_t *)_alloca((f->bits_per_modem_frame) * sizeof(uint8_t));
    #else
  ...
  ofdm_ldpc_interleave_tx(f->ofdm, f->ldpc, (cf_complex *)mod_out,
...
int freedv_comprx_700c(struct freedv *f, COMP demod_in_8kHz[]) {
  ...
  #ifdef _MSC_VER
  COMP *demod_in = (COMP *)_alloca((freedv_nin(f)) * sizeof(COMP));
  #else
  #ifdef _MSC_VER
  COMP *demod_in = (COMP *)_alloca((freedv_nin(f)) * sizeof(COMP));
  #else
  ...
  i = quisk_cfInterpDecim((cf_complex *)demod_in, freedv_nin(f),
...
int freedv_comp_short_rx_ofdm(struct freedv *f, void *demod_in_8kHz,
  ...
  cf_complex *rx_syms = (cf_complex *)f->rx_syms;
  float *rx_amps = f->rx_amps;

  #ifdef _MSC_VER
  int *rx_bits = (int *)_alloca((Nbitsperframe) * sizeof(int));
  #else
  #ifdef _MSC_VER
  int *rx_bits = (int *)_alloca((Nbitsperframe) * sizeof(int));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  short *txt_bits = (short *)_alloca((f->ofdm_ntxtbits) * sizeof(short));
  #else
  #ifdef _MSC_VER
  short *txt_bits = (short *)_alloca((f->ofdm_ntxtbits) * sizeof(short));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  COMP *payload_syms = (COMP *)_alloca((Npayloadsymsperpacket) * sizeof(COMP));
  #else
  #ifdef _MSC_VER
  COMP *payload_syms = (COMP *)_alloca((Npayloadsymsperpacket) * sizeof(COMP));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  float *payload_amps = (float *)_alloca((Npayloadsymsperpacket) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *payload_amps = (float *)_alloca((Npayloadsymsperpacket) * sizeof(float));
  #else
  ...
  #ifdef _MSC_VER
  uint8_t *rx_uw = (uint8_t *)_alloca((f->ofdm_nuwbits) * sizeof(uint8_t));
  #else
  #ifdef _MSC_VER
  uint8_t *rx_uw = (uint8_t *)_alloca((f->ofdm_nuwbits) * sizeof(uint8_t));
  #else
  ...
  if ((ofdm->sync_state == synced) || (ofdm->sync_state == trial)) {
    ...
    memcpy(&rx_syms[Nsymsperpacket - Nsymsperframe], ofdm->rx_np,
           sizeof(cf_complex) * Nsymsperframe);
    ...
    if (ofdm->modem_frame == (ofdm->np - 1)) {
      ...
      #ifdef _MSC_VER
      COMP *payload_syms_de = (COMP *)_alloca((Npayloadsymsperpacket) * sizeof(COMP));
      #else
      #ifdef _MSC_VER
      COMP *payload_syms_de = (COMP *)_alloca((Npayloadsymsperpacket) * sizeof(COMP));
      #else
      ...
      #endif
      #ifdef _MSC_VER
      float *payload_amps_de = (float *)_alloca((Npayloadsymsperpacket) * sizeof(float));
      #else
      #ifdef _MSC_VER
      float *payload_amps_de = (float *)_alloca((Npayloadsymsperpacket) * sizeof(float));
      #else
      ...
      #ifdef _MSC_VER
      float *llr = (float *)_alloca((Npayloadbitsperpacket) * sizeof(float));
      #else
      #ifdef _MSC_VER
      float *llr = (float *)_alloca((Npayloadbitsperpacket) * sizeof(float));
      #else
      ...
      #endif
      #ifdef _MSC_VER
      uint8_t *decoded_codeword = (uint8_t *)_alloca((Npayloadbitsperpacket) * sizeof(uint8_t));
      #else
      #ifdef _MSC_VER
      uint8_t *decoded_codeword = (uint8_t *)_alloca((Npayloadbitsperpacket) * sizeof(uint8_t));
      #else
      ...
      if (f->test_frames) {
        ...
        #ifdef _MSC_VER
        uint8_t *payload_data_bits = (uint8_t *)_alloca((Ndatabitsperpacket) * sizeof(uint8_t));
        #else
        #ifdef _MSC_VER
        uint8_t *payload_data_bits = (uint8_t *)_alloca((Ndatabitsperpacket) * sizeof(uint8_t));
        #else
