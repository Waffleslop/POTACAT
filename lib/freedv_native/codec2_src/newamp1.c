...
float rate_K_mbest_encode(int *indexes, float *x, float *xq, int ndim,
  ...
  #ifdef _MSC_VER
  float *target = (float *)_alloca((ndim) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *target = (float *)_alloca((ndim) * sizeof(float));
  #else
...
#ifdef _MSC_VER
  float *pre = (float *)_alloca(K * sizeof(float));
#else
  float pre[K];
#endif
  float e_before = 0.0;
...
void resample_rate_L(C2CONST *c2const, MODEL *model, float rate_K_vec[],
                     float rate_K_sample_freqs_kHz[], int K) {
#ifdef _MSC_VER
  float *rate_K_vec_term = (float *)_alloca((K + 2) * sizeof(float));
  float *rate_K_sample_freqs_kHz_term = (float *)_alloca((K + 2) * sizeof(float));
#else
...
void determine_phase(C2CONST *c2const, COMP H[], MODEL *model, int Nfft,
  ...
  int Ns = Nfft / 2 + 1;
#ifdef _MSC_VER
  float *Gdbfk = (float *)_alloca(Ns * sizeof(float));
  float *sample_freqs_kHz = (float *)_alloca(Ns * sizeof(float));
  float *phase = (float *)_alloca(Ns * sizeof(float));
#else
...
#ifdef _MSC_VER
  COMP *S = (COMP *)_alloca(Nfft * sizeof(COMP));
  COMP *R = (COMP *)_alloca(Nfft * sizeof(COMP));
#else
  COMP S[Nfft], R[Nfft];
#endif
...
void newamp1_indexes_to_model(C2CONST *c2const, MODEL model_[], COMP H[],
                              ...
                              int post_filter_en) {
#ifdef _MSC_VER
  float *rate_K_vec_ = (float *)_alloca(K * sizeof(float));
  float *rate_K_vec_no_mean_ = (float *)_alloca(K * sizeof(float));
...
#ifdef _MSC_VER
  float *aWo_ = (float *)_alloca(M * sizeof(float));
#else
  float aWo_[M];
#endif
#ifdef _MSC_VER
  int *avoicing_ = (int *)_alloca(M * sizeof(int));
  int *aL_ = (int *)_alloca(M * sizeof(int));
  int i;
#else
