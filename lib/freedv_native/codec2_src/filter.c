...
void quisk_filt_cfInit(struct quisk_cfFilter *filter, float *coefs, int taps) {
  ...
  filter->cSamples = (cf_complex *)MALLOC(taps * sizeof(cf_complex));
  memset(filter->cSamples, 0, taps * sizeof(cf_complex));
...
int quisk_cfInterpDecim(cf_complex *cSamples, int count,
  ...
  if (count > filter->nBuf) {  // increase size of sample buffer
    ...
    filter->cBuf =
        (cf_complex *)MALLOC(filter->nBuf * sizeof(cf_complex));
  }

  memcpy(filter->cBuf, cSamples, count * sizeof(cf_complex));
  nOut = 0;
...
\*---------------------------------------------------------------------------*/
#if 0
int quisk_ccfInterpDecim(cf_complex * cSamples, int count, struct quisk_cfFilter * filter, int interp, int decim) {
    // Interpolate by interp, and then decimate by decim.
    ...
    if (count > filter->nBuf) {    // increase size of sample buffer
        ...
        if (filter->cBuf)
            FREE(filter->cBuf);
        filter->cBuf = (cf_complex *)MALLOC(filter->nBuf * sizeof(cf_complex));
    }
...
void quisk_cfTune(struct quisk_cfFilter *filter, float freq) {
  ...
  if (!filter->cpxCoefs)
    filter->cpxCoefs =
        (cf_complex *)MALLOC(filter->nTaps * sizeof(cf_complex));
...
void quisk_ccfFilter(cf_complex *inSamples, cf_complex *outSamples,
