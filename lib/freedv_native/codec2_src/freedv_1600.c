...
void freedv_1600_open(struct freedv *f) {
  ...
  f->n_codec_frames = 1;
  f->tx_payload_bits = (uint8_t *)MALLOC(f->bits_per_codec_frame);
  assert(f->tx_payload_bits != NULL);
  f->rx_payload_bits = (uint8_t *)MALLOC(f->bits_per_codec_frame);
...
int freedv_comprx_fdmdv_1600(struct freedv *f, COMP demod_in[]) {
  ...
  #ifdef _MSC_VER
  COMP *ademod_in = (COMP *)_alloca((f->nin) * sizeof(COMP));
  #else
  #ifdef _MSC_VER
  COMP *ademod_in = (COMP *)_alloca((f->nin) * sizeof(COMP));
  #else
  ...
  if (f->sync) {
    ...
    if (f->evenframe == 0) {
      ...
      if (f->test_frames == 0) {
        ...
        #ifdef _MSC_VER
        short *error_pattern = (short *)_alloca((fdmdv_error_pattern_size(f->fdmdv)) * sizeof(short));
        #else
        #ifdef _MSC_VER
        short *error_pattern = (short *)_alloca((fdmdv_error_pattern_size(f->fdmdv)) * sizeof(short));
        #else
