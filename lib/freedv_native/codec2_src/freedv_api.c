...
static void codec2_encode_upacked(struct freedv *f, uint8_t unpacked_bits[],
  ...
  #ifdef _MSC_VER
  uint8_t *packed_codec_bits = (uint8_t *)_alloca((n_packed) * sizeof(uint8_t));
  #else
  #ifdef _MSC_VER
  uint8_t *packed_codec_bits = (uint8_t *)_alloca((n_packed) * sizeof(uint8_t));
  #else
...
void freedv_tx(struct freedv *f, short mod_out[], short speech_in[]) {
  assert(f != NULL);
  #ifdef _MSC_VER
  COMP *tx_fdm = (COMP *)_alloca((f->n_nom_modem_samples) * sizeof(COMP));
  #else
  #ifdef _MSC_VER
  COMP *tx_fdm = (COMP *)_alloca((f->n_nom_modem_samples) * sizeof(COMP));
  #else
...
/* cf_complex output version of freedv_tx() */
...
unsigned short freedv_crc16_unpacked(unsigned char unpacked_bits[], int nbits) {
  ...
  int nbytes = nbits / 8;
  #ifdef _MSC_VER
  uint8_t *packed_bytes = (uint8_t *)_alloca((nbytes) * sizeof(uint8_t));
  #else
  #ifdef _MSC_VER
  uint8_t *packed_bytes = (uint8_t *)_alloca((nbytes) * sizeof(uint8_t));
  #else
...
void freedv_rawdatatx(struct freedv *f, short mod_out[],
  ...
  assert(f != NULL);
  #ifdef _MSC_VER
  COMP *mod_out_comp = (COMP *)_alloca((f->n_nat_modem_samples) * sizeof(COMP));
  #else
  #ifdef _MSC_VER
  COMP *mod_out_comp = (COMP *)_alloca((f->n_nat_modem_samples) * sizeof(COMP));
  #else
...
int freedv_rawdatapreamblecomptx(struct freedv *f, COMP mod_out[]) {
  ...
  if (f->mode == FREEDV_MODE_FSK_LDPC) {
    ...
    cf_complex *tx_preamble = (cf_complex *)mod_out;
...
int freedv_rawdatapostamblecomptx(struct freedv *f, COMP mod_out[]) {
  ...
  if (is_ofdm_data_mode(f)) {
    struct OFDM *ofdm = f->ofdm;
    cf_complex *tx_postamble = (cf_complex *)mod_out;
...
/*---------------------------------------------------------------------------*\
  ...
  #ifdef _MSC_VER
  short *demod_in = (short *)_alloca((freedv_get_n_max_modem_samples(f)) * sizeof(short));
  #else
  #ifdef _MSC_VER
  short *demod_in = (short *)_alloca((freedv_get_n_max_modem_samples(f)) * sizeof(short));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  short *speech_out = (short *)_alloca((freedv_get_n_max_speech_samples(f)) * sizeof(short));
  #else
  #ifdef _MSC_VER
  short *speech_out = (short *)_alloca((freedv_get_n_max_speech_samples(f)) * sizeof(short));
  #else
...
int freedv_rx(struct freedv *f, short speech_out[], short demod_in[]) {
  ...
  if (FDV_MODE_ACTIVE(FREEDV_MODE_2400A, f->mode) ||
      ...
      FDV_MODE_ACTIVE(FREEDV_MODE_800XA, f->mode)) {
    #ifdef _MSC_VER
    float *rx_float = (float *)_alloca((f->n_max_modem_samples) * sizeof(float));
    #else
    #ifdef _MSC_VER
    float *rx_float = (float *)_alloca((f->n_max_modem_samples) * sizeof(float));
    #else
  ...
  if (FDV_MODE_ACTIVE(FREEDV_MODE_1600, f->mode) ||
    ...
    assert(nin <= f->n_max_modem_samples);
    #ifdef _MSC_VER
    COMP *rx_fdm = (COMP *)_alloca((f->n_max_modem_samples) * sizeof(COMP));
    #else
    #ifdef _MSC_VER
    COMP *rx_fdm = (COMP *)_alloca((f->n_max_modem_samples) * sizeof(COMP));
    #else
...
int freedv_comprx(struct freedv *f, short speech_out[], COMP demod_in[]) {
  ...
  #ifdef _MSC_VER
  short *demod_in_short = (short *)_alloca((f->nin_prev) * sizeof(short));
  #else
  #ifdef _MSC_VER
  short *demod_in_short = (short *)_alloca((f->nin_prev) * sizeof(short));
  #else
  ...
  if ((rx_status & FREEDV_RX_SYNC) == 0) {
    if (!f->squelch_en) {
      ...
      if ((f->mode == FREEDV_MODE_2020) || (f->mode == FREEDV_MODE_2020B)) {
        ...
        #ifdef _MSC_VER
        float *tmp = (float *)_alloca((nout) * sizeof(float));
        #else
        #ifdef _MSC_VER
        float *tmp = (float *)_alloca((nout) * sizeof(float));
        #else
...
int freedv_rawdatarx(struct freedv *f, unsigned char *packed_payload_bits,
  ...
  assert(nin <= f->n_max_modem_samples);
  #ifdef _MSC_VER
  COMP *demod_in_comp = (COMP *)_alloca((f->n_max_modem_samples) * sizeof(COMP));
  #else
  #ifdef _MSC_VER
  COMP *demod_in_comp = (COMP *)_alloca((f->n_max_modem_samples) * sizeof(COMP));
  #else
