...
struct quisk_cfFilter {     // Structure to hold the static data for FIR filters
  ...
  cf_complex *cpxCoefs;  // complex filter coefficients
  ...
  cf_complex *cSamples;  // storage for old samples
  cf_complex *ptcSamp;   // next available position in cSamples
  cf_complex *cBuf;      // auxiliary buffer for interpolation
};

extern int quisk_cfInterpDecim(cf_complex *, int, struct quisk_cfFilter *,
                               int, int);
...
extern void quisk_ccfFilter(cf_complex *, cf_complex *, int,
