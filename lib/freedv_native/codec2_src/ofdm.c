...
static float cnormf(cf_complex);
...
static void dft(struct OFDM *, cf_complex *, cf_complex *);
static void idft(struct OFDM *, cf_complex *, cf_complex *);
static cf_complex vector_sum(cf_complex *, int);
static int est_timing(struct OFDM *, cf_complex *, int, int, float *, int *,
                      int);
static float est_freq_offset_pilot_corr(struct OFDM *, cf_complex *, int,
                                        int);
 ...
 */
static const cf_complex qpsk[] = {1.0f + 0.0f * I, 0.0f + 1.0f * I,
                                     0.0f - 1.0f * I, -1.0f + 0.0f * I};

static const cf_complex qam16[] = {
...
static float cnormf(cf_complex val) {
 ...
 */
cf_complex qpsk_mod(int *bits) { return qpsk[(bits[1] << 1) | bits[0]]; }
 ...
 * 11 | 10
 */
void qpsk_demod(cf_complex symbol, int *bits) {
  cf_complex rotate = symbol * cmplx(ROT45);

  bits[0] = crealf(rotate) <= 0.0f;
  bits[1] = cimagf(rotate) <= 0.0f;
}

cf_complex qam16_mod(int *bits) {
  return qam16[(bits[3] << 3) | (bits[2] << 2) | (bits[1] << 1) | bits[0]];
}

void qam16_demod(cf_complex symbol, int *bits) {
  float dist[16];
  int i;
...
struct OFDM *ofdm_create(const struct OFDM_CONFIG *config) {
  ...
  ofdm->pilot_samples =
      (cf_complex *)MALLOC(sizeof(cf_complex) * ofdm->samplespersymbol);
  assert(ofdm->pilot_samples != NULL);

  ofdm->rxbuf = (cf_complex *)MALLOC(sizeof(cf_complex) * ofdm->nrxbuf);
  assert(ofdm->rxbuf != NULL);
  for (int i = 0; i < ofdm->nrxbuf; i++) ofdm->rxbuf[i] = 0;

  ofdm->pilots =
      (cf_complex *)MALLOC(sizeof(cf_complex) * (ofdm->nc + 2));
   ...
   */
  ofdm->rx_sym = (cf_complex **)MALLOC(sizeof(cf_complex) * (ofdm->ns + 3));
  ...
  ofdm->rx_np = (cf_complex *)MALLOC(sizeof(cf_complex) * (ofdm->rowsperframe * ofdm->nc));
  assert(ofdm->rx_np != NULL);

  ofdm->rx_amp = (float *)MALLOC(sizeof(float) * (ofdm->rowsperframe * ofdm->nc));
  assert(ofdm->rx_amp != NULL);

  ofdm->aphase_est_pilot_log =
      (float *)MALLOC(sizeof(float) * (ofdm->rowsperframe * ofdm->nc));
   ...
   */
  ofdm->uw_ind = (int *)MALLOC(sizeof(int) * ofdm->nuwbits);
  assert(ofdm->uw_ind != NULL);

  ofdm->uw_ind_sym = (int *)MALLOC(sizeof(int) * (ofdm->nuwbits / ofdm->bps));
  ...
  ofdm->tx_uw_syms =
      (cf_complex *)MALLOC(sizeof(cf_complex) * (ofdm->nuwbits / ofdm->bps));
  ...
  cf_complex *temp = (cf_complex *)MALLOC(sizeof(cf_complex) * ofdm->m);
  assert(temp != NULL);
...
static void allocate_tx_bpf(struct OFDM *ofdm) {
  ofdm->tx_bpf = (struct quisk_cfFilter *)MALLOC(sizeof(struct quisk_cfFilter));
...
static void allocate_rx_bpf(struct OFDM *ofdm) {
  ofdm->rx_bpf = (struct quisk_cfFilter *)MALLOC(sizeof(struct quisk_cfFilter));
 ...
 */
static void idft(struct OFDM *ofdm, cf_complex *result,
                 cf_complex *vector) {
  int row, col;
  ...
  for (row = 1; row < ofdm->m; row++) {
    cf_complex c = cmplx(ofdm->tx_nlower * ofdm->doc * row);
    cf_complex delta = cmplx(ofdm->doc * row);
 ...
 * This algorithm was optimized for speed
 */
static void dft(struct OFDM *ofdm, cf_complex *result,
                cf_complex *vector) {
  int row, col;
...
static cf_complex vector_sum(cf_complex *a, int num_elements) {
  cf_complex sum = 0.0f;
  int i;
...
static cf_complex ofdm_complex_dot_product(cf_complex *left,
                                              cf_complex *right,
                                              int numSamples) {
  cf_complex result = 0;
 ...
 * +/- 25 Hz for 700D).
 */
static int est_timing(struct OFDM *ofdm, cf_complex *rx, int length,
                      int fcoarse, float *timing_mx, int *timing_valid,
                      int step) {
  cf_complex corr_st, corr_en;
  int Ncorr = length - (ofdm->samplesperframe + ofdm->samplespersymbol);
  #ifdef _MSC_VER
  float *corr = (float *)_alloca((Ncorr) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *corr = (float *)_alloca((Ncorr) * sizeof(float));
  #else
  ...
  #ifdef _MSC_VER
  cf_complex *wvec_pilot = (cf_complex *)_alloca((ofdm->samplespersymbol) * sizeof(cf_complex));
  #else
  #ifdef _MSC_VER
  cf_complex *wvec_pilot = (cf_complex *)_alloca((ofdm->samplespersymbol) * sizeof(cf_complex));
  #else
  cf_complex wvec_pilot[ofdm->samplespersymbol];
  #endif
  #endif
  ...
  #ifdef _MSC_VER
  float *rx_real = (float *)_alloca((length) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *rx_real = (float *)_alloca((length) * sizeof(float));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  float *wvec_pilot_real = (float *)_alloca((ofdm->samplespersymbol) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *wvec_pilot_real = (float *)_alloca((ofdm->samplespersymbol) * sizeof(float));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  float *wvec_pilot_imag = (float *)_alloca((ofdm->samplespersymbol) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *wvec_pilot_imag = (float *)_alloca((ofdm->samplespersymbol) * sizeof(float));
  #else
 ...
 */
static float est_freq_offset_pilot_corr(struct OFDM *ofdm, cf_complex *rx,
  ...
  // look for peak
  for (int f = st; f < en; f++) {
    cf_complex corr_st = 0.0f;
    cf_complex corr_en = 0.0f;
    float tmp = TAU * f / ofdm->fs;
    cf_complex delta = cmplxconj(tmp);
    cf_complex w = cmplxconj(0.0f);
    int i;

    for (i = 0; i < ofdm->samplespersymbol; i++) {
      // "mix" down (correlate) the pilot sequences from frame with 0 Hz offset
      // pilot samples
      cf_complex csam = wvec_pilot[i] * w;
 ...
 * ----------------------------------------------
 */
void ofdm_txframe(struct OFDM *ofdm, cf_complex *tx,
                  cf_complex *tx_sym_lin) {
  int _af_rows = ofdm->np * ofdm->ns, _af_cols = ofdm->nc + 2;
#ifdef _MSC_VER
  cf_complex *_aframe_flat = (cf_complex *)_alloca(_af_rows * _af_cols * sizeof(cf_complex));
  memset(_aframe_flat, 0, _af_rows * _af_cols * sizeof(cf_complex));
  #define aframe_at(r,c) _aframe_flat[(r) * _af_cols + (c)]
  cf_complex *asymbol = (cf_complex *)_alloca((ofdm->m) * sizeof(cf_complex));
#else
  cf_complex aframe[ofdm->np * ofdm->ns][ofdm->nc + 2];
  #define aframe_at(r,c) aframe[r][c]
  cf_complex asymbol[ofdm->m];
#endif
  #ifdef _MSC_VER
  cf_complex *asymbol_cp = (cf_complex *)_alloca((ofdm->samplespersymbol) * sizeof(cf_complex));
  #else
  #ifdef _MSC_VER
  cf_complex *asymbol_cp = (cf_complex *)_alloca((ofdm->samplespersymbol) * sizeof(cf_complex));
  #else
  cf_complex asymbol_cp[ofdm->samplespersymbol];
  #endif
  #endif
  int i, j, k, m;
...
#ifdef _MSC_VER
    idft(ofdm, asymbol, &_aframe_flat[i * _af_cols]);
#else
 ...
 * PAPR */
void ofdm_hilbert_clipper(struct OFDM *ofdm, cf_complex *tx, size_t n) {
  ...
  if (ofdm->tx_bpf_en) {
    assert(ofdm->tx_bpf != NULL);
    #ifdef _MSC_VER
    cf_complex *tx_filt = (cf_complex *)_alloca((n) * sizeof(cf_complex));
    #else
    #ifdef _MSC_VER
    cf_complex *tx_filt = (cf_complex *)_alloca((n) * sizeof(cf_complex));
    #else
    cf_complex tx_filt[n];
    #endif
    #endif

    quisk_ccfFilter(tx, tx_filt, n, ofdm->tx_bpf);
    memmove(tx, tx_filt, n * sizeof(cf_complex));
  }
...
void ofdm_mod(struct OFDM *ofdm, COMP *result, const int *tx_bits) {
  ...
  cf_complex *tx =
      (cf_complex *)result;  // complex has same memory layout
  #ifdef _MSC_VER
  cf_complex *tx_sym_lin = (cf_complex *)_alloca((length) * sizeof(cf_complex));
  #else
  #ifdef _MSC_VER
  cf_complex *tx_sym_lin = (cf_complex *)_alloca((length) * sizeof(cf_complex));
  #else
  cf_complex tx_sym_lin[length];
  #endif
  #endif
  int dibit[2];
  int s, i;
...
int ofdm_sync_search(struct OFDM *ofdm, COMP *rxbuf_in) {
  ...
  memmove(&ofdm->rxbuf[0], &ofdm->rxbuf[ofdm->nin],
          (ofdm->nrxbuf - ofdm->nin) * sizeof(cf_complex));
  memmove(&ofdm->rxbuf[(ofdm->nrxbuf - ofdm->nin)], rxbuf_in,
          ofdm->nin * sizeof(cf_complex));
...
static float est_timing_and_freq(struct OFDM *ofdm, int *t_est, float *foff_est,
                                 cf_complex *rx, int Nrx,
                                 cf_complex *known_samples, int Npsam,
  ...
  *foff_est = 0.0;
  for (float afcoarse = fmin; afcoarse <= fmax; afcoarse += fstep) {
    ...
    #ifdef _MSC_VER
    cf_complex *mvec = (cf_complex *)_alloca((Npsam) * sizeof(cf_complex));
    #else
    #ifdef _MSC_VER
    cf_complex *mvec = (cf_complex *)_alloca((Npsam) * sizeof(cf_complex));
    #else
    cf_complex mvec[Npsam];
    #endif
    #endif
    for (int i = 0; i < Npsam; i++) {
      cf_complex ph = cmplx(w * i);
      mvec[i] = conjf(known_samples[i] * ph);
    }
    for (int t = 0; t < Ncorr; t += tstep) {
      cf_complex corr = ofdm_complex_dot_product(&rx[t], mvec, Npsam);
...
static void burst_acquisition_detector(struct OFDM *ofdm, cf_complex *rx,
                                       int n, cf_complex *known_sequence,
...
static int ofdm_sync_search_burst(struct OFDM *ofdm) {
  ...
  burst_acquisition_detector(ofdm, ofdm->rxbuf, st,
                             (cf_complex *)ofdm->tx_preamble, &pre_ct_est,
...
static int ofdm_sync_search_core(struct OFDM *ofdm) {
  if (ofdm->rx_bpf_en) {
    assert(ofdm->rx_bpf != NULL);
    cf_complex *rxbuf_in = &ofdm->rxbuf[(ofdm->nrxbuf - ofdm->nin)];
...
void ofdm_demod(struct OFDM *ofdm, int *rx_bits, COMP *rxbuf_in) {
  cf_complex *rx =
      (cf_complex *)&rxbuf_in[0];  // complex has same memory layout
  int i, j;
...
static void ofdm_demod_core(struct OFDM *ofdm, int *rx_bits) {
  ...
  if (ofdm->timing_en == true) {
    ...
    #ifdef _MSC_VER
    cf_complex *work = (cf_complex *)_alloca(((en - st)) * sizeof(cf_complex));
    #else
    #ifdef _MSC_VER
    cf_complex *work = (cf_complex *)_alloca(((en - st)) * sizeof(cf_complex));
    #else
    cf_complex work[(en - st)];
    #endif
    #endif
  ...
  #ifdef _MSC_VER
  cf_complex *work = (cf_complex *)_alloca((ofdm->m) * sizeof(cf_complex));
  #else
  #ifdef _MSC_VER
  cf_complex *work = (cf_complex *)_alloca((ofdm->m) * sizeof(cf_complex));
  #else
  cf_complex work[ofdm->m];
  #endif
  #endif
  ...
  cf_complex aphase_est_pilot_rect;
  #ifdef _MSC_VER
  float *aphase_est_pilot = (float *)_alloca((ofdm->nc + 2) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *aphase_est_pilot = (float *)_alloca((ofdm->nc + 2) * sizeof(float));
  #else
  ...
  #endif
  #ifdef _MSC_VER
  float *aamp_est_pilot = (float *)_alloca((ofdm->nc + 2) * sizeof(float));
  #else
  #ifdef _MSC_VER
  float *aamp_est_pilot = (float *)_alloca((ofdm->nc + 2) * sizeof(float));
  #else
 ...
 */
float ofdm_esno_est_calc(cf_complex *rx_sym, int nsym) {
  float sig_var = 0;
...
void ofdm_get_demod_stats(struct OFDM *ofdm, struct MODEM_STATS *stats,
                          cf_complex *rx_syms, int Nsymsperpacket) {
  stats->Nc = ofdm->nc;
  ...
  for (int c = 0; c < ofdm->nc; c++) {
    for (int r = 0; r < Nrowsperpacket; r++) {
      cf_complex rot = rx_syms[r * ofdm->nc + c] * cmplx(ROT45);
...
void ofdm_assemble_qpsk_modem_packet_symbols(struct OFDM *ofdm,
                                             cf_complex modem_packet[],
                                             COMP payload_syms[],
                                             uint8_t txt_bits[]) {
  cf_complex *payload =
      (cf_complex *)&payload_syms[0];  // complex has same memory layout
...
void ofdm_disassemble_qpsk_modem_packet(struct OFDM *ofdm,
                                        ...
                                        short txt_bits[]) {
  cf_complex *codeword =
      (cf_complex *)&codeword_syms[0];  // complex has same memory layout
  int Nsymsperpacket = ofdm->bitsperpacket / ofdm->bps;
  int Nuwsyms = ofdm->nuwbits / ofdm->bps;
  int Ntxtsyms = ofdm->ntxtbits / ofdm->bps;
  int dibit[2];
...
void ofdm_disassemble_qpsk_modem_packet_with_text_amps(
    struct OFDM *ofdm, cf_complex rx_syms[], float rx_amps[],
 ...
 */
void ofdm_extract_uw(struct OFDM *ofdm, cf_complex rx_syms[],
...
void ofdm_generate_payload_data_bits(uint8_t payload_data_bits[], int n) {
  #ifdef _MSC_VER
  uint16_t *r = (uint16_t *)_alloca((n) * sizeof(uint16_t));
  #else
  #ifdef _MSC_VER
  uint16_t *r = (uint16_t *)_alloca((n) * sizeof(uint16_t));
  #else
...
void ofdm_generate_preamble(struct OFDM *ofdm, COMP *tx_preamble, int seed) {
  ...
  #ifdef _MSC_VER
  uint16_t *r = (uint16_t *)_alloca((ofdm_preamble.bitsperpacket) * sizeof(uint16_t));
  #else
  #ifdef _MSC_VER
  uint16_t *r = (uint16_t *)_alloca((ofdm_preamble.bitsperpacket) * sizeof(uint16_t));
  #else
  ...
  #ifdef _MSC_VER
  int *preamble_bits = (int *)_alloca((ofdm_preamble.bitsperpacket) * sizeof(int));
  #else
  #ifdef _MSC_VER
  int *preamble_bits = (int *)_alloca((ofdm_preamble.bitsperpacket) * sizeof(int));
  #else
...
// hilbert clipper
void ofdm_clip(cf_complex tx[], float clip_thresh, int n) {
  cf_complex sam;
  float mag;
  int i;
