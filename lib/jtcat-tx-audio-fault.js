...
function describeJtcatTxAudioFault(fault) {
  ...
  if (name === 'NotFoundError') {
    ...
    return 'TX refused: the saved FT8 audio output device is not present (' + reason + ') — ' +
  ...
  return 'TX refused: the saved FT8 audio output device could not be opened (' + reason + '). ' +
    'POTACAT will not key the radio with no audio. ' + FIX +
