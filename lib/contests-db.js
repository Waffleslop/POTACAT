...
function _resolveForYear(rule, year, now) {
  ...
  // nth-weekday-of:<MM>:<n>:<day>[+/-<days>] — an optional day offset from
  // the anchor weekday, for events pinned to a holiday rather than a
  // weekend: Route 66 On The Air starts the Saturday AFTER Labor Day
  // (1st Monday of September), i.e. `nth-weekday-of:9:1:Mon+5`: the 6th
  // through the 12th, while the 2nd Saturday runs 8th–14th, so
  // `nth-weekend-of:9:2` reads a week LATE whenever September opens on a
  // Sunday or Monday (2024, 2025).
  m = rule.match(/^nth-weekday-of:(\d+):(-?\d+):([A-Za-z]+)([+-]\d+)?$/);
  if (m) {
    const anchor = nthWeekdayOf(year, parseInt(m[1], 10), parseInt(m[2], 10), m[3]);
    if (!anchor || !m[4]) return anchor;
    return new Date(anchor.getTime() + parseInt(m[4], 10) * 86400000);
  }
...
// the ECHOCAT participation-history blob. Only meaningful for the
// year-anchored rules (fixed:/range:/nth-weekend-of:/nth-weekday-of:); the
// year-agnostic weekly/monthly patterns resolve relative to `now`, so they
// return null here by design.
const YEAR_BOUND_RULE_RE = /^(fixed|nth-weekend-of|nth-weekday-of|range):/;
function resolveStartForYear(rule, year) {
  if (!YEAR_BOUND_RULE_RE.test(rule || '')) return null;
  return _resolveForYear(rule, year, new Date(0)); // `now` unused by these branches
}
