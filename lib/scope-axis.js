...
(function (root, factory) {
  ...
  /**
   * Snap a tapped frequency to a step that matches what a finger or a mouse
   * can actually resolve at this span — 10 Hz up to 10 kHz, 50 Hz to 50 kHz,
   * 100 Hz to 200 kHz, 500 Hz above. A 360-point phone at a 100 kHz span has
   * ~280 Hz per point; the raw value is false precision. The mobile client
   * wrote this rule first; every client uses it so a tap means the same thing.
   */
  function snapTapHz(hz, spanHz) {
    var s = Number(spanHz) || 0;
    var step = s <= 10000 ? 10 : s <= 50000 ? 50 : s <= 200000 ? 100 : 500;
    return Math.round((Number(hz) || 0) / step) * step;
  }
  ...
  return {
    ...
    scopeAxis: scopeAxis, binToHz: binToHz, hzToBin: hzToBin, downsampleBins: downsampleBins, applyFloor: applyFloor, snapTapHz: snapTapHz,
