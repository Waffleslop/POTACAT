...
class RigController extends EventEmitter {
  ...
  _startPolling() {
    ...
    this._pollTimer = setInterval(() => {
      ...
      if ((this._transmitting || this._model.pollTxMetersAlways) && this._pollCount % 2 === 0) {
        ...
        if (this._codec.getPowerMeter) this._codec.getPowerMeter();
      }
  ...
  _holdTxForCw(text) {
    ...
    this.noteTransmitting(Math.ceil(text.length * perCharMs));
  }
   ...
   * Successive calls extend; a CAT PTT readback still overrides.
   */
  noteTransmitting(ms) {
    const now = Date.now();
    const base = Math.max(now, this._cwHoldUntil || 0);
    this._cwHoldUntil = base + Math.max(0, Number(ms) || 0) + (this._cwHoldTimer ? 0 : 500);
