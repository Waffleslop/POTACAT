...
class RigController extends EventEmitter {
  ...
  constructor(model, transport, codec) {
    ...
    this._transport.on('connect', () => {
      ...
      if (typeof this._codec.setAutoInfo === 'function') this._codec.setAutoInfo(true);
    ...
    this._codec.on('rfgain', (pct) => this.emit('rfgain', pct));
    ...
    this._codec.on('breakInDelay', (val) => this.emit('breakInDelay', val));
    this._codec.on('rit', (on) => this.emit('rit', on));
    this._codec.on('txClar', (on) => this.emit('txClar', on));
    this._codec.on('clarFreq', (val) => this.emit('clarFreq', val));
  ...
  disconnect() {
    if (this.connected && typeof this._codec.setAutoInfo === 'function') this._codec.setAutoInfo(false);
    this._stopPolling();
    this._cwPttRelease();
  ...
  _startPolling() {
    ...
    this._pollTimer = setInterval(() => {
      ...
      if (this._pollCount++ % 5 === 0) {
        ...
        if (caps.nbLevel && this._codec.getNbLevel) this._codec.getNbLevel();
        ...
        if (caps.nrLevel && this._codec.getNoiseReductionLevel) this._codec.getNoiseReductionLevel();
        ...
        if (caps.breakInDelay && this._codec.getBreakInDelay) this._codec.getBreakInDelay();
        if (caps.ftx1Clar && this._codec.getClarState) this._codec.getClarState();
        if (caps.ftx1Clar && this._codec.getClarFreq) this._codec.getClarFreq();
      }
    }, interval);
  }
  ...
  setClarTx(on) {
    if (!this.connected) return;
    if (typeof this._codec.setClarTx === 'function') {
      this._codec.setClarTx(on);
      this._log(`TX clarifier ${on ? 'on' : 'off'}`);
    }
  }

  /** Does this rig support native transmit-offset control? (FTX-1 uses TX CLAR via CF.) */
  get hasNativeXit() {
  ...
  sendCwText(text) {
    ...
    if (cwCaps.text === 'ky1') {
      ...
      const avgDitsPerChar = cwCaps.kyMode === 'km' ? 6 : 10;
      const tailMs = cwCaps.kyMode === 'km' ? 100 : 1000;
      const durationMs = Math.ceil((text.length * avgDitsPerChar * 1200) / wpm) + tailMs;
  ...
  setPreampTarget(target) {
    if (!this.connected) return;
    if (typeof this._codec.setPreampTarget === 'function') this._codec.setPreampTarget(target);
  }
  ...
  setClarFreq(hz) {
    if (!this.connected) return;
    if (typeof this._codec.setClarFreq === 'function') this._codec.setClarFreq(hz);
  }
