...
const SQL_OPEN = 50;             // default open threshold (metric 0-100) —
                                 // raised from 40 after first on-air run
                                 // (K3SBP 2026-07-14) let noise garbage
                                 // through; operator-adjustable (setSquelch)
const SQL_CLOSE_GAP = 15;        // close at open-15 (hysteresis)
...
class PskEngine extends EventEmitter {
  constructor() {
    ...
    this._sqlOpen = SQL_OPEN;
    this._sqlClose = SQL_OPEN - SQL_CLOSE_GAP;
  ...
  _decideSymbol(zi, zq) {
    ...
    // Instant DCD: 16 consecutive ZERO bits = PSK31 idle (reversal every
    // symbol) — a signature only a real PSK transmission produces, so open
    // immediately instead of waiting for the EMA. The all-ONES trigger that
    // shipped first is deliberately GONE: a steady carrier (birdie, tune-up,
    // AM remnant) decodes as endless 1-bits, so any carrier near the center
    // force-opened the squelch permanently and carrier+noise streamed
    // short-code garbage (e t o i n …) for hours. First on-air run,
    // K3SBP 2026-07-14. A plain carrier must NOT open the squelch.
    this._dcdShift = ((this._dcdShift << 1) | bit) >>> 0;
    if ((this._dcdShift & 0xFFFF) === 0) this._quality = 1;

    const metric = 100 * Math.max(0, this._quality);
    if (!this._dcd && metric >= this._sqlOpen) {
      this._dcd = true;
    } else if (this._dcd && metric < this._sqlClose) {
      this._dcd = false;
      this._vcode = 0;
      this._vLastBit = 1;
    }
  ...
  /**
   * Squelch level 10-90 (the popout's SQL slider). Higher = stricter.
   * Close threshold trails by SQL_CLOSE_GAP for hysteresis. Band noise
   * varies too much for one hardcoded threshold — fldigi ships a slider
   * for the same reason.
   */
  setSquelch(level) {
    const v = Math.max(10, Math.min(90, parseInt(level, 10) || SQL_OPEN));
    this._sqlOpen = v;
    this._sqlClose = Math.max(5, v - SQL_CLOSE_GAP);
  }
