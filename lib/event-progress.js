...
function retroStampMatches(ev, rawQsos) {
  ...
  for (let i = 0; i < (rawQsos || []).length; i++) {
    ...
    if (!call) continue;
    const entry = sched.find((s) => qsoDayInScheduleEntry(r.QSO_DATE || '', s));
    if (!entry) continue;
    if (board === 'checklist') {
      const item = matchChecklistItem(ev.tracking && ev.tracking.items, call);
      if (item) out.push({ index: i, item: item.id, itemName: item.name || '' });
    } else if (matchRegionPatterns(ev.callsignPatterns, call)) {
      out.push({ index: i, item: entry.region || '', itemName: entry.regionName || '' });
    }
  }
  return out;
}
