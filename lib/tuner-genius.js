...
class TunerGeniusClient extends EventEmitter {
  ...
  _parseStatus(payload) {
    // Remove leading "state " or "status " if present
    let text = payload;
    if (text.startsWith('state ')) text = text.slice(6);
    else if (text.startsWith('status ')) text = text.slice(7);
    const kv = {};
    ...
    if (kv.antA != null) {
      // antA is zero-indexed: 0=Ant1, 1=Ant2, 2=Ant3
      this.antenna = (parseInt(kv.antA, 10) || 0) + 1;
    }
    if (kv.operate != null) {
