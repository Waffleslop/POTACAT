...
class RemoteServer extends EventEmitter {
  ...
  _startMdns(tlsCert) {
    ...
    for (const { name, address } of targets) {
      try {
        // bonjour-service forwards opts to multicast-dns; the
        // `interface` key binds the underlying UDP socket to the
        // chosen IPv4 address (see node_modules/multicast-dns/
        // index.js line 134 / 153).
        const inst = new Bonjour({ interface: address });
