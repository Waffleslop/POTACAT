...
`;
  ...
  _handleConnection(ws, req) {
    ...
    this._sendTo(ws, protocol.buildServerHello({
      ...
      capabilities: ['diagnostic-snapshot'],
  ...
  sendAllQsos(qsos) {
    ...
    // Legacy single-frame clients: bound the payload. The most-recent N records
    // preserve each record's original `idx`, so edit/delete still reference the
    // correct full-log position; `truncated` lets a capable-enough UI say so.
    if (total > ALL_QSOS_LEGACY_MAX) {
      this._sendTo(ws, { type: 'all-qsos', data: list.slice(total - ALL_QSOS_LEGACY_MAX), total, truncated: true });
    } else {
      this._sendTo(ws, { type: 'all-qsos', data: list });
    }
  }
