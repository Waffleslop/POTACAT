...
const EchocatToken = require('./echocat-token');
...
// cookie helpers + the anonymous connect page. Pure; see the module.
const WebGate = require('./echocat-web-gate');
...
function getOrCreateTlsCert(certDir, opts = {}) {
  ...
  try {
    ...
    // console only reaches a dev terminal. The caller decides what this
    // means (start() refuses plain HTTP behind the Cloud Tunnel) and puts
    // it in the log that bug reports carry — until 2026-09-17 the one line
    // explaining a dead tunnel origin was the one line nobody could see.
...
const PUSH_LOG_EXCLUDED = new Set([
  'tx-meter', 'smeter', 'swr', 'alc', 'power', 'signal', 'jtcat-spectrum', 'scope-frame',
]);
...
class RemoteServer extends EventEmitter {
  ...
  constructor() {
    ...
    this._cwPaddleHoldAware = false; // client repeats a held contact (see _armCwPaddleWatchdog)
    ...
    // Local listener outcome — see serverState(). The tunnel forwards to
    // this port, so main.js needs to know when it is NOT bound or NOT TLS.
    this._bindFailed = false;
    this._tlsFailed = false;
    ...
    // _passGateSessions: Guest Pass sessions a browser signed in with
    // through POST /api/pass-gate (sessionId -> {code, expiresAt}); the
    // HTTP gate and the WS cookie branch both consult it. A miss after a
    // desktop restart is re-validated with the cloud (_revalidatePassCookie)
    // and negative answers are remembered briefly (_passGateDenied) so a
    // stale cookie cannot turn every page load into a cloud round trip.
    // _tunnelPostLimiter throttles the tunnel-open POSTs by visitor IP —
    // the cloud only ever sees the DESKTOP's IP on validate-session, so
    // this is the one place a pass-code scanner can be slowed down.
    this._passGateSessions = new Map();
    this._passGateDenied = new Map();
    this._tunnelPostLimiter = new WebGate.FailureRateLimiter();
  }
  ...
  setTunnelExposed(exposed) {
    ...
    if (next) {
      ...
      if (this.running && !this._https) {
        // The tunnel was switched on over a plain-HTTP fallback: every
        // remote request will 502. Keep serving the LAN, but say so and
        // let the origin check turn the tunnel card amber.
        this._lastServerError = 'The Cloud Tunnel is on, but this server is running WITHOUT TLS (the certificate could not be created at start). Remote connections will fail with a 502 until POTACAT is restarted with a working certificate.';
        this.emit('log', '[remote] WARN: ' + this._lastServerError);
  ...
  start(port, token, opts = {}) {
    ...
    const handler = (req, res) => {
      this._handleHttpRequest(req, res).catch((err) => {
        const msg = `HTTP handler failed for ${req.method} ${req.url}: ${err && err.message ? err.message : err}`;
    ...
    this._lastServerError = '';
    if (!tlsCert) {
      // No certificate. Behind the Cloud Tunnel that is fatal, not a
      // fallback: the ingress speaks HTTPS to this port, so a plain-HTTP
      // listener answers every remote request with a 502 while the app
      // shows "Live" (K5AWJ 2026-09-17). Refuse to bind and say why. On a
      // LAN-only station plain HTTP still lets a browser in (without the
      // microphone), so it is allowed there — but no longer silently.
      const why = tlsError && tlsError.message ? tlsError.message : 'unknown error';
      this._tlsFailed = true;
      if (this._tunnelExposed) {
        this._lastServerError = `ECHOCAT server NOT started: could not create its TLS certificate (${why}). The Cloud Tunnel requires HTTPS, so serving plain HTTP would only turn every remote connection into a 502. Restart POTACAT; if this repeats, send a bug report.`;
        console.error('[Echo CAT]', this._lastServerError);
        this.emit('log', this._lastServerError);
        this._emitServerState();
        return;
      }
      this._lastServerError = `Could not create the TLS certificate (${why}) — serving plain HTTP. Phone microphone audio will NOT work, and the Cloud Tunnel cannot be used until this is fixed. Restart POTACAT; if this repeats, send a bug report.`;
      console.warn('[Echo CAT]', this._lastServerError);
      this.emit('log', this._lastServerError);
    }
    ...
    // listener there, a busy port threw an unhandled 'error' — so the
    // careful EADDRINUSE retry below never got to run: the app died with a
    // fatal-error dialog instead. The HTTP server's own handler owns it.
    this._wss.on('error', () => {});
    ...
    // After the fast attempts it does NOT give up: a stale process usually
    // exits a minute or two later, and a station that lost that race used
    // to stay unreachable until someone restarted the app — with the tunnel
    // reporting healthy the whole time. Slow retries continue until the
    // port binds or stop() is called; 'server-state' carries the outcome
    // to main so the UI can show it instead of a green pill.
    const retry = Object.assign({ fastAttempts: 5, fastDelayMs: 800, slowDelayMs: 30000 }, opts.bindRetry || {});
    let _attempts = 0;
    const server = this._httpServer;
    // ONE 'listening' handler for the life of the server object. Passing a
    // callback to each listen() call registers another once-listener per
    // attempt, and every one of them fires on the attempt that succeeds.
    server.on('listening', () => {
      if (this._httpServer !== server) return;
      this.running = true;
      this._bindFailed = false;
      this._bindRetryTimer = null;
    ...
    const _tryListen = () => {
      if (this._httpServer !== server) return; // stop() ran
      _attempts++;
    ...
    server.on('error', (err) => {
      ...
      if (busy && _attempts < retry.fastAttempts) {
        const msg = `Port ${this._port} busy (attempt ${_attempts}/${retry.fastAttempts}); retrying in ${retry.fastDelayMs}ms…`;
        console.warn(`[Echo CAT] ${msg}`);
        this.emit('log', msg);
        // Node's http.Server doesn't reuse a server that errored on
        // listen; close it and listen again on the same object.
        this._bindRetryTimer = setTimeout(() => { try { server.close(); } catch {} _tryListen(); }, retry.fastDelayMs);
        return;
      }
      // Never re-emit as 'error' — no listener on the main side, and
      // EventEmitter would throw uncaughtException and kill POTACAT.
      const failMsg = busy
        ? `ECHOCAT server could not bind to port ${this._port} after ${_attempts} attempts. Another process is holding it — usually a POTACAT left over from before a restart. Close it or restart the computer; POTACAT keeps retrying every ${Math.round(retry.slowDelayMs / 1000)} s.`
        : `Server error: ${err && err.message ? err.message : err}`;
      const firstFailure = !this._bindFailed;
      this._bindFailed = true;
      this._lastServerError = failMsg;
      // The first failure is loud; the slow retries report every tenth
      // attempt (5 min at the default) so the log stays readable while a
      // long outage still leaves a trail.
      if (firstFailure || (_attempts - retry.fastAttempts) % 10 === 0) {
        console.error('[Echo CAT]', failMsg);
        this.emit('log', failMsg);
      }
      if (firstFailure) this._emitServerState();
      if (busy) {
        this._bindRetryTimer = setTimeout(() => { try { server.close(); } catch {} _tryListen(); }, retry.slowDelayMs);
      }
    });
    _tryListen();
  ...
  /**
   * What the local listener is doing right now — the origin the Cloud
   * Tunnel forwards to. Consumed by main.js for the tunnel origin check,
   * the Settings card and the bug report.
   */
  serverState() {
    return {
      listening: !!this.running,
      https: this.running ? !!this._https : (this._tlsFailed ? false : null),
      port: this._port || null,
      bindFailed: !!this._bindFailed,
      tlsFailed: !!this._tlsFailed,
      lastError: this._lastServerError || '',
    };
  }

  _emitServerState() {
    try { this.emit('server-state', this.serverState()); } catch {}
  }
  ...
  async _handleHttpRequest(req, res) {
    ...
    //     appeared.
    //   - /api/pass-gate, /api/web-signout (POST): ECHOCAT Web sign-in
    //     for Guest Pass holders / sign-out. Origin-checked, JSON-only,
    //     rate-limited; each enforces its own credential (2026-09-11).
    //
    // ECHOCAT Web over POTACAT Cloud (2026-09-11): the whitelist and the
    // stub are now the pure decideHttpGate in lib/echocat-web-gate.js,
    // and two things changed for a visitor over the tunnel. (1) An
    // anonymous GET / gets a static CONNECT PAGE instead of the 503 —
    // still a constant string with no template inputs, so e696ad5's
    // leak rule holds; every other anonymous path stays the stub.
    // (2) A browser carrying a live __Host- cookie minted by
    // /api/pair-account {web:true} (owner) or /api/pass-gate (guest)
    // is served the SPA with __authMode="cookie", and ONLY the SPA
    // paths — /api/ptt/* stays unreachable from the internet even with
    // a valid cookie. The WS layer makes the same decision from the
    // same cookie (wsAuthModeFor in _handleConnection).
    //
    // The stub / connect page are for PUBLIC visitors only (people who
    // reached <callsign>.potacat.com over the Cloud Tunnel). LAN and
    // Tailscale users must get the real ECHOCAT web UI even with the
    // tunnel on — that's the free, no-app, no-subscription path. Gating
    // on _tunnelExposed alone stubbed everyone (regression 2026-06-13);
    // now we only gate requests that actually came via the tunnel / a
    // public source. See _isTunnelOrPublicRequest.
    const fromTunnel = RemoteServer._isTunnelOrPublicRequest(
      req.headers, req.socket && req.socket.remoteAddress);
    const gate = await this._decideWebGate(req, pathname, fromTunnel);
    if (gate.action === 'stub') {
      res.writeHead(503, {
    ...
    if (gate.action === 'connect-page') {
      ...
      // A stale/revoked cookie is deleted here so the browser stops
      // presenting it (and stops costing a lookup per request).
      if (gate.clearCookies.length) headers['Set-Cookie'] = gate.clearCookies.map((n) => WebGate.buildClearCookie(n));
      res.writeHead(200, headers);
      res.end(WebGate.buildConnectPageHtml());
      return;
    }
    // 'open' (tunnel-open path) and 'serve' fall through; gate.authMode
    // is the __authMode the SPA must be built with.
    ...
    if (pathname.startsWith('/api/ptt/')) {
      ...
      if (this._requireToken && this._token) {
        ...
        if (!EchocatToken.sameToken(qToken, this._token)) {
    ...
    // ECHOCAT Web: a Guest Pass holder's browser arrives from
    // guest-pass.html / login.potacat.com with #pass=<code>.<sessionId>
    // in the fragment and posts it here. The credential is validated by
    // the SAME cloud validator the WS pass branch uses (main.js
    // setPassValidator -> validate-session) and then stored in an
    // HttpOnly cookie, so the SPA and its WebSocket present it without
    // the page ever holding it. Browser-only: JSON body + the Origin
    // must be us, so a cross-site page cannot plant a pass in a
    // victim's browser (login CSRF).
    if (pathname === '/api/pass-gate' && req.method === 'POST') {
      if (!this._tunnelPostAllowed(req, res, fromTunnel)) return;
      if (!this._browserOriginOk(req)) {
        res.writeHead(403, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
        res.end(JSON.stringify({ error: 'origin' }));
        return;
      }
      let body = '';
      req.on('data', chunk => { body += chunk; if (body.length > 4096) req.destroy(); });
      req.on('end', async () => {
        const fromIp = this._visitorIp(req, fromTunnel);
        ...
        const parsed = WebGate.parsePassCookie(
          String(payload.passCode || '').trim().toLowerCase() + '.' + String(payload.sessionId || '').trim().toLowerCase());
        if (!parsed) return json(400, { error: 'pass_invalid' });
        const masked = parsed.code.slice(0, 4) + '****';
        if (!this._passValidator) {
          this.emit('log', `[Web] Guest Pass sign-in REFUSED from ${fromIp}: no pass validator (cloud not wired)`);
          return json(503, { error: 'owner_station_offline' });
        }
        let profile = null;
        try { profile = await this._passValidator(parsed.code, parsed.sessionId); }
        catch (err) {
          this.emit('log', `[Web] Guest Pass sign-in ERROR code=${masked} from ${fromIp}: ${err.message || err}`);
          return json(503, { error: 'pass_validate_failed' });
        }
        if (!profile || !profile.code) {
          this.emit('log', `[Web] Guest Pass sign-in DENIED code=${masked} from ${fromIp} (not found / expired / revoked / session mismatch)`);
          return json(404, { error: 'pass_invalid' });
        }
        const expMs = profile.expires_at ? Date.parse(profile.expires_at) : NaN;
        let maxAgeSec = WebGate.PASS_COOKIE_MAX_AGE_SEC;
        if (Number.isFinite(expMs)) maxAgeSec = Math.min(maxAgeSec, Math.floor((expMs - Date.now()) / 1000));
        if (maxAgeSec <= 0) return json(410, { error: 'pass_invalid' });
        this._rememberPassGateSession(parsed.sessionId, profile);
        this._passGateDenied.delete(parsed.sessionId);
        json(200, { ok: true, ownerCallsign: profile.owner_callsign || null, expiresAt: profile.expires_at || null }, {
          'Set-Cookie': WebGate.buildSetCookie(WebGate.COOKIE_PASS, parsed.code + '.' + parsed.sessionId, { maxAgeSec }),
        });
        this.emit('log', `[Web] Guest Pass signed in: code=${masked} owner=${profile.owner_callsign || '?'} from ${fromIp} (browser)`);
      });
      return;
    }

    // ECHOCAT Web sign-out. Clears both cookies and, for an owner
    // session, revokes the browser's paired-device row too — a cookie
    // value the operator chose to end must not keep working if it was
    // copied somewhere. Exists so the web client can always recover
    // from a cookie/WS disagreement by signing out and reloading
    // (renderer/remote.js webSignOutAndReload) instead of reload-looping.
    if (pathname === '/api/web-signout' && req.method === 'POST') {
      if (!this._browserOriginOk(req)) {
        res.writeHead(403, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
        res.end(JSON.stringify({ error: 'origin' }));
        return;
      ...
      req.on('data', () => {});
      req.on('end', () => {
        const cookies = WebGate.parseCookieHeader(req.headers.cookie);
        const pass = WebGate.parsePassCookie(cookies[WebGate.COOKIE_PASS]);
        if (pass) this._passGateSessions.delete(pass.sessionId);
        const device = cookies[WebGate.COOKIE_DEVICE] ? this._findDeviceByToken(cookies[WebGate.COOKIE_DEVICE]) : null;
        if (device && device.platform === 'web') {
          this.revokeDevice(device.id);
          this.emit('log', `[Web] Browser signed out: ${device.name} (${device.id}) from ${this._visitorIp(req, fromTunnel)}`);
        }
        res.writeHead(200, {
          'Content-Type': 'application/json',
          'Cache-Control': 'no-store',
          'Set-Cookie': [WebGate.buildClearCookie(WebGate.COOKIE_DEVICE), WebGate.buildClearCookie(WebGate.COOKIE_PASS)],
        });
    ...
    if (pathname === '/api/pair-account' && req.method === 'POST') {
      ...
      req.on('end', async () => {
        ...
        // ECHOCAT Web variant: the browser's connect page posts
        // {pairToken, web:true} after login.potacat.com bounced it
        // back with #pair=. The device token goes into an HttpOnly
        // cookie and is OMITTED from the body — the page never holds
        // it. Browser-only, so the Origin must be us.
        const web = payload.web === true;
        if (web && !this._browserOriginOk(req)) {
          res.writeHead(403, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
          res.end(JSON.stringify({ error: 'origin' }));
          return;
        }
        ...
        const device = this.mintPairedDevice({
          deviceName: web ? WebGate.browserDeviceName(req.headers['user-agent']) : (payload.deviceName || 'POTACAT Desktop'),
          devicePlatform: web ? 'web' : (payload.devicePlatform || 'desktop'),
          accountLinked: true,
          expiresAt: null,
        });
        let fingerprint = '';
        ...
        if (web) {
          reply.ok = true;
          headers['Set-Cookie'] = WebGate.buildSetCookie(WebGate.COOKIE_DEVICE, device.token, { maxAgeSec: WebGate.DEVICE_COOKIE_MAX_AGE_SEC });
        } else {
          reply.deviceToken = device.token;
        }
        res.writeHead(200, headers);
        res.end(JSON.stringify(reply));
        this.emit('log', `[Pair-Account] OK ${device.name} (${device.id}) from ${fromIp} via cloud attestation${web ? ' platform=web' : ''}`);
      });
      return;
    }
    ...
    if (pathname === '/' || pathname === '/remote.html') {
      try {
        ...
        this._cachedInlinedHtml = this._buildInlinedHtml(fromTunnel, gate.authMode);
        res.writeHead(200, {
...
`;
  ...
  _buildInlinedHtml(fromTunnel = false, authModeOverride = null) {
    ...
    try {
      ...
      // Band-scope axis maths — one copy for main, the desktop pop-out and this page.
      const scopeAxisJs = fs.readFileSync(path.join(this._basePath, '..', 'lib', 'scope-axis.js'), 'utf8');
      html = html.replace('<!-- scope-axis-js -->', () => '<script>' + String.fromCharCode(10) + scopeAxisJs + String.fromCharCode(10) + '</script>');
    } catch (err) {
    ...
    //
    // ECHOCAT Web (2026-09-11): the HTTP gate (decideHttpGate) passes
    // its authMode in as authModeOverride — 'cookie' for a browser that
    // signed in through POTACAT Cloud — and the WS side derives ITS
    // answer from the same cookie via wsAuthModeFor. Both come from
    // lib/echocat-web-gate.js, whose tests assert they agree for every
    // combination, which is the guard against the 2026-06-13 drift.
    const requiresAuth = this._requireToken || (this._tunnelExposed && fromTunnel);
    const authMode = authModeOverride || (requiresAuth ? 'token' : 'none');
  ...
  // --- ECHOCAT Web over POTACAT Cloud (HTTP half) ---

  /**
   * The HTTP half of the web gate. The decision is the pure decideHttpGate
   * (lib/echocat-web-gate.js); this wrapper supplies the lookups and does
   * the one async piece: a pass cookie whose session is not in memory (the
   * desktop restarted since the guest signed in) is re-validated with the
   * cloud through the same validator the WS pass branch uses, so a live
   * pass survives a restart without the guest re-opening their link.
   */
  async _decideWebGate(req, pathname, fromTunnel) {
    const gated = !!(this._tunnelExposed && fromTunnel);
    const cookies = gated ? WebGate.parseCookieHeader(req.headers.cookie) : {};
    if (gated && cookies[WebGate.COOKIE_PASS] && !cookies[WebGate.COOKIE_DEVICE]) {
      await this._revalidatePassCookie(cookies[WebGate.COOKIE_PASS], req);
    }
    return WebGate.decideHttpGate({
      tunnelExposed: this._tunnelExposed,
      fromTunnel,
      requireToken: this._requireToken,
      method: req.method,
      pathname,
      cookies,
      findDevice: (t) => this._findDeviceByToken(t),
      findPassSession: (sid) => this._passGateSessions.get(sid) || null,
    });
  }

  async _revalidatePassCookie(value, req) {
    const parsed = WebGate.parsePassCookie(value);
    if (!parsed || !this._passValidator) return;
    const known = this._passGateSessions.get(parsed.sessionId);
    if (known && known.code === parsed.code) return;
    const now = Date.now();
    if ((this._passGateDenied.get(parsed.sessionId) || 0) > now) return;
    // Counts against the same per-visitor budget as the POSTs: a stale
    // cookie replayed by a scanner is a failed credential like any other.
    const key = WebGate.FailureRateLimiter.keyFor(req.headers, req.socket && req.socket.remoteAddress);
    if (!this._tunnelPostLimiter.check(key, now).allowed) return;
    let profile = null;
    try { profile = await this._passValidator(parsed.code, parsed.sessionId); } catch { profile = null; }
    if (profile && profile.code && String(profile.code).toLowerCase() === parsed.code) {
      this._rememberPassGateSession(parsed.sessionId, profile);
      this._tunnelPostLimiter.record(key, 200, now);
      this.emit('log', `[Web] Guest Pass cookie re-validated: code=${parsed.code.slice(0, 4)}**** (desktop restarted since sign-in)`);
    } else {
      this._passGateDenied.set(parsed.sessionId, now + 60_000);
      if (this._passGateDenied.size > 1000) this._passGateDenied.delete(this._passGateDenied.keys().next().value);
      this._tunnelPostLimiter.record(key, 404, now);
    }
  }

  _rememberPassGateSession(sessionId, profile) {
    const exp = profile && profile.expires_at ? Date.parse(profile.expires_at) : NaN;
    this._passGateSessions.set(sessionId, {
      code: String(profile.code).toLowerCase(),
      expiresAt: Number.isFinite(exp) ? exp : null,
    });
    // One entry per guest sign-in; 200 is generous for a single station.
    if (this._passGateSessions.size > 200) this._passGateSessions.delete(this._passGateSessions.keys().next().value);
  }

  /** Browser-only endpoints and cookie-authenticated upgrades: the Origin
   *  must be present AND be us. Browsers always send Origin on a fetch
   *  POST and on a WebSocket upgrade; a cookie without one is not a
   *  browser we served. See originAllowed for the host rules. */
  _browserOriginOk(req) {
    const origin = req.headers.origin;
    if (!origin) return false;
    return WebGate.originAllowed(origin, req.headers.host, this._altHosts.cloudHost);
  }

  /** The visitor's address for logs: cf-connecting-ip over the tunnel
   *  (the socket peer is cloudflared on loopback), else the socket. */
  _visitorIp(req, fromTunnel) {
    if (fromTunnel && req.headers['cf-connecting-ip']) return String(req.headers['cf-connecting-ip']).split(',')[0].trim();
    return (req.socket && req.socket.remoteAddress) || 'unknown';
  }

  /** Tunnel-sourced POST throttle (FailureRateLimiter). Answers 429 and
   *  returns false when the visitor is cooling down; otherwise arms a
   *  record of the eventual status so failures escalate. LAN callers
   *  are never throttled. */
  _tunnelPostAllowed(req, res, fromTunnel) {
    if (!fromTunnel) return true;
    const key = WebGate.FailureRateLimiter.keyFor(req.headers, req.socket && req.socket.remoteAddress);
    const verdict = this._tunnelPostLimiter.check(key);
    if (!verdict.allowed) {
      res.writeHead(429, {
        'Content-Type': 'application/json',
        'Cache-Control': 'no-store',
    ...
    res.once('finish', () => { try { this._tunnelPostLimiter.record(key, res.statusCode); } catch {} });
    return true;
  }
  ...
  _handleConnection(ws, req) {
    ...
    this._sendTo(ws, protocol.buildServerHello({
      ...
      capabilities: ['diagnostic-snapshot', 'spot-target', 'rx-gain-sync', 'tx-gain-sync', 'tx-drive', 'js8', 'activity', 'scope'],
    ...
    //
    // ECHOCAT Web over POTACAT Cloud (2026-09-11): a browser that
    // signed in through login.potacat.com carries an HttpOnly __Host-
    // cookie on the upgrade (an owner device token from /api/pair-account
    // {web:true}, or a Guest Pass session from /api/pass-gate). The page
    // never held the credential, so it cannot send an 'auth' message —
    // the cookie IS the auth, and this is where it is honoured. The
    // decision is wsAuthModeFor (lib/echocat-web-gate.js), the sibling
    // of the HTTP decideHttpGate that served the SPA with
    // __authMode="cookie"; the tests assert the two agree for every row.
    const fromTunnel = RemoteServer._isTunnelOrPublicRequest(
      req.headers, req.socket && req.socket.remoteAddress);
    const webAuth = this._webCookieAuthFor(req, fromTunnel);
    const authMode = WebGate.wsAuthModeFor({
      requireToken: this._requireToken,
      tunnelExposed: this._tunnelExposed,
      fromTunnel,
      cookieAuth: !!webAuth,
    });
    const requiresAuth = authMode !== 'none';
    ...
    } else if (webAuth && webAuth.device) {
      // Owner browser: the same per-device path the 'auth' message
      // takes, so revokeDevice/listPairedDevices/expiry all apply.
      this._authenticateWsWithDevice(ws, req, webAuth.device, { via: 'cookie' });
    } else if (webAuth && webAuth.pass) {
      // Guest browser: _authenticatePass re-validates with the cloud and
      // does its own auth-ok + pass hydration (NOT _hydrateClient).
      this._authenticatePass(ws, { passCode: webAuth.pass.code, sessionId: webAuth.pass.sessionId }, req);
    }
  ...
  _handleMessage(ws, msg, req) {
    ...
    if (msg && msg.type === 'hello') {
      ...
      this.emit('client-hello', { platform: ws._clientPlatform });
      // A LAN connection with no token, or a cloud-cookie browser, is
      // authenticated and hydrated BEFORE its hello is read — so the worked
      // history went out on the legacy one-frame path, hit the 256 KB cap and
      // was skipped, and the chunked copy this client can take never
      // followed: check marks "for today only", for good (LZ3AW round 6).
      // Now that we know it can take chunks, send what it missed.
      if (ws._authenticated && ws._workedQsosSkippedForSize
          && ws._clientCapabilities.includes('chunked-worked-qsos')) {
        ws._workedQsosSkippedForSize = false;
        ws._workedQsosSent = false;
    ...
    if (msg.type === 'auth') {
      ...
      if (msg.token && this._token && EchocatToken.sameToken(msg.token, this._token)) {
        // Token mode (legacy single shared token)
        authenticated = true;
      } else if (msg.token) {
        // Per-device token from a paired mobile app. Match against the
        // long-lived token minted during /api/pair. The expiry / sliding
        // TTL / lastSeen / auth-ok sequence is _authenticateWsWithDevice,
        // shared with the ECHOCAT Web cookie branch in _handleConnection.
        const device = this._findDeviceByToken(msg.token);
        if (device) {
          this._authenticateWsWithDevice(ws, req, device, { via: 'token' });
          return;
        }
    ...
    switch (msg.type) {
      ...
      case 'typed-call':
        // The callsign a web field holds (CW-macro {call}); main merges it
        // with the desktop's fields and broadcasts the winner.
        this.emit('typed-call', { call: String(msg.call || ''), source: String(msg.source || '') });
        break;
      ...
      case 'scope-subscribe':
        // Band scope (FT-710 over USB): the shack runs the helper only while
        // somebody is looking — this window or the desktop pop-out.
        // `guest` rides along so main can show a Guest Pass the right card
        // from the first frame (a guest may watch, never change the radio).
        this.emit('scope-subscribe', { on: !!msg.on, guest: !!ws._passSession });
        break;
      case 'scope-enable-radio':
        // Writes a menu item on the HOST's radio (EX0301261;). A Guest Pass
        // may watch the scope, not change the radio — same line set-tx-drive
        // draws. main answers the guest with a diag that says so.
        this.emit('scope-enable-radio', { guest: !!ws._passSession });
        break;
      case 'jtcat-rx-gain':
      ...
      case 'paddle':
        ...
        // A client that repeats a held contact (`hold: true`, every ~400 ms)
        // is proving the contact is still down, which is what keeps the
        // watchdog below honest — see _armCwPaddleWatchdog.
        if (msg.hold) this._cwPaddleHoldAware = true;
        if (msg.contact === 'dit') {
          this._cwKeyer.paddleDit(!!msg.state);
        } else if (msg.contact === 'dah') {
          this._cwKeyer.paddleDah(!!msg.state);
        }
        this._armCwPaddleWatchdog();
        break;
  ...
  /**
   * ECHOCAT Web: what the cookies on a WS upgrade prove, or null. Only
   * consulted for gated (tunnel-exposed + tunnel-sourced) connections, so
   * LAN/Tailscale behaviour is byte-identical. A cookie without a
   * matching Origin is refused — that is the cross-site WebSocket case
   * (evil.example opens wss://k3sbp.potacat.com and the browser attaches
   * the cookie); native clients carry no cookie so never hit this.
   */
  _webCookieAuthFor(req, fromTunnel) {
    if (!(this._tunnelExposed && fromTunnel)) return null;
    const cookies = WebGate.parseCookieHeader(req.headers.cookie);
    const devTok = cookies[WebGate.COOKIE_DEVICE];
    const passVal = cookies[WebGate.COOKIE_PASS];
    if (!devTok && !passVal) return null;
    if (!this._browserOriginOk(req)) {
      this.emit('log', `[Web] cookie auth refused: origin=${req.headers.origin || '(none)'} host=${req.headers.host || '?'}`);
      return null;
    }
    if (devTok) {
      const device = this._findDeviceByToken(devTok);
      if (device && WebGate.deviceIsLive(device)) return { device };
    }
    if (passVal) {
      const parsed = WebGate.parsePassCookie(passVal);
      const known = parsed && this._passGateSessions.get(parsed.sessionId);
      if (parsed && known && known.code === parsed.code
          && (known.expiresAt == null || known.expiresAt > Date.now())) {
        return { pass: parsed };
      }
    }
    return null;
  ...
  /**
   * Per-device authentication (the 'auth' message's token branch and the
   * ECHOCAT Web cookie branch share this). Expiry check, sliding TTL for
   * guest-tier rows, lastSeen stamp, then the common auth-ok/hydrate tail.
   * Returns false when the token has expired (auth-fail already sent).
   */
  _authenticateWsWithDevice(ws, req, device, opts = {}) {
    ...
    this.emit('paired-devices-changed', this.listPairedDevices());
    this._finishWsAuth(ws, req);
    console.log(`[Echo CAT] Client authenticated via ${opts.via || 'token'}`);
    return true;
  }

  /** The tail every successful non-pass auth ends with. */
  _finishWsAuth(ws, req) {
    ws._authenticated = true;
    this._displaceCurrentClient(ws, req);
    this._client = ws;
  ...
  async _authenticatePass(ws, msg, req) {
    ...
    try {
      ...
      // The session id is HALF the guest's credential (with the code it is
      // the __Host-echocat_pass cookie value and the WS pass auth), so the
      // log keeps a prefix — enough to match a cloud row, not enough to
      // ride the pass from a pasted bug report.
      const sidTag = msg.sessionId ? String(msg.sessionId).slice(0, 8) + '…' : 'n/a';
      this.emit('log', `Guest Pass authenticated: code=${profile.code} owner=${profile.owner_callsign} guest-session=${sidTag} class=${profile.privilege_class} maxW=${profile.max_power_w} from=${addr}`);
  ...
  // its own pairing).
  broadcastPassEnded(reason, code = null) {
    let n = 0;
    const endedCodes = new Set();
    if (code) endedCodes.add(String(code).toLowerCase());
    for (const client of this._wss.clients) {
      if (client._passSession && client.readyState === WebSocket.OPEN) {
        ...
        if (client._passSession.code) endedCodes.add(String(client._passSession.code).toLowerCase());
        n++;
      }
    }
    // ECHOCAT Web: forget the browser sign-ins for the ended pass so a
    // reload lands on the connect page ("Your Guest Pass has ended")
    // instead of a dead SPA. With no code known at all, clear the lot —
    // the WS re-validates with the cloud anyway, so this only costs a
    // live guest one re-validation on reconnect.
    let purged = 0;
    for (const [sid, sess] of this._passGateSessions) {
      if (!endedCodes.size || endedCodes.has(sess.code)) { this._passGateSessions.delete(sid); purged++; }
    }
    if (n > 0 || purged > 0) this.emit('log', `Broadcast pass-ended reason=${reason} clients=${n} webSessions=${purged}`);
  }
  ...
  _onClientDisconnected() {
    ...
    // "Repeats a held contact" belongs to THAT client. Left set, the next
    // client that does not (the native app, an older page) would get the
    // hold-aware watchdog rules without the holds that feed them.
    this._cwPaddleHoldAware = false;
    if (this._cwKeyer) {
      this._cwKeyer.stop();
    }
  ...
  /** The callsign being worked (CW-macro {call}); cached so a connect hydrates it. */
  sendTypedCall(call) {
    this._typedCall = String(call || '');
    this.sendToClient({ type: 'typed-call', call: this._typedCall });
  }
  ...
  _hydrateClient(ws) {
    ...
    if (this._typedCall) this._sendTo(ws, { type: 'typed-call', call: this._typedCall });
  ...
  _sendWorkedQsosCapped(ws) {
    ...
    ws._workedQsosSent = true;
    if (cached.oversized) {
      ws._workedQsosSkippedForSize = true; // a later hello with chunked-worked-qsos re-sends
  ...
  setRemoteSettings(obj) {
    ...
    // The paddle keyer here and the browser's local sidetone keyer must
    // agree on mode and paddle swap. The browser reads both from these
    // settings; this keyer only ever heard the mode when the page sent
    // cw-config (a button click) and never heard swap, so with swap on — or
    // iambic A set on the desktop — the sidetone played different elements
    // from what the radio sent (LZ3AW round 6: "local side tone doesn't
    // work correctly").
    if (obj && ['iambicA', 'iambicB', 'straight'].includes(obj.cwKeyerMode)) {
      this._cwMode = obj.cwKeyerMode;
      if (this._cwKeyer) this._cwKeyer.setMode(this._cwMode);
    }
    if (obj && 'cwSwapPaddles' in obj) {
      this._cwSwapPaddles = !!obj.cwSwapPaddles;
      if (this._cwKeyer) this._cwKeyer.setSwapPaddles(this._cwSwapPaddles);
    }
    this._cachedInlinedHtml = null;
  ...
  /** One band-scope frame: peak-held bins for a small screen (~10 fps). */
  broadcastScopeFrame(payload) {
    if (!payload || !payload.bins) return;
    if (this._client && this._client.readyState === WebSocket.OPEN) {
      this._sendTo(this._client, { type: 'scope-frame', bins: payload.bins, seq: payload.seq, kind: payload.kind });
    }
  }

  /** Band-scope axis + helper status; sent on change and on subscribe. */
  broadcastScopeState(state) {
    if (!state) return;
    if (this._client && this._client.readyState === WebSocket.OPEN) {
      this._sendTo(this._client, { type: 'scope-state', ...state });
    }
  }
  ...
  broadcastJtcatTxGainState(state) {
    // Same arrangement for TX power (main.js applyJtcatTxGain, 2026-09-09).
    if (this.hasClient()) this._sendTo(this._client, { type: 'jtcat-tx-gain-state', value: state.value });
  }
  ...
  /** FTx mode hop QSY'd the rig — clients follow the mode + dial. */
  broadcastJtcatModeHopped(payload) {
    if (this.hasClient()) this._sendTo(this._client, { type: 'jtcat-mode-hopped', ...payload });
  }
  ...
  _initCwKeyer() {
    ...
    this._cwKeyer.setSwapPaddles(!!this._cwSwapPaddles);
    ...
    this._cwKeyer.on('key', (evt) => {
      ...
      // Keying proves the contact is still held. Clients that send hold
      // keepalives re-arm the watchdog themselves and keep its strict
      // window; everyone else is kept on the air from here (see
      // _armCwPaddleWatchdog).
      if (this._cwPaddleWatchdog && !this._cwPaddleHoldAware) this._armCwPaddleWatchdog();
  ...
  /**
   * Force a paddle release if the client's key-up never arrives (an Android
   * Bluetooth MIDI note-off, or a browser tab killed mid-press).
   *
   * It fires 1.5 s after the last paddle message — and until 2026-09-19 that
   * was the ONLY thing that re-armed it. A held paddle sends exactly one
   * message: press. The keyer then generates elements on its own, so a
   * squeeze, a long dah, or a steady string of dits went quiet a second and a
   * half in, mid-character, and only a fresh press revived it — LZ3AW's
   * TinyMidi paddle "stops transmitting after a few seconds", which survived
   * the 1.10.18 fix because that one addressed the link watchdog instead.
   *
   * A client that repeats a held contact (hold keepalive) re-arms this the
   * honest way and keeps the strict 1.5 s guarantee. A client that does not
   * (the native app, an older build) re-arms it from the keyer's own key
   * events instead: the keyer only emits while a contact is genuinely held,
   * so keying survives, and a lost key-up is still caught — by the keyer's
   * own 10 s maximum-streak stop (lib/keyer.js), after which no more key
   * events arrive and this fires 1.5 s later.
   */
  _armCwPaddleWatchdog() {
    ...
    this._cwPaddleWatchdog = setTimeout(() => {
      ...
      // Only fire a redundant key-up at the radio if we actually emitted a
      // key-down without a matching key-up. stop() above will emit one if
      // state was non-IDLE, so this only kicks in when stop()'s internal
      // check missed something. (Unconditional key-ups hit the CW key line a
      // second time and reset the rig's BK-IN timer — KM4CFT 2026-04-23.)
  ...
  static generateToken() {
    // No look-alike characters (lib/echocat-token.js): a 5 read as an S.
    return EchocatToken.generateToken();
  }
