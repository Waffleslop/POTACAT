...
class RemoteServer extends EventEmitter {
  ...
  _handleHttpRequest(req, res) {
    ...
    // Force cache bust: redirect bare / to /?v=timestamp
    if (pathname === '/' && !url.searchParams.has('v')) {
      res.writeHead(302, { 'Location': '/?v=' + Date.now() });
      res.end();
      return;
    }
  ...
  _buildInlinedHtml() {
    ...
    // When no token required, hide connect screen and show main UI directly in HTML
    if (authMode === 'none') {
      html = html.replace('id="connect-screen"', 'id="connect-screen" class="hidden"');
      html = html.replace('id="main-ui" class="hidden"', 'id="main-ui"');
    }
  ...
  _handleConnection(ws, req) {
    ...
    ws.on('message', (raw) => {
      ...
      if (msg.type && msg.type.startsWith('kiwi')) {
        try { require('fs').appendFileSync(require('path').join(require('os').homedir(), 'AppData', 'Roaming', 'potacat', 'kiwi-debug.log'), `[${new Date().toISOString()}] ws.onmessage: type="${msg.type}" auth=${!!ws._authenticated}\n`); } catch {}
      }
  ...
  _handleMessage(ws, msg) {
    ...
    if (msg.type && msg.type.startsWith('kiwi')) {
      const fs = require('fs');
      const p = require('path').join(require('os').homedir(), 'AppData', 'Roaming', 'potacat', 'kiwi-debug.log');
      fs.appendFileSync(p, `[${new Date().toISOString()}] PRE-AUTH msg.type="${msg.type}" auth=${!!ws._authenticated} isClient=${ws === this._client}\n`);
    }
    if (!ws._authenticated || ws !== this._client) return;

    // Debug: log kiwi messages
    if (msg.type && msg.type.startsWith('kiwi')) {
      const fs = require('fs');
      const p = require('path').join(require('os').homedir(), 'AppData', 'Roaming', 'potacat', 'kiwi-debug.log');
      fs.appendFileSync(p, `[${new Date().toISOString()}] switch msg.type="${msg.type}" auth=${ws._authenticated} isClient=${ws === this._client}\n`);
    }
    ...
    switch (msg.type) {
      ...
      case 'kiwi-connect':
        ...
        require('fs').appendFileSync(require('path').join(require('os').homedir(), 'AppData', 'Roaming', 'potacat', 'kiwi-debug.log'), `[${new Date().toISOString()}] remote-server kiwi-connect: ${JSON.stringify(msg).substring(0, 200)}\n`);
