...
class RemoteServer extends EventEmitter {
  ...
  _handleConnection(ws, req) {
    ...
    if (!this._requireToken && !this._clubMode) {
      ...
      this._client = ws;
      this._sendTo(ws, { type: 'auth-ok', colorblindMode: !!this._colorblindMode, settings: this._remoteSettings, cwAvailable: this._cwEnabled, vfoLocked: !!this._vfoLocked });
  ...
  _handleMessage(ws, msg) {
    ...
    if (msg.type === 'auth') {
      ...
      if (authenticated) {
        ws._authenticated = true;
        this._client = ws;
        const authOk = { type: 'auth-ok', colorblindMode: !!this._colorblindMode, settings: this._remoteSettings, cwAvailable: this._cwEnabled, vfoLocked: !!this._vfoLocked };
        if (member) {
          authOk.member = {
    ...
    switch (msg.type) {
      ...
      case 'paddle':
        ...
        // hard-stop the keyer + force a radio key-up. Without this the iambic
        // state machine can desync and leave the radio transmitting a stuck
        // straight-key tone (reported by KM4CFT 2026-04-22).
        ...
        this._cwPaddleWatchdog = setTimeout(() => {
          ...
          // Belt-and-suspenders: stop() only emits key-up if state wasn't
          // IDLE, so always drive a key-up through the output callback —
          // same pattern as _onClientDisconnected — to ensure the radio RXs.
