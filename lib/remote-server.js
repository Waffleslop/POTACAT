...
class RemoteServer extends EventEmitter {
  ...
  setTunnelExposed(exposed) {
    ...
    if (next) {
      ...
      this.emit('log', '[remote] Cloud Tunnel disabled — auth policy reverts to the configured requireToken / club setting.');
    }
  }
  ...
  _handleHttpRequest(req, res) {
    ...
    // every HTTP route EXCEPT /health (low-info, useful for the
    // operator's own connectivity diagnostics) is closed behind a
    // generic stub. Without this, an unauthenticated visitor — or a
    // scanner — could harvest the app version baked into the
    // renderer, the operator's callsign baked into the UI defaults,
    // the renderer JS source, and confirmation that the
    // /api/pair and /api/ptt endpoints exist on this hostname.
    ...
    // K3SBP 2026-06-02.
    if (this._tunnelExposed && pathname !== '/health') {
      res.writeHead(503, {
...
`;
  ...
  _handleConnection(ws, req) {
    ...
    const requiresAuth = this._requireToken || this._clubMode || this._tunnelExposed;
    const authMode = this._clubMode ? 'club' : (requiresAuth ? 'token' : 'none');
    this._sendTo(ws, { type: 'auth-mode', mode: authMode });

    // If no auth is required (LAN-only deployment, no token, no club
    // mode, no public tunnel), auto-authenticate immediately. This
  ...
  _handleMessage(ws, msg) {
    ...
    if (msg.type === 'auth') {
      ...
      } else if (!this._clubMode && msg.passCode && this._passValidator) {
  ...
  async _authenticatePass(ws, msg, req) {
    ...
    try {
      const profile = await this._passValidator(msg.passCode);
      if (!profile || !profile.code) {
        this.emit('log', `Guest Pass DENIED (code not found / expired / revoked): code=${passCodeMasked} from=${addr}`);
        this._sendTo(ws, { type: 'auth-fail', reason: 'Pass not found, expired, or revoked' });
        return;
      }
      if (this._onPassAuth) {
