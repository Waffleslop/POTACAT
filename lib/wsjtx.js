...
class BinaryWriter {
  ...
  writeUtf8(s) {
    ...
    // Coerce, don't trust: a QSO object reaches this encoder from a dozen log
    // paths that do not agree on types (the Log QSO pop-out sent txPower as a
    // Number), and `Buffer.from(100, 'utf-8')` THROWS — which aborts the whole
    // datagram and loses the QSO, exactly as the WRL sender's escXml did
    // (W9TEF 2026-09-19). One bad type must not cost an operator a contact.
    const strBuf = Buffer.isBuffer(s) ? s : Buffer.from(String(s), 'utf-8');
