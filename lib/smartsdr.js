...
// Forward-power meter value → watts. Flex declares the TX bridge's FWDPWR
// meter in dBm (P[W] = 10^((dBm-30)/10)); a meter that says W is already
// watts. Anything else is treated as dBm, because that is the only unit a
// Flex has ever been seen to report and a dBm→W mistake reads as a
// plausible wattage while a W→W mistake would not (see _parseMeterPacket).
function fwdMeterToWatts(value, unit) {
  const v = Number(value) || 0;
  if (/^w(atts)?$/i.test(String(unit || '').trim())) return v;
...
class SmartSdrClient extends EventEmitter {
  ...
  _setupMeterUdp() {
    ...
    this._fwdMeterUnit = '';   // its declared unit — the Flex bridge says dBm, not W
  ...
  _parseMeterPacket(buf) {
    ...
    for (let i = 0; i < numMeters; i++) {
      ...
      if (meterId === this._smeterMeterId) {
        ...
        // Forward power. The value is raw/128 like the other bridge meters,
        // but the UNIT is what the meter list declares — and the Flex TX
        // bridge declares dBm, not watts (`8 TX- FWDPWR unit=dBm lo=0.0
        // hi=53.0` on an 8600 v4.2.20; 53 dBm = 200 W, the radio's ceiling.
        // K3SBP 2026-09-10). From the day this meter shipped it was emitted
        // as watts, so every wattmeter downstream read dBm with a W after
        // it: 100 W showed "50 W", 20 W showed "43 W", 1 W showed "30 W"
        // — plausible enough that nobody questioned it. Convert here, the
        // one place the unit is known; a meter that declares W passes
        // through. Nothing on the wire is lost: 0 dBm (1 mW) rounds to 0.
        this.emit('fwd-power', fwdMeterToWatts(rawValue / 128.0, this._fwdMeterUnit));
      }
    }
  }
      ...
      for (const [id, m] of Object.entries(meters)) {
        ...
        if (m.src && m.nam) {
          ...
          if ((m.nam === 'FWDPWR' || m.nam === 'FWD_PWR' || m.nam === 'PWR') && !this._fwdMeterId) {
            ...
            this._fwdMeterUnit = String(m.unit || '');
            console.log(`[SmartSDR] Forward-power meter: id ${numId} (nam=${m.nam} src=${m.src} unit=${this._fwdMeterUnit || '?'} hi=${m.hi || '?'})`);
          }
        } else if (m.nam) {
  ...
  /**
   * ATU status — the answer to "did the tune match?":
   *   S<handle>|atu status=TUNE_IN_PROGRESS atu_enabled=1 memories_enabled=1 using_mem=0
   * status= walks TUNE_NOT_STARTED -> TUNE_IN_PROGRESS -> TUNE_SUCCESSFUL /
   * TUNE_OK (memory) / TUNE_FAIL_BYPASS / TUNE_ABORTED; BYPASS states are
   * the resting position. We subscribe (`sub atu all`) on connect so this
   * flows for every tune, whoever started it. Emitted verbatim as
   * 'atu-status'; the SWR guard in main keys its latch off it — until
   * 2026-09-13 the guard re-enabled TX when `atu start` was SENT, before the
   * radio had tried anything (K3SBP, 19:1 on 20 m).
   */
  _parseAtuStatus(line) {
    const m = line.match(/\|atu (.+)$/);
    if (!m) return;
    const kv = {};
    for (const part of m[1].split(' ')) {
      const eq = part.indexOf('=');
    ...
    if (kv.status != null) this.atuStatus = kv.status;
    this.emit('atu-status', kv);
  }
...
module.exports = { SmartSdrClient, setColorblindMode, fwdMeterToWatts };
