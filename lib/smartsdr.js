...
class SmartSdrClient extends EventEmitter {
  ...
  _parseStatusMessage(line) {
    // Meter status: look for any meter-related status
    if (line.includes('meter') || line.includes('Meter') || line.includes('METER')) {
      console.log(`[SmartSDR] meter line: ${line.slice(0, 200)}`);
    }
    // Look for S-meter readings in various SmartSDR formats
    const meterMatch = line.match(/val=(-?[\d.]+)/);
    const isSLevel = /nam=(?:LVL|S_METER|SIG|LEVEL|SIGNAL)/i.test(line) && /src=SLC/i.test(line);
    if (meterMatch && isSLevel) {
