...
class SmartSdrClient extends EventEmitter {
  ...
  _parseMeterPacket(buf) {
    ...
    // Debug: log first packet to see what meter IDs arrive
    if (!this._meterPacketLogged) {
      this._meterPacketLogged = true;
      const ids = [];
      for (let j = 0; j < Math.min(numMeters, 20); j++) {
        const o = payloadStart + j * 4;
        ids.push(`${buf.readUInt16BE(o)}=${buf.readInt16BE(o + 2)}`);
      }
      console.log(`[SmartSDR] vita49 meter packet: ${numMeters} meters: ${ids.join(', ')}`);
    }
    ...
    for (let i = 0; i < numMeters; i++) {
      ...
      if (meterId === this._smeterMeterId) {
        ...
        if (!this._swrFirstLog) { this._swrFirstLog = true; console.log(`[SmartSDR] SWR first value: raw=${rawValue} swr=${swr.toFixed(2)}`); }
        if (swr >= 1.0) {
          // Emit raw SWR ratio directly (not RM1 format) for Flex
