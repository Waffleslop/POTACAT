...
class KiwiSdrClient extends EventEmitter {
  ...
  _parseText(msg) {
    // Only log important messages, not config/load dumps
    if (msg.includes('badp') || msg.includes('too_busy') || msg.includes('audio_init')) {
      console.log(`[KiwiSDR] ${msg.substring(0, 100)}`);
    }
