...
class KiwiSdrClient extends EventEmitter {
  ...
  connect(host, port, password) {
    ...
    this._ws.on('message', (data) => {
      if (typeof data === 'string') {
        ...
        this._parseBinary(Buffer.isBuffer(data) ? data : Buffer.from(data));
      }
    });
  ...
  _parseText(msg) {
    console.log(`[KiwiSDR] Text: ${msg.substring(0, 200)}`);
    // Server text messages: status, version, config, etc.
    if (msg.startsWith('MSG')) {
      if (msg.includes('too_busy')) {
        ...
        this.disconnect();
      } else if (msg.includes('audio_rate')) {
