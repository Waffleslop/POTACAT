...
class KiwiSdrClient extends EventEmitter {
  ...
  tune(freqKhz, mode, bwLow, bwHigh) {
    ...
    if (bwLow != null && bwHigh != null) {
      ...
      switch (m) {
        ...
        // CW: passband must be wide enough to catch any rig's BFO pitch.
        // KiwiSDR's CW BFO defaults to 500 Hz but some servers run 600 or
        // 700, and rigs use anywhere from 400 to 1000 Hz. The old 300-800
        // window was too narrow — DA2PK on Neandertal/DF0TWN got silence
        // when keying at 7045 because his rig's CW pitch landed outside
        // the filter (2026-05-03). Widen so anyone's BFO is audible.
        case 'cw':  low = 200; high = 1200; break;
  ...
  _parseText(msg) {
    ...
    if (msg.includes('too_busy')) {
      ...
      if (code !== 0) {
        // Reference semantics (jks-prv/kiwiclient client.py): the value is a
        // slot COUNT — "all N client slots taken" — and the message itself is
        // a refusal. On chan_no_pwd=0 receivers it arrives ~10s into a
        // streaming session: password-less users get a courtesy preview slot,
        // then the bump. "KiwiSDR is full" was wrong (8 idle channels) and
        // unactionable — N3VD burned four attempts across two receivers with
        // no hint the receiver's POLICY was the cause (work item
        // kiwi-preview-bump-too-busy, 2026-08-26). This string reaches the
        // phone verbatim, so it is the whole of the user's explanation.
        this.emit('error', 'This KiwiSDR limits sessions for users without its password (' + code + ' public slot' + (code === 1 ? '' : 's') + ' taken) — it granted a short preview, then bumped us. Pick a different receiver, or one whose password you have.');
        this.disconnect();
      }
