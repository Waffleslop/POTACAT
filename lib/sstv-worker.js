...
class SstvDecoder {
  ...
  decodeYCbCrLine(freqs, y, _syncPeakIdx) {
    ...
    if (mode.halfChrom) {
      ...
      //
      // Sample the middle 60% of the separator window to avoid BPF
      // ring-up at the edges. 1900 Hz is the midpoint between BLACK
      // and WHITE; below that → Cr, above → Cb. (We use the freqs[]
      // already-demodulated buffer, no new DSP needed.)
      let isCrLine;
      ...
      if (sepLen > 0) {
        ...
        for (let i = start; i < end && i < freqs.length; i++) {
          if (freqs[i] > 0) { sum += freqs[i]; n++; }
        }
        if (n > 0) {
          const avgFreq = sum / n;
          isCrLine = avgFreq < 1900; // < 1900 Hz → BLACK separator → Cr
        } else {
          isCrLine = (y % 2) === 0; // fallback: parity (legacy behavior)
        }
      } else {
        isCrLine = (y % 2) === 0;
      }
