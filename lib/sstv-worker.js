...
class SstvDecoder {
  constructor() {
    ...
    this.freqOffset = 0;      // Hz to subtract from all measurements (leader should be 1900)
    this.leaderFreqAccum = 0; // accumulate leader freq for calibration
  ...
  measureFreq(sample) {
    ...
    return (this.freqSmoothSum / this.freqSmoothCount) - this.freqOffset;
  }
  ...
  // STATE_IDLE: wait for sustained ~1900 Hz (wide band pre-calibration)
  _stateIdle(freq) {
    if (freq > 1700 && freq < 2150) {
      this.leaderCount++;
      // Accumulate leader frequency for calibration
      this.leaderFreqAccum += freq;
      this.leaderFreqCount++;
      if (this.leaderCount >= this.leaderMinSamples) {
        // Calibrate: leader should be 1900 Hz, compute offset
        ...
        return { type: 'rx-debug', state: 'LEADER', avgFreq: Math.round(freq), detail: 'Leader detected, offset=' + Math.round(this.freqOffset) + ' Hz (' + (this.leaderCount / SAMPLE_RATE * 1000).toFixed(0) + 'ms)' };
      }
    } else {
      this.leaderCount = Math.max(0, this.leaderCount - 2);
      // Decay calibration accumulators too
      if (this.leaderCount === 0) { this.leaderFreqAccum = 0; this.leaderFreqCount = 0; }
    }
    return null;
  }
  ...
  _stateLeader(freq) {
    ...
    if (!this.breakSeen) {
      ...
      if (isLeader) {
        ...
        // Still in break area (before or between leader segments) — reset grace
        this._transitionGrace = 0;
      } else {
        // Transition zone — allow generous grace for smoothing lag through break+leader
        this._transitionGrace = (this._transitionGrace || 0) + 1;
        if (this._transitionGrace > Math.round(SAMPLE_RATE * 0.030)) { // 30ms grace (break=10ms + smoothing)
          this.state = STATE_IDLE;
          this.leaderCount = 0;
    ...
    switch (msg.type) {
      ...
      case 'set-sample-rate':
        if (msg.sampleRate && msg.sampleRate !== SAMPLE_RATE) {
          ...
          decoder.reset();
          // Recalculate sample-dependent thresholds
          decoder.leaderMinSamples = Math.round(SAMPLE_RATE * 0.1);
          decoder.visExpectedSamples = Math.round(SAMPLE_RATE * 30 / 1000);
        }
        break;
