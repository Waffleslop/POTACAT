...
class SstvDecoder {
  ...
  _emitImage() {
    ...
    // Two-pass re-decode was tried (commit-bench 2026-05-31) but the
    // naive form using the FINAL slantFactor regressed PSNR by 2-5 dB
    // across all drifted modes. Root cause: the regressor oscillates
    // (-993 → -1006 → -1023 ppm across iterations), so the FINAL
    // slantFactor isn't the GLOBAL best-fit for any individual line —
    // it's the best-fit for whatever lines fed the last regression.
    // Re-applying it to early lines moves them FURTHER from optimum.
    // Two-pass needs a different design (e.g. capture raw freq
    // samples + re-detect line boundaries from scratch with the
    // converged factor). Left _lineHistory wiring in place as the
    // foundation for that future work; it's a no-op now.
    const result = {
      type: 'rx-image',
