...
class SstvDecoder {
  constructor() {
    ...
    // 48 samples at 48kHz = 1ms — balances HF noise tolerance with VIS bit timing (30ms/bit)
    this.freqSmoothBuf = new Float64Array(48);
  ...
  measureFreq(sample) {
    ...
    const rawFreq = -dPhase * SAMPLE_RATE / TWO_PI;
  ...
  decodeMartinLine(freqs, y) {
    ...
    let offset = mode.syncMs + mode.porchMs;
  ...
  decodeScottieLine(freqs, y) {
    const mode = this.mode;
    const w = mode.width;
    // Scottie: sep(1.5) + G(138.24) + sep(1.5) + B(138.24) + sync(9.0) + porch(1.5) + R(138.24)
    let offset = mode.separatorMs;
    const gVals = this.extractChannel(freqs, offset, mode.scanMs, w);
    offset += mode.scanMs + mode.separatorMs;
    const bVals = this.extractChannel(freqs, offset, mode.scanMs, w);
