...
function mutateLog(logPath, mutate, { sink } = {}) {
  ...
  for (const rec of after) {
    const uuid = rec.APP_POTACAT_UUID;
  ...
  for (const [rec, prev] of snapshot) {
    if (!kept.has(rec) && prev.fields.APP_POTACAT_UUID) gone.set(prev.fields.APP_POTACAT_UUID, prev);
  }
  ...
  for (const [rec, prev] of snapshot) {
    ...
    const uuid = prev.fields.APP_POTACAT_UUID;
...
function appendRecords(logPath, records, { existing, sink } = {}) {
  ...
  const inUse = new Set((existing || readLog(logPath)).map((r) => r.APP_POTACAT_UUID).filter(Boolean));
  for (const rec of records) {
    if (!rec.APP_POTACAT_UUID || inUse.has(rec.APP_POTACAT_UUID)) {
      rec.APP_POTACAT_UUID = crypto.randomUUID();
      rec.APP_POTACAT_VERSION = '1';
...
module.exports = { mutateLog, appendRecords, repairFieldNames, readLog, fingerprint };
