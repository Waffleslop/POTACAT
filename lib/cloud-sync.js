  ...
  _request(method, path, body, skipAuth = false) {
    return new Promise((resolve, reject) => {
      ...
      const req = transport.request(options, (res) => {
        ...
        res.on('end', () => {
          // 5xx from the gateway (nginx 502/503/504) means the upstream API
          // is down or restarting. The body is HTML, not JSON, so the JSON
          // parse below would surface a wall of `<html>...` text in an alert.
          // Map these to a single readable line — users just need to know
          // it's not their fault and to retry.
          if (res.statusCode >= 500 && res.statusCode < 600) {
            const label = res.statusCode === 503 ? 'temporarily unavailable'
                       : res.statusCode === 502 ? 'gateway error'
                       : res.statusCode === 504 ? 'gateway timeout'
                       : 'error';
            reject(new Error(`POTACAT cloud is ${label} (HTTP ${res.statusCode}). Please try again in a few minutes — if it keeps happening, email casey@potacat.com.`));
            return;
          }
          try {
