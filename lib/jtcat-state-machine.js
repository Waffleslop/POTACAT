...
/**
 * Does a decoded CQ satisfy a hunt filter?
 *
 * The text half is the original rule: 'all' takes any CQ, 'pota'/'sota'/'fd'
 * take the matching CQ modifier.
 *
 * The second half is why this moved out of main.js. POTA/SOTA hunt used to
 * require the decode to literally start "CQ POTA " / "CQ SOTA ", and most
 * activators simply call CQ — the modifier costs characters in a
 * 13-character message and buys the activator nothing, because their hunters
 * came from the spot page, not from the airwaves. So POTA hunt could sit
 * silent through an entire activation it was hearing perfectly (feature
 * request 2026-09-02). The spot list is the same authority a hunter uses by
 * hand, and POTACAT already has it open: a station spotted at a park who is
 * calling CQ on our band is an activator, whatever the message says.
 *
 * `spottedSigs` are the program SIGs (POTA/SOTA/WWFF/...) that the live spot
 * list places this caller at RIGHT NOW — the caller supplies them, so this
 * stays pure. An empty list means "not spotted", which leaves the original
 * text-only behaviour exactly as it was.
 *
 * 'event' (popout "Hunt: Event stations", offered only while an opted-in
 * checklist event is near its window) answers a CQ from a tracked-event
 * station the operator still needs — Route 66 On The Air, 13 Colonies. The
 * caller supplies `eventNeeded` from the same needed / new-slot / worked
 * classification the decode badge shows (lib/event-decode-match.js), so the
 * hunt and the badge can never disagree about who is still wanted. The CQ's
 * wording is irrelevant (an event station calls "CQ", "CQ NA", "CQ POTA" —
 * whatever it likes); being a CQ at all is not.
 *
 * @param {string} text            decoded message
 * @param {string} filterMode      'all' | 'pota' | 'sota' | 'fd' | 'event'
 * @param {object} [opts]
 * @param {string[]} [opts.spottedSigs]     SIGs this caller is spotted at
 * @param {boolean}  [opts.spottedEnabled]  false = text rule only (setting off)
 * @param {boolean}  [opts.eventNeeded]     this caller is a tracked-event
 *                                          station still needed (event mode)
 */
function matchesHuntFilter(text, filterMode, opts) {
  const upper = String(text || '').toUpperCase();
  if (!upper.startsWith('CQ ')) return false;          // never answer a non-CQ
  const mode = filterMode || 'all';
  if (mode === 'all') return true;
  if (mode === 'pota' && upper.startsWith('CQ POTA ')) return true;
  if (mode === 'sota' && upper.startsWith('CQ SOTA ')) return true;
  ...
  // the class+section exchange, not grid/report. Never spot-matched: Field
  // Day is not a spotting program.
  if (mode === 'fd') return upper.startsWith('CQ FD ');
  // Event hunt: the station, not the message, is what qualifies.
  if (mode === 'event') return !!(opts && opts.eventNeeded);
  if (mode !== 'pota' && mode !== 'sota') return false;
  const o = opts || {};
  if (o.spottedEnabled === false) return false;
  const want = mode === 'pota' ? 'POTA' : 'SOTA';
  return Array.isArray(o.spottedSigs) && o.spottedSigs.includes(want);
}
