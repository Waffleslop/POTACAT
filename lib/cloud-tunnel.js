...
class CloudTunnelManager extends EventEmitter {
  ...
  constructor(opts) {
    ...
    // Origin self-test (K5AWJ 2026-09-17). The cloud can only say whether
    // cloudflared is connected; whether THIS app is answering behind it is
    // main.js's to know (lib/origin-health.js). `probeOrigin` returns the
    // decided { state, label, reason }; it runs after every healthy tick
    // and whenever main calls recheckOrigin(). Never faster than the tick
    // on its own — it is local, but the tick cadence is a cost guardrail.
    this._probeOrigin = typeof opts.probeOrigin === 'function' ? opts.probeOrigin : null;
    this._origin = null; // { state, label, reason, actionable, checkedAt } | null when off
    this._originProbeRunning = false;
  ...
  getState() {
    return {
      ...
      // origin: whether POTACAT's own server is answering behind the
      // tunnel. `state === 'ok'` is the only value that earns a green
      // "Live" and a clickable <callsign>.potacat.com link.
      origin: this._origin,
  ...
  /**
   * Re-run the origin self-test now — main.js calls this when the local
   * server binds, fails to bind, or stops, so the card flips within a
   * second instead of at the next 5-minute tick.
   */
  async recheckOrigin() {
    if (!this._enabled || !this._probeOrigin) {
      if (this._origin) { this._origin = null; this.emit('change', this.getState()); }
      return null;
    }
    if (this._originProbeRunning) return this._origin;
    this._originProbeRunning = true;
    try {
      const next = await this._probeOrigin();
      const prevState = this._origin && this._origin.state;
      this._origin = next ? Object.assign({}, next, { checkedAt: Date.now() }) : null;
      const nextState = this._origin && this._origin.state;
      if (nextState !== prevState) {
        if (nextState && nextState !== 'ok' && nextState !== 'pending' && nextState !== 'off') {
          this._log(`[cloud-tunnel] ORIGIN: ${this._origin.label} — ${this._origin.reason}`);
        } else if (prevState && prevState !== 'ok' && nextState === 'ok') {
          this._log('[cloud-tunnel] origin recovered — POTACAT is answering behind the tunnel again');
        }
        this.emit('change', this.getState());
      }
    } catch (err) {
      this._log('[cloud-tunnel] origin probe failed: ' + (err && err.message ? err.message : err));
    } finally {
      this._originProbeRunning = false;
    }
    return this._origin;
  ...
  async _checkOnce() {
    ...
    try {
      ...
      if (res && res.healthy) {
        ...
        // The cloud vouches for cloudflared; now check the origin it
        // forwards to. Healthy tunnel + dead origin was the invisible case.
        await this.recheckOrigin();
      } else {
  ...
  async enable() {
    ...
    try {
      ...
      if (err.code === 'subscription_required' || /402|entitlement|subscription/i.test(msg)) {
        this._lastError = msg;
        this._setStatus('off');
  ...
  _setStatus(next) {
    ...
    if (next === 'off') this._origin = null; // nothing to be behind
