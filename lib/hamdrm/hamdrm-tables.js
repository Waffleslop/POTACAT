...
// Which branches are emitted for each PP_TYPE, in order. QSSTV's ConvEncoder::
// Encode calls `Convolution(reg, b)` for each b in this list — so the order
// matters for byte-for-byte interop. The naming bN...b0 is MSB..LSB, i.e.
// PP_TYPE_0001 emits branch 0 only; PP_TYPE_0111 emits branches 0, 1, 2; etc.
const PP_BRANCH_ORDER = {
  [PP_TYPE_0000]: [],
  [PP_TYPE_1111]: [0, 1, 2, 3],
  [PP_TYPE_0111]: [0, 1, 2],
  [PP_TYPE_0011]: [0, 1],
  [PP_TYPE_0001]: [0],
  [PP_TYPE_0101]: [0, 2],
...
// Lookup: how many output bits each PP_TYPE emits.
const PP_OUT_COUNT = {
  [PP_TYPE_0000]: 0, [PP_TYPE_1111]: 4, [PP_TYPE_0111]: 3,
  [PP_TYPE_0011]: 2, [PP_TYPE_0001]: 1, [PP_TYPE_0101]: 2,
};
